...
The pending-revision line is there because this example workspace has a
...
The block under **my_context product rules** is the one part of that text that did not come from
this project. Those are constants about my_context itself: they ship inside the package, arrive
at every session start and every subagent dispatch, and spend none of this project's injection
budget. `mycontext list` does not name them and nothing in a workspace can edit or supersede
them — read them with `mycontext rules list`.
...
An item is set to `always: true` at capture with `mycontext add … --always`, by promoting it
with `mycontext review promote <id> --always` while it is still a draft, or by `mycontext
pin <id>` once it governs. The first and the last ask you to confirm, and each shows what
the pin costs — `add` prints the pinned budget in the confirmation a normative capture
...
| `mycontext conversation [rebuild\|list\|subagents\|persist\|forget]` | the **conversation archive**: index the transcripts Claude Code has already written for this project, and list what the index holds. Nothing is recorded, and nothing is copied unless you ask for it by name — the transcripts are read where the harness put them, and the index is one row per session that a rebuild reconstructs entirely from disk, so losing it costs time and never knowledge. `rebuild` re-reads only transcripts whose size or mtime has moved; `--full` re-reads them all. `list` prints the sessions newest first with the counts of what a person asked and what the model answered — **not** a count of `message.role`, which would report every tool result as something you typed. A transcript longer than the scan cap is indexed with its counts marked as floors rather than silently short. `subagents` lists the helper agents one session dispatched, with the id of the `Agent` call that dispatched each. `persist <session>` is the one thing here that writes outside your project: it marks one session to be KEPT — a copy under `~/.my-context/conversations/`, brought up to date at the end of every assistant turn by tailing the bytes the transcript has grown by — so that a session the harness later prunes stays readable in the same viewer, marked as the copy. It says what a copy of a session holds before it asks, and writes nothing without `--yes`; `--off` stops the copying and deliberately leaves the file. `forget` drops the whole index and the marks with it, leaves every copy alone, and stands the end-of-turn refresh down. Browsing and reading one is the [Conversations screen](#the-web-ui--mycontext-ui); this command is what fills the index for it, because the web UI never writes. `--limit`, `--yes`, `--off`, `--json` |
...
`mycontext pack import <path>` reads a pack somebody else wrote — a directory or a ZIP —
...
**A full export is refused outright, before anything else about it is even read.** It is an
archive to copy back — with `cp`, `git clone`, or however it reached you — not something this
command imports. `mycontext export --as-pack` (above) is what projects a corpus into
something `pack import` reads; a plain `mycontext export` writes the other kind, and this is
the command that will not take one, `--name` included.
...
you pass it. A full export never reaches this flag at all: it carries no name and is refused
outright before `--name` is even considered — see above.
...
export — which carries none — is refused there too, the same artefact `mycontext pack
import` also will not take: run `mycontext export --as-pack` first to write something either
command can found a workspace from.
...
<!-- `ui/watch-model.ts` · `import { AUDIT_KINDS, type AuditKind, type AuditRecord } from '../core/audit.ts';` · ~3 -->
...
| `--request "<text>"` | the words a **person** wrote when they asked for this item, copied verbatim — their spelling, their punctuation, their line breaks. It is documentation and nothing else: it is never injected into a session, it is not part of what a summary is measured against, and it is outside the item's checksum, so recording one makes nothing stale and clearing one restores the file byte for byte. Do **not** paraphrase, tidy or translate it, and never write one from what the item says — an absent request truthfully means nobody recorded one, and an invented one is a quotation of something that was never said, which nothing afterwards can tell apart from the real thing. On `edit` it records a request on an item that has **none**, `--request=` removes one, and an item that already records one is refused: a correction is a new request, not an improved old one. The same field is the `request` argument of `create_item` and `update_item` <!-- `core/validate.ts` · `export function validateRequest(request: string): void {` · ~578 --> | `add`, `edit` |
...
| `--always` | pin the item: inject it in full at every session start, whatever files you touch. On `add` it pins **at capture**, in one act rather than a capture followed by a second command — which is what an item carried in from another corpus needs, since seven of the forty-four in `.my_context.nested-44/` are already pinned. `review promote --always` sets it while the item is still a draft; `mycontext edit --always` sets it, or `--always=false` clears it, at any point — behind the confirmation an item that already governs earns. `mycontext pin <id>` and `mycontext unpin <id>` are those two edits under a shorter name. It is the most expensive thing a capture can ask for, because the pinned tier is **one** budget shared by every pinned item in the workspace: an item that no longer fits is not injected at all, and the only place that is ever said is the session-start hook. `add` therefore prints the budget in its normative confirmation before asking, and adds no second gate — every capture the flag can land on is already behind `--yes`, because a rationale-tier category refuses it outright (selection admits only normative items to the pinned tier, so the field would be stored governing nothing). Omitting it captures the item unpinned; `--always=false` says the same thing in words, is accepted on `add`, and is not advertised there because it asks for the default | `add`, `review promote`, `edit` |
...
| Flag | What it does | Where it works |
|---|---|---|
| `--name <text>` | what to call the pack **here**: the directory its history is filed under, and the name `mycontext pack list` shows. It defaults to the name the pack's own manifest declares. A full export carries no name and never reaches this flag: it is refused outright before `--name` is considered. Two packs that call themselves the same thing are kept apart without it — each import is filed under its name *and* where it was read from — so this is how you tell them apart by name rather than by source | `pack import` |
...
| `--pack <path>` | found this workspace from an artefact, in the same command that creates it. It is the only flag `mycontext init` accepts, and everything else — a positional, `--global`, `--yes`, `--overwrite-changed` — is still refused by name. It asks nothing and takes no `--name`, so a full export is refused there too, the same artefact `pack import` also will not take | `init` |
...
--severity/--always`. One surface cannot express `always` at all rather than refusing it —
ingest hardcodes `always: false` for every candidate — so no route stores one and ignores
it. (`mycontext add --always` was on that list until 2026-09-03; it now carries the
refusal instead, which is the stronger answer: a migration that pins at capture is told
so on the spot rather than finding the flag missing.) Selection admits only
...
only one of the fifteen that does it on a *stranger's* behalf: everything a pack brings in
...
widens, and setting an axis, which narrows. The other three — `--show`, `--preview` and
`--relations` — report and change nothing, and they **refuse** `--yes` by name rather than
ignoring it, because a confirmation accepted where nothing is confirmed teaches a reader that
this command asks before it reports. A permission rule cannot tell the two apart — it matches
the command string — so the rule below denies the whole command, and the reporting forms are
the price.

`config` is the newest member, and the other one that changes no single item: `mycontext
config <name> --delete|--disable --yes` acts on a whole **category** in `config.json` rather
than on an item's own file. `--disable` — legal on a shipped or custom category alike — stops
new captures of that type and drops every item that already carries it out of selection for
injection, without editing or deleting a single one of them; a shipped category can never be
un-recognised, only turned off this way, and `--delete` refuses it outright, naming
`--disable` as the alternative. On a **custom** category, `--delete` removes the whole entry
from `config.json`, so this workspace stops recognising what it names and every item that
already carries it falls back to the absent-category defaults for scope and edit policy.
Either write backs up `config.json` first, when one exists, and prints where; and either warns
with the real number of items the category already carries before the gate — the count is the
blast radius of the one write about to happen, not a second consent.
