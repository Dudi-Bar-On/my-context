...
table inside `status --full`, both for the same width. `mycontext review list` was never
over the layout — it fits with its title in place — and keeps the column.
...
`status --full` lost the same column for the same reason. `review list` did not: it was
never over the layout, so it keeps its title.
