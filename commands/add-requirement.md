...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add requirement "<title>" --body "<why it holds>" --scope "<glob>" --tags "<tag>" --yes` captures the same fields from a
