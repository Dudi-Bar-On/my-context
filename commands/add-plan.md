...
Capture a **plan** — A named body of work: its goal, the order it is taken in, and the condition that finishes it — in this project's my_context
knowledge base.
...
2. Call the `create_item` tool on the `mycontext` MCP server with
   `type: "plan"` and a `title` that states the claim in one sentence
...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add plan "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>"` captures the same fields from a
