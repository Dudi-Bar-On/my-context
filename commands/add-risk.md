...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add risk "<title>" --body "<why it holds>" --scope "<glob>" --tags "<tag>"` captures the same fields from a
