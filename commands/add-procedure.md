...
Capture a **procedure** — An ordered operation performed once and then finished; a repeatable one is a runbook — in this project's my_context
knowledge base.
...
2. Call the `create_item` tool on the `mycontext` MCP server with
   `type: "procedure"` and a `title` that states the claim in one sentence
...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add procedure "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>" --yes` captures the same fields from a
