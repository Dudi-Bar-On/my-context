...
Capture a **decision** — Lightweight decision not warranting a full ADR — in this project's my_context
knowledge base.
...
2. Call the `create_item` tool on the `mycontext` MCP server with
   `type: "decision"` and a `title` that states the claim in one sentence
...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add decision "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>"` captures the same fields from a
