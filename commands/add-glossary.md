...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add glossary "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>" --yes` captures the same fields from a
