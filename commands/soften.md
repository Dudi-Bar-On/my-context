---
description: Make a normative item advisory (edit --severity=soft)
argument-hint: "[the item id]"
disable-model-invocation: true
---

`soften` an item in this project's my_context knowledge base: it sets `severity` to soft, so the item advises rather than binds.
...
2. Run it WITHOUT `--yes`, exactly as written:
   ...
   `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" soften <id>`
...
4. Print the same command with `--yes` on the end, for the USER to run, and stop.
   ...
   `soften` is `mycontext edit <id> --severity=soft` under a shorter name — same gate,
