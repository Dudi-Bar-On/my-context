...
argument-hint: "[the open-question in one sentence — Deliberately undecided; the agent must not decide it alone]"
disable-model-invocation: true
---

Capture a **open_question** — Deliberately undecided; the agent must not decide it alone — in this project's my_context
knowledge base.
...
2. Call the `create_item` tool on the `mycontext` MCP server with
   `type: "open_question"` and a `title` that states the claim in one sentence
...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add open_question "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>" --yes` captures the same fields from a
