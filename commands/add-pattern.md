...
argument-hint: "[the pattern in one sentence — Reusable solution, or an anti-pattern to avoid]"
disable-model-invocation: true
---

Capture a **pattern** — Reusable solution, or an anti-pattern to avoid — in this project's my_context
knowledge base.
...
2. Call the `create_item` tool on the `mycontext` MCP server with
   `type: "pattern"` and a `title` that states the claim in one sentence
...
If the MCP server is not available, `node "${CLAUDE_PLUGIN_ROOT}/src/cli/index.ts" add pattern "<title>" --body "<why it holds>" --summary "<one plain sentence>" --scope "<glob>" --tags "<tag>" --yes` captures the same fields from a
