...
function instructionsFor(request: Omit<ExtractionRequest, 'instructions'>): string[] {
  ...
  return [
    ...
    '"scope", "tags" and "observations" must each be a JSON ARRAY — never a bare string. Set "scope" only to the directories the item actually governs, as POSIX globs such as "src/auth/**". "**", "*" and "**/*" are all rejected as too broad. Omitting scope is safe: the item is still indexed and searchable, it is simply never auto-injected.',
