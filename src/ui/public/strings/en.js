...
export const strings = {
  ...
  'conv.help.body': 'Claude Code already writes every session to a file on your disk. This screen reads those files where they are and never copies them, so nothing here enters your repository. A session your machine has deleted is gone from here too — that is the cost of not copying. The list is refreshed at the end of each of your turns, so it keeps up with a session while you are still in it. It never refreshes itself when you open this page: reading is all this app does. To bring it up to date right now, run this in a terminal: {m:mycontext conversation rebuild}',
  ...
  'conv.arch.idle': 'Type at least {n} characters. Two words you remember are enough: they are looked for next to each other, then in the same sentence, then anywhere in the same turn, in any language. Put a hyphen in front of a word to leave it out.',
  'conv.arch.searching': 'Reading the archive…',
  'conv.arch.matched': '{n} passage(s) hold those words, best match first.',
  /* -- THE THREE READINGS OF ONE QUERY -- `semantic/4`, search-grammar report Section 5 -- */
  'conv.arch.tier.phrase': 'Your words, next to each other',
  'conv.arch.tier.near': 'Your words within {n} characters of each other — the same sentence',
  'conv.arch.tier.both': 'All your words, somewhere in the same turn',
  'conv.arch.tierBounded': 'Showing {shown} here; this reading matches {matched} passage(s) in all. The ones it does not show are behind the bound — narrow by session, by who spoke or by date to reach them.',
  'conv.arch.short': 'Too short to look for on its own: {words}. This index reads runs of {n} characters, so a shorter word cannot be matched at all. It is still part of the first reading below, where the letters around it can match, and it is left out of the other two. That is a limit of the index, not an answer about the archive.',
  'conv.arch.excluded': 'Leaving out {words}. A word with a hyphen in front of it is removed from every reading — delete the hyphen to bring it back.',
  ...
  'conv.anchors.gotoAt': 'Go to byte {mv:byte} in {where}',
   ...
   * subagent that reads in its own window, verifies against the code and git,
   * and returns a small cited file - and then he reads that file and chooses
   * what, if anything, comes back. Every word below is about that order, and
   * `conv.recall.safe` is the one sentence the whole feature rests on. */
  'conv.recall.h': 'Reconstruct a subject',
  'conv.recall.sub': 'Paste a passage you remember, or ask what was being worked on. This does not answer you from the archive — it writes a brief for a helper that reads in a window of its own, checks what it finds against the code and the history, and writes back a short, cited file. Nothing reaches your context until you choose it.',
  'conv.recall.mode.h': 'How do you want to come at it?',
  'conv.recall.mode.from-selection': 'from a passage I copied',
  'conv.recall.mode.free-text': 'from what I remember it said',
  'conv.recall.mode.list-subjects': 'I am lost — what was being worked on?',
  'conv.recall.mode.list-anchors': 'where were the fixed points?',
  'conv.recall.passage.label': 'The passage:',
  'conv.recall.passage.hint': 'Paste it here. A passage worth copying is dense in NAMES — item ids, file paths, function names — and names are what this matches on: measured against this archive, item ids matched 68% of the time where headings matched 4%.',
  'conv.recall.scope.h': 'Where to look',
  'conv.recall.scope.session': 'Session',
  'conv.recall.scope.any': 'every session',
  'conv.recall.scope.from': 'From',
  'conv.recall.scope.to': 'To',
  'conv.recall.prepare': 'Write the brief',
  'conv.recall.preparing': 'Working out what to look for…',
  'conv.recall.query.h': 'What it will look for',
  'conv.recall.query.names': 'Names:',
  'conv.recall.query.terms': 'Terms:',
  'conv.recall.query.none': 'Nothing in that passage can be matched against the archive. It is not searching anyway — a guess that finds something is worse than no answer at all. Paste something that names things: an item id, a file path, a function name.',
  'conv.recall.mission.h': 'The brief, as the helper will read it',
  'conv.recall.mission.hint': 'Copy this into a fresh window and let it run. It carries POINTERS and never the passages themselves, which is the whole point: the noise never enters a context window, including the helper’s.',
  'conv.recall.mission.copy': 'Copy the brief',
  'conv.recall.mission.where': 'It will write its answer into the retrieval directory, and this screen lists what is there.',
  'conv.recall.results.h': 'What has been asked before',
  'conv.recall.results.none': 'Nothing has been asked yet. When a helper writes one, it appears here.',
  'conv.recall.results.dir': 'They live in {mv:dir}, which is gitignored — they hold conversation text, and conversation files stay out of git.',
  'conv.recall.results.leak': 'WARNING: {mv:dir} is NOT ignored by this checkout’s .gitignore. A result holds conversation text. Do not commit until that is fixed.',
  'conv.recall.results.row': '{n} claims · {mode} · {at}',
  'conv.recall.open': 'Read it',
  'conv.recall.result.h': 'What it found',
  'conv.recall.result.aged': 'This has AGED: {n} of its citations no longer resolve. Read it as a record of what was true, and check the ones that matter.',
  'conv.recall.result.unchecked': '{n} citations could not be checked from here — a turn needs the archive index and a commit needs git, and this screen opens neither. Not checked is not the same as checked and fine.',
  'conv.recall.result.finding': 'Something is wrong with this result:',
  'conv.recall.result.pick': 'Tick what you want back. You can take one line — a table you were looking for — and leave the rest.',
  'conv.recall.result.deeper': 'Go deeper on this one',
  'conv.recall.round2': 'Round 2 — going into one subject',
  'conv.recall.round2.of': 'The subject you picked: {mv:subject}',
  'conv.recall.return': 'Show me what would come back',
  'conv.recall.returning': 'Marking it…',
  'conv.recall.marked.h': 'What would come back, marked',
  'conv.recall.safe': 'NOTHING HAS BEEN SENT ANYWHERE. This page cannot put anything into a context window — it can only show you the text. Copying it is your act, and so is pasting it.',
  'conv.recall.marked.copy': 'Copy it, to paste into this session',
  'conv.recall.marked.portion': '{chosen} of {total} claims. {left} were left behind, and the text above says so.',
  'conv.recall.reversed.h': 'Reversed since — read these first',
  'conv.recall.reversed.one': '{mv:id} is {status} and is answered by {mv:by}.',
  'conv.recall.reversed.orphan': '{mv:id} is {status}, and the corpus records nothing that answers it.',
  'conv.recall.unknown.h': 'Named here, and not found in the corpus',
  'conv.recall.unknown.hint': 'Nothing is claimed about these either way. They may have been renamed, or may never have been items.',
  'conv.recall.fresh.h': 'Or put it into a fresh window instead',
  'conv.recall.fresh.hint': 'A result brought into this session competes with everything already in it — which is often the situation you are trying to escape. Into a cleared window it arrives into room. Staging writes it to a file FIRST, so clearing cannot destroy it, and nothing is delivered until you approve it and clear the window yourself.',
  'conv.recall.fresh.stage': 'Stage it for a fresh window',
  'conv.recall.fresh.staging': 'Writing it to disk…',
  'conv.recall.fresh.then': 'Staging leaves a proposal. Approving it is a separate act and it is yours, and so is the clear — there is no command for the clear anywhere in this product, and no button for it here.',
  'conv.recall.staged.ok': 'Staged as {mv:key} and read back off the disk — {bytes} bytes, in {mv:file}. NOTHING HAS BEEN DELIVERED: it is a proposal until you approve it, and a session starting now would receive nothing.',
  'conv.recall.staged.failed': 'NOT SAFE TO CLEAR — {reason} Do not clear this window: what is on disk is not what you were shown, so clearing now would lose this rather than move it.',
  'conv.recall.approve': 'Approve it for the next session',
  'conv.recall.approve.opening': 'Reading back what is staged…',
  'conv.recall.approve.h': 'What you are approving',
  'conv.recall.approve.hint': 'This is the short form, read back off the staged record rather than from this page — the payload itself is {bytes} bytes and this is what it is short for. Approving does not inject anything now: the NEXT session that starts in this project receives it once. Approve first, then clear your window, in that order.',
  'conv.recall.approve.yes': 'Yes — approve it',
  'conv.recall.approving': 'Approving…',
  'conv.recall.approved': 'IT IS SAFE TO CLEAR. {mv:key} is approved and was re-read on disk — {bytes} bytes. The next session that starts in this project receives it once, and the record is then marked delivered. Clearing this window is yours to do.',
  'conv.recall.approve.unsafe': 'NOT SAFE TO CLEAR — {reason} Do not clear this window.',
  'conv.recall.form.h': 'What you would be approving',
  'conv.recall.copied': 'Copied. Nothing has been sent anywhere — paste it where you want it.',
  ...
  'conv.doc.syn.task': 'Background task finished',
  'conv.doc.syn.slash': 'Slash command',
  'conv.doc.syn.meta': 'Harness note',
  'conv.doc.syn.reminder': 'System reminder',
  'conv.doc.syn.compaction': 'Carried over from an earlier window',
  'conv.doc.subagent': 'Subagent',
  'conv.doc.shell': 'Shell',
  'conv.doc.thinking': 'Thinking',
  'conv.doc.thinkingNever': 'Claude’s reasoning is not written to a transcript, so these steps record that it happened and can never carry its words.',
  'conv.doc.deed.ask': 'Question put to you',
  'conv.doc.deed.ran': 'Command',
  'conv.doc.deed.ok': 'succeeded',
  'conv.doc.deed.bad': 'failed',
  'conv.doc.deed.chose': 'chosen',
  'conv.doc.deed.noAnswer': 'No answer was given to this question.',
  'conv.doc.deed.noResult': 'No result for this call is recorded in the file.',
  ...
  'conv.nav.foundPrev': 'Previous match',
  'conv.nav.foundNext': 'Next match',
  'conv.nav.founds': '{n} turn(s) here hold what you typed.',
  ...
  'conv.nav.marksNoneKind': 'No marked point here is of this kind — {kind}. Choose every kind above to step through the rest.',
  'conv.nav.atMarkKind': '{kind}, {n} of {total}:',
  'conv.nav.markLastKind': 'Nothing of this kind — {kind} — is marked after this point. It is the last one of its kind in the conversation, and it is still where you are.',
  'conv.nav.markFirstKind': 'Nothing of this kind — {kind} — is marked before this point. It is the first one of its kind in the conversation, and it is still where you are.',
  ...
  'conv.find.scanCost': 'There is no index behind this box, so every turn of words in this transcript is read one at a time. This one took {ms} ms over {scanned} of them.',
  ...
  'conv.find.reSlow': 'The search stopped after {ms} ms without finishing, so there are turns further down that were not read. A narrower query will finish.',
  ...
  // ── THE FOUR MODES, THE HELP AND THE OPERATORS — `semantic/11`, and his own
  // words after using what `semantic/9` shipped: *"regex is complex and a regex
  // help with examples should be added … alternatives to regex A - wildcard
  // character pattern matching … B - some logical ops … like AND OR LIKE NEAR"*.
  //
  // THE ONE SENTENCE THESE ALL HAVE TO KEEP TRUE: this box never reaches SQLite.
  // He asked for the operators "especially if supportd by sqlite", and the
  // honest answer is that they are implemented in the SCAN — which is better
  // here, because an operator in the scan composes with Match case and with the
  // folding, and an FTS5 operator would compose with neither.
  'conv.find.mNormal': 'Plain words',
  'conv.find.mWildcard': 'Wildcards — {m:*} and {m:?}',
  'conv.find.mLogical': 'Words joined by {m:AND} {m:OR} {m:NOT} {m:NEAR}',
  'conv.find.wildNote': 'On, {m:*} is any run of characters including none, and {m:?} is exactly one character. Write {m:\\*} for a real star. What you type is looked for anywhere inside a turn, so a {m:*} at the start or the end asks for nothing and colours nothing.',
  'conv.find.wildFold': 'The pieces between the wildcards are read exactly as plain words are, folding and all, so {m:b*t...} still reaches {m:byte…}. This mode is never turned into a regular expression, which is why nothing you type here can be refused for the shape of a pattern you did not write.',
  'conv.find.logNote': 'On, {m:AND}, {m:OR}, {m:NOT} and {m:NEAR} in capitals are operators and every other word is a word to find — {m:and} in small letters is a word. Two words side by side mean {m:AND}, a {m:"} at each end of two words makes them one phrase, and brackets group.',
  'conv.find.logCount': 'A turn matches when the whole expression is true of it, and then every occurrence of every word that made it true is coloured. The two numbers keep the meanings they have in every other mode: turns are turns, times are coloured places — so under {m:AND} both words are coloured, wherever in the turn they stand.',
  'conv.find.logNear': '{m:a NEAR b} means the two stand within {n} characters of each other — characters and not words — and only the pairs that really are near are coloured. Write {m:NEAR/120} for a distance of your own.',
  'conv.find.whyLike': '{m:LIKE} is SQL’s wildcard, and this search never reaches SQL: it reads every turn of this transcript in the browser’s own engine. What {m:%} and {m:_} do in SQL, {m:*} and {m:?} do in the Wildcards mode above — so it is that mode, not a second spelling of it. Nothing was searched for.',
  'conv.find.whyQuote': 'A quotation mark was opened and never closed, so there is no way to tell where the phrase was meant to end. Nothing was searched for.',
  'conv.find.whyParen': 'The brackets do not balance. Nothing was searched for.',
  'conv.find.whyOperand': 'An operator has nothing on one side of it — {m:byte AND} is missing its second word. Nothing was searched for.',
  'conv.find.whyEmpty': 'There is nothing in there to look for.',
  'conv.find.whyNearRange': 'A {m:NEAR} distance is a number of characters from {m:1} to {max}. Nothing was searched for.',
  'conv.find.help': 'Help and examples for this way of typing',
  'conv.find.helpClick': 'Click any example to put it in the box and run it.',
  'conv.find.helpNormal': 'Type the words you are looking for and nothing else — no punctuation means anything special here.',
  'conv.find.helpWild': 'Two characters, and they are the two everyone already expects.',
  'conv.find.helpLog': 'Four operators, written in capitals, over words read the ordinary way.',
  'conv.find.helpRe': 'The browser’s own regular expressions, run over the text exactly as written.',
  'conv.find.helpWildSafe': 'A wildcard can never be refused for its shape. {m:*} and {m:?} are matched by stitching the pieces between them, not by building a regular expression, so the pattern the regular-expression mode refuses cannot be written in this one.',
  'conv.find.helpReRefuse': 'One shape is refused before it is run: a repeated group that itself contains a repeat, {m:(X+)+}. Typed against this repository’s own 139 MB session, {m:^(\\w+\\s?)+$} took 108,785 ms and had to be abandoned — and no time budget can end that, because the freeze happens inside a single turn, inside the browser’s pattern engine, which cannot be interrupted from the page. A canary that ran the pattern over a short string first was written and then measured: that same pattern is 2 ms over 4 words, 3,848 ms over 8, and does not return over 12 — so a canary long enough to tell a bad pattern from a good one is long enough to hang on the bad one. The refusal is therefore static, and it is narrow: repeating one of the two is enough, and {m:(\\w+){1,3}} and {m:(foo|bar)+} both run.',
  'conv.find.egN1': 'Three dots find the {m:…} this product writes and a keyboard cannot type — 1,024 spans against the 463 a plain substring reaches.',
  'conv.find.egN2': 'A phrase, in the words you would say it in.',
  'conv.find.egN3': 'Hebrew: this also finds the glued {m:השורה}, until Whole word is ticked.',
  'conv.find.egW1': 'Every TypeScript file named anywhere in this session.',
  'conv.find.egW2': 'The lanes numbered with a single digit — {m:?} is exactly one character.',
  'conv.find.egW3': 'Every report this project wrote today.',
  'conv.find.egW4': 'The pieces fold, so a wildcard still reaches {m:byte…} through three dots.',
  'conv.find.egL1': 'Turns that hold both words. Both are coloured, wherever in the turn they stand.',
  'conv.find.egL2': 'Either phrase. Quotation marks make two words into one phrase.',
  'conv.find.egL3': 'Turns about the one and not the other. A word under {m:NOT} is not there, so it colours nothing.',
  'conv.find.egL4': 'The two standing within {n} characters of each other — and only the pairs that really are near are coloured.',
  'conv.find.egR1': 'Every timing this session printed.',
  ...
  'conv.doc.matched': '{shown} of {total} sections match by their opening or by the tools they ran. The whole-text search has not answered yet.',
  'conv.doc.matchedFull': '{shown} of {total} sections are shown. {turns} turn(s) of words hold what you typed, {matches} time(s) — searched in the {scanned} turns of words this transcript has, out of {records} records. Tool output and machinery are not indexed and can only be found here by the tools they ran.',
