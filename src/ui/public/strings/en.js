...
export const strings = {
  ...
  'sim.snap': 'Every value between two rungs behaves identically, so the slider snaps to rungs — dragging lands on meaning rather than on {offrung}. A red rung is an eviction: more budget, fewer items.',
  'sim.tier': 'Tier',
  'sim.budget': 'Budget',
  'sim.fits': 'Fits',
  'sim.spills': 'Spills',
  'sim.chipn': 'The fits column is a ratio, not a count: "{fits} of {eligible}" says how much of what was eligible actually arrived, and the chip flips at the boundary. The row for the tier being dragged follows the slider.',
  ...
  'doc.d5': 'another plugin writes durable learnings here — the same kind as {m:lesson}, in a second spelling with no shared ids. It is a watched path, so edits nudge and a human decides.',
  // Decay
  'dec.h': 'Decay',
  ...
  'cfg.watchednote': '{m:init} writes what this repository actually has, rather than shipping three paths from one workflow that match nothing here. The list replaces and never merges — a list you wrote must not silently gain globs you did not.',
  ...
  'pr.md': 'Steps are a {m:## Steps} section in the Markdown, parsed the way {m:## Observations} already is. "{done} of {steps}" is counted, never stored — there is no second place a procedure could disagree with itself.',
  ...
  'port.sub': 'Built, and this screen reports it. It used to list five open questions; all five are answered now, so it lists the answers instead.',
  'port.what': 'What travels',
  'port.yes': 'travels',
  'port.filtered': 'filtered',
  'port.no': 'rebuilt',
  'port.hist': 'History travels, and it is filtered. Mutations carry; injections, hook actions, focus records, refused requests and procedure ticks do not — they describe a machine, not a corpus. Imported records land in {m:.audit/imported/} so a receiver can always tell what it witnessed from what it was told.',
  ...
  'pk.active': 'draft',
  'pk.draft': 'draft',
  'pk.trustn': 'Both routes land the same way, and it is draft — there is one importer behind them, and it writes every item a pack brings in as a proposal. Choosing a pack at init looks like the act of trust, but an empty corpus is exactly where an unreviewed opinion is hardest to notice, so the import asks rather than assumes. There is no {m:--trust} flag; a boundary a flag can override is not a boundary.',
