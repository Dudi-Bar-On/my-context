 ...
 * (`pre-tool-use.ts` · ``changes to `.my_context/config.json` are the user`` · ~120):
 ...
 * null-prototype (`core/config.ts` · `  const categories: Record<string, ResolvedCategory> = Object.create(null);` · ~1513)
 ...
 * `skippedKeys` (`core/config.ts` · `  skippedKeys: string[];` · ~575) says in its
 ...
 * them (`core/config.ts` · `export const TOP_LEVEL_KEYS = [` · ~1025).
 ...
 * (`core/config.ts` · ` * Declaration order is the order the refusal lists them in — \`enumError\`` · ~152):
 ...
 * (`core/config.ts` · `function requireBudgets(raw: unknown): Budgets {` · ~1377) and
 ...
 * `/api/select` uses (`read-model.ts` · `export function parseSelectQuery(` · ~307):
...
export function apiConfigPreview(ws: Workspace, url: URL, body: unknown): JsonResult {
  ...
  return withStores(ws, (store) => {
    ...
    //    (`core/config.ts` · `export function agentEditsFor(config: Config, type: string): AgentEdits {` · ~229
    //    and `core/config.ts` · `export function scopePolicyFor(config: Config, type: string): ScopePolicy {` · ~207).
