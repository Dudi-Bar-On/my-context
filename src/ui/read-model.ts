...
// **The primitives moved DOWN, and this line is why nothing above had to move
// with them.** `badRequest`, `unknownParams`, `repeatedParams`, `withStores`,
// `COVERAGE_FILE_LIMIT` and `coverageFiles` now live in `read-model-base.ts`;
// see its header for the measurement (ten sibling modules already imported
// them from here). They are imported for this file's own use and re-exported
// below so every one of those ten, and every test, keeps the import line it
// has — the move is a fact about where the source sits, not about what any
// caller may reach for.
import {
  badRequest, coverageFiles, repeatedParams, unknownParams, withStores, COVERAGE_FILE_LIMIT,
} from './read-model-base.ts';

export {
  badRequest, coverageFiles, repeatedParams, unknownParams, withStores, COVERAGE_FILE_LIMIT,
};

// **Three handler groups moved OUT, re-exported so no caller had to move with
// them** (`TASK-the-largest-file-in-the-repository-is-twenty-independent`).
//
// Each was a contiguous run this file had already fenced off with its own
// section banner, each is covered by test files that name its handlers and no
// others, and each was measured before the move to reference nothing declared
// elsewhere here and to be referenced by nothing elsewhere here. The
// re-exports are one-way: those modules import from `read-model-base.ts`, never
// from this file, so there is no cycle — which is the whole reason the
// primitives went down first.
export {
  apiTutorials, apiTutorialDoc, TUTORIAL_DOC_MAX_CHARS, TUTORIAL_REQUIRED_HEADINGS,
  type TutorialCellState, type TutorialDocBody, type TutorialListRow, type TutorialsBody,
} from './read-model-tutorials.ts';
export {
  apiDoc, apiDocList, buildDocManifest, docHeadings,
  type DocBody, type DocHeading, type DocListBody, type DocManifestEntry,
} from './read-model-documents.ts';
export {
  apiCorpusFile, apiCorpusList, CORPUS_FILE_LIMIT,
  type CorpusFileBody, type CorpusListBody,
} from './read-model-corpus.ts';
