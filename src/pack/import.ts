 ...
 * (`core/mutate.ts` · `function resolveCategory(ctx: MutationContext, type: string): ResolvedCategory {` · ~130),
 ...
 * duplicate (`core/mutate.ts` · `      if (itemContentHash(existing) === hash) return duplicateOf(existing);` · ~377),
...
export interface ImportAgainst {
   ...
   * (`core/store.ts` · `  get(id: string): Item | null {` · ~483). A predicate
 ...
 * (`core/validate.ts` · `export function normalizeSteps(steps: string[]): Step[] {` · ~335)
 * sets `checked: false` on every entry, and `checked` is part of the content
 * hash (`core/content-hash.ts` · `function canonicalStep(s: Step): Step {` · ~41),
...
export function applyImport(
  ...
  for (const item of plan.buckets.new) {
    ...
    // already captured (`core/mutate.ts` · `      message: \`my_context: already captured as ${anchored.id}. Nothing changed.\`,` · ~305).
