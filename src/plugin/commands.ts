...
`,
    },
    {
      file: 'handover.md',
      content: `${frontmatter(
        'Ask for the handover note NOW, before compacting or starting a new session',
        '[--anyway]',
      )}
...
Run: \`${CLI} handover ask $ARGUMENTS\`
...
`,
    },
    {
      file: 'ui.md',
      content: `${frontmatter(
        "Open the read-only web UI over this project's my_context corpus",
        '[--port N] [--no-open]',
      )}
...
${previewThenHandBack(
  `${CLI} inbox-promote <id> --to <category> --summary "<one plain sentence>"`,
  `
...
2. If the USER learned it, print this command for the user to run, filled in, and stop:
   ...
   \`${CLI} lesson "<the lesson in one sentence>" --summary "<one plain sentence>"\`
...
3. If YOU learned it, record it yourself. There are two honest routes, and this file used
   ...
   - If the MCP server is not available, the same claim from a shell:
     ...
     \`${CLI} lesson --agent "<the lesson in one sentence>" --summary "<one plain sentence>"\`
...
`,
    },
    {
      file: 'procedure.md',
      content: `${frontmatter(
        'Walk a one-time procedure: what is ready, what is running, what is finished',
        '[list|show|activate|done|step] [the procedure id] [the step number]',
      )}
...
1. If no id was given, run \`${CLI} procedure list\`. It groups every procedure by stage —
   ...
   **Three of the five subcommands are yours to run:** \`procedure list\`,
   \`${CLI} procedure show <id>\`, and \`${CLI} procedure step <id> <n>\` (with \`--undo\`
...
**\`${CLI} procedure done <id>\` is the user's in exactly the same way**, and for the same
