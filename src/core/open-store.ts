...
import { Store, type OpenProfile } from './store.ts';
...
export interface OpenStoreOptions {
  ...
  retryOnBusy?: boolean;
  /**
   * The `Store.open` contention profile. The hook path passes
   * `HOOK_OPEN_PROFILE` (ROADMAP E4): `hooks.json` kills the hook at 10s, so
   * the default policy's ~15–23s contended worst case is not patience, it is
   * a killed process and a silently missing injection. Every other caller
   * omits it and gets `DEFAULT_OPEN_PROFILE` — see `OpenProfile` (store.ts).
   */
  profile?: OpenProfile;
}
...
export function openRebuiltStore(ws: Workspace, options: OpenStoreOptions = {}): OpenedStore {
  const store = Store.open(ws.dbPath, options.profile);
  try {
