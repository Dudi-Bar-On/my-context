...
import { appendJsonlLine, ensureLogDir } from './jsonl-log.ts';
...
// The read half of this log — the protocol string, the on-disk line shape, the
// two path helpers, `readLog`, `foldLog` and the count spelling — lives in
// `core/revision-log.ts` (web-ui plan 1, Task 6). It was moved, not copied: a
// second parser of this file is exactly the drift the module comment above
// refuses. It moved because this module imports `updateItem` from mutate.ts at
// runtime, so a read-only surface that merely wanted to COUNT the queue could
