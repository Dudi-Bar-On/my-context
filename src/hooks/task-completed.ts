...
import { TASK_COMPLETED } from './task-events.ts';

/**
 * The `TaskCompleted` binary. The spec — and the whole argument for what these
 * two events do and deliberately do not do — is in `hooks/task-events.ts`,
 * which `TaskCreated` shares.
 *
 ...
 */
if (isMainEntry(import.meta.filename, process.argv[1])) runObservationHook(TASK_COMPLETED);
