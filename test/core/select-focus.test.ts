...
function item(over: Partial<Item> = {}): Item {
  return {
    ...
    severity: 'soft', always: true, continuity: false, summary: null, summaryOf: null, summaryWas: [], acknowledged: {}, scope: [], tags: [], origin: 'human',
...
function ordinary(over: Partial<Item> = {}): Item {
  return item({ always: false, continuity: false, summary: null, summaryOf: null, ...over });
}
...
test('an always:true item is never hidden by focus, whatever its severity', () => {
  const pinnedSoft = item({ id: 'INSTR-pinned', severity: 'soft', always: true, continuity: false, summary: null, summaryOf: null, tags: ['auth'] });
  const ordinary = item({ id: 'RULE-auth', severity: 'soft', always: false, continuity: false, summary: null, summaryOf: null, tags: ['auth'] });
...
test('the two exemptions are reported apart, and an item that is both is counted once', () => {
  ...
  const both = item({ id: 'INV-both', type: 'invariant', severity: 'hard', always: true, continuity: false, summary: null, summaryOf: null, tags: ['auth'] });
...
test('the report counts the load-bearing relation focus left dangling', () => {
  const openq = item({
    id: 'OPENQ-x', type: 'open_question', tags: ['design'], always: false, continuity: false, summary: null, summaryOf: null,
...
test('a referential relation into hidden knowledge is not reported as a cost', () => {
  const lesson = item({ id: 'LESSON-a', type: 'lesson', tags: ['auth'], always: false });
  const rule = item({
    id: 'RULE-b', tags: ['billing'], always: false, continuity: false, summary: null, summaryOf: null,
