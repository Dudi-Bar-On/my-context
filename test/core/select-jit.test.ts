...
test('an unscoped pinned item is JIT-eligible, and the seen filter is what dedupes it', () => {
  const items = [item({ id: 'CONST-pinned', always: true, continuity: false, summary: null, summaryOf: null, scope: [] })];
...
test('a scoped item is still restricted to its globs — the inversion did not make scope inert', () => {
  const sel = select(
    [item({ id: 'CONST-db', always: true, continuity: false, summary: null, summaryOf: null, scope: ['src/db/**'] })],
