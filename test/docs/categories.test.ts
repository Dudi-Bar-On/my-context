 ...
 * generated: both documents carry an `<!-- example-md: help categories -->` block
...
const DOCUMENTS = [
  { file: 'README.md', heading: '### The two profiles, and the one that was removed' },
  { file: path.join('docs', 'README.he.md'), heading: '### שני הפרופילים, והאחד שהוסר' },
] as const;
...
test('both documents get their definitions from the generated help block', () => {
  for (const { file } of DOCUMENTS) {
    assert.match(
      read(file), /<!-- example-md: help categories -->/,
...
/**
 * The catalogue table must actually render as a table.
 *
 * It did not. The block was inside a ```` ```text ```` fence, so GitHub showed
 * its twenty rows as literal `|` pipes and its headings as literal hashes —
 * while a hand-written three-row table three inches below it rendered
 * properly, which made the categories the *worse*-presented half of the
 * section the owner asked to be able to see. That is C8.
 *
 * Fixing it could not mean hand-writing the table: the block's whole value is
 * that it is `mycontext help categories`' real output, regenerated by
 * `npm run gen:docs` and diffed by `test/docs/examples.test.ts`. So the block
 * kept its marker and lost its fence — the `example-md` form, whose body is
 * the same output with the one transformation `toDocumentMarkdown` names.
 *
 * Three things are asserted, in both languages, and each is a different way
 * the fix could be undone. The block is a `markdown`-kind example, so it is
 * still generated and still diffed. Its body is not inside any fenced region
 * of the document, which is what makes the pipes render. And a real table row
 * is present in it — a `markdown` block emptied of its table would satisfy the
 * first two.
 */
test('the catalogue table is rendered Markdown, and still generated output', () => {
  for (const { file } of DOCUMENTS) {
    const text = read(file);
    const block = collectExamples(text).find((e) => e.command === 'help categories');
    assert.ok(block, `${file} no longer carries a \`help categories\` example block at all`);
    assert.equal(
      block.kind, 'markdown',
      `${file} puts \`help categories\` back inside a \`\`\`text fence, where GitHub renders ` +
      `its twenty-row table as literal | pipes. That is the defect C8 named. The block must ` +
      `stay an <!-- example-md --> block: still generated, still diffed, and rendered.`,
    );
    assert.equal(block.fence, '', 'a markdown-form block has no fence by construction');

    // Not inside any fenced region of the document either — the marker form is
    // necessary but a fence wrapped around the whole thing would defeat it.
    const fenced = fenceTracker();
    const insideAFence = text.slice(0, block.start).split('\n').map((l) => fenced(l)).at(-1);
    assert.equal(insideAFence, false, `${file}'s categories block sits inside a fenced region`);

    assert.match(
      block.body, /^\| `constraint` \| normative \| `CONST-` \|/m,
      `${file}'s categories block no longer carries the catalogue table row for \`constraint\`. ` +
      `An empty rendered block passes every other assertion here.`,
    );
    assert.doesNotMatch(
      block.body, /^ {0,3}#{1,6} /m,
      `${file}'s categories block carries an ATX heading. Written unfenced those become ` +
      `sections of the README, which parity.test.ts and capabilities.test.ts both key on.`,
    );
  }
});
...
test('the category comparisons are not collapsed behind a <details>', () => {
  for (const { file } of DOCUMENTS) {
    const text = read(file);
    const at = text.indexOf('<!-- example-md: help categories -->');
