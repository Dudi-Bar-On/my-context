...
const FLAGS_NOT_OFFERED: Record<string, Record<string, string>> = {
  ...
  'lesson-accept': {
    ...
    //
    // `summary` ITSELF LEFT THIS LEDGER 2026-09-22, exactly the way the
    // sentence that used to stand here said it would: `lib/palette-defs.js`
    // now offers it (`{ name: 'summary', input: 'text' }`, added the same
    // day), so the `excused && offered` branch below would fail if this row
    // still named it. A `lesson-accept` line composed by the palette used to
    // carry no summary and be refused by the gate on every run — measured
    // directly, and the regression test for it is
    // `e2e/composer-write-execute.spec.ts`'s "the six writes that need a
    // queue run" case. `summary-omitted` stays excused below: nothing added
    // a control for it, so a candidate accepted with no summary at all is
    // still a composed command this screen cannot produce.
 ...
 *
 * **THE COUNTS ARE NOT WRITTEN DOWN HERE, AND THE REASON IS THIS PARAGRAPH'S
 * OWN HISTORY.** It used to open "the registry dispatches 39 COMMANDS … one of
 * 53 command STRINGS", and it went on to name `builder/3`'s title as stale for
 * saying 38. Both of its own figures were stale within a fortnight — the
 * registry dispatches 44 and expands to 58 as this is written — so a paragraph
 * whose whole subject is hand-kept numbers drifting was itself the fifth
 * hand-kept number in this file's history. It is not replaced with corrected
 * figures, because a corrected figure is the same defect with a later date on
 * it.
 *
 * Two questions, and neither is answered by a number typed here:
 *
 *   - **How many commands exist** — `docs/cli-ui-coverage.md`, generated by
 *     `scripts/gen-cli-ui-coverage.ts` from the registry and regenerated by
 *     `test/docs/doc-system.test.ts`, which fails on any difference. It moves
 *     the day the CLI does. That document is the DENOMINATOR, per owner
 *     decision D4 (2026-09-06), and `the generated coverage document and this
 *     partition name the same commands` below is what makes it one rather than
 *     leaving two derivations to agree by luck.
 *   - **How many command STRINGS** — `commandStrings()`, derived on every run
 *     from the registry and the five commands' own `SUBCOMMANDS` exports. A
 *     catalogue entry and a permission rule are each written against a STRING,
 *     which is why the partition below is over strings and not over commands.
 *
 * A third number — how many entries `PALETTE` holds — answers neither, and
 * mixing it in is how "the catalogue covers everything" goes unchallenged: one
 * command can be five entries (`review`), so an entry count overstates and
 * understates at once. Nothing below counts entries.
 */
...
/**
 * The generated document's own command list, read the way a reader reads it.
 *
 * Parsed out of the committed Markdown rather than obtained by calling
 * `liveCommands()`, and the difference is the whole value of the test below.
 * Calling the generator would compare `[...COMMANDS.values()]` with
 * `[...COMMANDS.keys()]` — one registry against itself, which cannot fail for
 * any reason worth catching. What a reader of `builder/3` is sent to is the
 * FILE, so the file is what is read here; `test/docs/doc-system.test.ts`
 * separately regenerates it and fails on any difference, so this parse is of
 * something already held current, and the two tests together close the loop
 * without either of them being the generator.
 */
function documentedCommands(): string[] {
  const doc = readFileSync(path.resolve(import.meta.dirname, '../../docs/cli-ui-coverage.md'), 'utf8');
  const table = doc.slice(doc.indexOf('## Every CLI command'));
  return [...table.matchAll(/^\| `mycontext ([a-z][\w-]*)` \|/gm)].map((m) => m[1]).sort();
}

/**
 * **The denominator, and the one thing the partition above could not check
 * about itself** (owner decision D4, 2026-09-06).
 *
 * The test above partitions `commandStrings()` between the catalogue and two
 * tables of named gaps, in both directions, and it is right to do so — a
 * catalogue entry is written against a command STRING. But it derives its own
 * denominator, and a partition that supplies its own denominator answers only
 * "is this list internally consistent". `builder/3` asks a different question:
 * does the catalogue cover every command THE GENERATED COVERAGE DOCUMENT
 * COUNTS. That document is the figure this project publishes — the one a reader
 * is sent to, the one `docs/cli-ui-coverage.md` puts in its first sentence —
 * and until this test there was nothing at all connecting it to the catalogue.
 *
 * Two independent derivations of "every command this tool has" therefore ran
 * side by side and could not disagree out loud: `gen-cli-ui-coverage.ts` walks
 * `COMMANDS.values()` after `cli/index.ts` has registered everything, and
 * `commandStrings()` walks `COMMANDS.keys()` and expands the five subcommanded
 * ones. They agree today. Nothing required them to, and the failure would have
 * been the quiet kind this plan exists to end: the published denominator moves,
 * the catalogue's gate keeps passing against its own, and "the catalogue covers
 * everything" stays true of a number nobody publishes.
 *
 * **BOTH WAYS, for the same reason the partition is.** A command in the
 * document that no string in the partition begins with is a command the
 * catalogue never had to account for. A first word in the partition that the
 * document does not list is the opposite fault — the catalogue accounting for
 * something the published surface says does not exist.
 */
test('the generated coverage document and this partition name the same commands', async () => {
  const { PALETTE } = await defs();
  const documented = documentedCommands();

  // Anti-vacuity twice over: a parse that matched nothing would make every
  // comparison below trivially true, and a parse that matched the wrong rows
  // would disagree with the count the document states about ITSELF.
  assert.ok(
    documented.length > 40,
    `parsed ${documented.length} commands out of docs/cli-ui-coverage.md. The table's row ` +
    'shape has changed and this test is now comparing against almost nothing.',
  );
  const doc = readFileSync(path.resolve(import.meta.dirname, '../../docs/cli-ui-coverage.md'), 'utf8');
  const headline = /my_context has (\d+) CLI commands/.exec(doc);
  assert.ok(headline, 'the coverage document no longer states how many CLI commands there are');
  assert.equal(
    documented.length, Number(headline[1]),
    'the coverage document\'s first sentence and its own table disagree about how many ' +
    'commands there are. Both are generated from one list, so this is a parse fault here — ' +
    'fix the row pattern rather than the document.',
  );

  // The partition, reduced to the commands it is written against. A string is
  // `verb` or `verb subcommand`; the document has one row per verb.
  const partition = [...PALETTE.map((def) => commandString(def)), ...Object.keys(WITHHELD),
    ...Object.keys(UNCATALOGUED)];
  const verbs = [...new Set(partition.map((s) => s.split(' ')[0]))].sort();

  assert.deepEqual(
    verbs, documented,
    'the catalogue partition and the generated coverage document do not name the same ' +
    'commands. The document is the DENOMINATOR (owner decision D4) — it is regenerated from ' +
    'the registry and is what a reader is sent to for the count — so a command it lists that ' +
    'nothing above catalogues or names is a gap in the catalogue, and a command named above ' +
    'that it does not list is the catalogue accounting for something the published surface ' +
    'does not have.',
  );
});
