...
const BORN_HERE: Record<string, string> = {
  ready: 'cli/commands/ready.ts — written after the lift, so its spec has never lived '
    + 'anywhere else.',
  path: 'cli/commands/path.ts — the command itself is newer than this module (rulings/93, '
    + '2026-09-16: the board under-reported what had shipped and there was no per-subject '
    + 'reading at all), so its spec was authored here and has never lived anywhere else.',
