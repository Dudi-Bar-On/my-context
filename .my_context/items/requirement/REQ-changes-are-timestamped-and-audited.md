...
severity: hard
always: false
summary: Every change and every delivery is written to a running log, so you can ask afterwards what happened and when, without depending on version control.
summary_of: eb45226864aabed5
scope:
  - src/core/item.ts
  - src/core/rebuild.ts
  - src/core/mutate.ts
