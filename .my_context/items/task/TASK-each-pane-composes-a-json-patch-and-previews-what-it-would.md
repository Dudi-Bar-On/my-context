...
AND THE STALE COMMENT IS WHY NOBODY NOTICED. config.js:24-40 still tells every reader: "the screen contract s fetcher takes a path and nothing else -- no method, no body ... an app.js extension that plan-2 Task 12 adds and that app.js does not have today". app.js HAS it. So the screen s own header instructs the next implementer that the thing they need does not exist, and every candidate-config preview below is ABSENT rather than approximated on that basis. THE COMMENT MUST BE CORRECTED IN THE SAME COMMIT as the first POST call, or the next reader stops for the same reason.
