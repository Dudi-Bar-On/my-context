...
FOUND 2026-08-25 by owner request 2026-08-25: preview, app vs mockup, on the REAL corpus. THE MOST VISIBLE DIFFERENCE ON THE SCREEN, and it has no task anywhere.
...
THE CAUSE, and it is honest rather than careless. `screens/preview.js:181-224` says so in its own header: "Not a markdown renderer". `bodyNodes()` handles exactly TWO shapes -- prose paragraphs, and `-`/`*` bullets -- because those are "the ONE structural shape the design of record draws inside a block". It was built to the mockup and the mockup s sample bodies use bullets.

THE FIXTURE HID IT, FOR THE SIXTH TIME. `.demo-corpus` bodies use bullets, which `bodyNodes` renders correctly. The REAL corpus is written by people who use blockquotes, and this project s own items are full of them. Every measurement ever taken of this screen was taken against the fixture.
...
CORRECTED 2026-08-25, BEFORE ANY CODE WAS WRITTEN. THIS TASK'S PREMISE WAS WRONG and the correction matters more than the finding.
...
PARTLY DONE 2026-08-25, code 18d4477 — and the part that was done is NOT the
part this task described.

The owner looked at the delivered pane and said the text was "not formated as
in the mockup". He was right, and the cause was a THIRD thing, neither the app
being wrong about blockquotes nor the mockup's sample being short:
...
That was a real app gap against the design of record, and it is closed:
`bodyNodes()` now delegates to `markdownNodes`, the mockup's own renderer,
which already emits exactly the four shapes `.blkbody` authors by hand.

WHAT REMAINS IS WHAT THE CORRECTION ABOVE SAYS: blockquotes. Both renderers are
now the same renderer, and it has no blockquote branch — because the mockup's
has none. Rendering `>` needs a mockup change first, drafted under
DEC-claude-drafts-the-mockup-and-the-owner-approves. Still worth doing; still
the owner's design decision and not a defect.
