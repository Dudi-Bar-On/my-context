...
that promise is the first thing the next session owes him.

**TWO LANES ARE RUNNING RIGHT NOW.** Their work will be uncommitted in the tree.
**Do not `git add -A` and do not `git add <a directory>`** — I swept a lane's
constraint edit into an unrelated commit tonight doing exactly that, and the
rule I wrote after the last three times says stage explicit PATHS.

1. `library/6` (D27) — the help tested for truth. Its bar is NOT D12's: a test
   passes when a claim on the screen equals the derivation it came from, not
   when the card renders.
2. `builder/16` + `builder/10` — the id filtering box, and the `key` field.

**THINGS RULED AND NOT DISPATCHED**

1. `budget/16` — BUILT, MEASURED, DELIBERATELY NOT COMMITTED. The patch is at
   `scratchpad/budget-16-spare-band.patch` (441 lines). It reds 49 fixtures,
   none a logic failure, and one can only be closed by rewriting README.md,
   which is `docsys/11` territory. The question with every number is
   `OPENQ-does-the-pinned-tier-spend-its-spare-room-on-governing-items`.
   A narrower form cuts the damage to 23 with an identical result here.
2. `budget/15` — a costed proposal, not an implementation. Read its report in
   the item before building anything.
3. `archive/6` — the ONE knowingly-red thing. `tutorial-manifest` is 5/7
   because `conversation.ts` and `conversations.js` are claimed by no tutorial.
   Not closed by editing the manifest on purpose: that roster is a coverage
   requirement, and a claim without a section is a false claim.

**NEXT SESSION, FIRST THREE THINGS**

1. Wake him with the D table, rebuilt from the corpus — never copied forward.
   `RULE-progress-is-reported-as-two-tables-with-stable-d-numbers` has the
   shape, the five chips and the D numbers, which are stable forever.
2. Verify the two running lanes yourself before believing any "done", then
   commit with explicit paths. `docs/cli-ui-coverage.md` is regenerated by
   several lanes — whoever finishes last must re-run `gen:cli-ui-coverage`.
3. Then `builder/5` (releases `/6` and `/8`), then `builder/11` (D12), then
   `docsys/11` (D28).

**A GAP IN MY OWN VERIFICATION, AND HE SHOULD BE TOLD PLAINLY.** The Playwright
MCP disconnected partway through the night. For every item after that, the
visual half rests on the LANE'S screenshots, not mine. Worth his eye on sight:
the Composer without its Run button, the help card's worked lines, and the new
conversation screen.

**WHAT LANDED TONIGHT**, all pushed, 0 unpushed: D6 (citation gate green), D16
(dependency budget parsed from the constraint and gated), D17 (the handover
says when it is read from a window that ended), D8 (Hebrew tutorials converted,
0 marks left), D22 (Run removed, the split now impossible by an inverted test),
D11's lesson split and `/api/staging`, D24/D25/D26 (the help skeleton),
`library/7` and `/8`, `handover/11` (the mute), `governance/4` and `/5`, and
`archive/1`–`/3` (the conversation archive). Fifteen items closed verified.

**FOUR CORRECTIONS OF MINE ARE ON THE RECORD, and the pattern matters more than
any one of them:** I told a lane SessionStart knew a percentage it does not —
following it would have made the warning LIE at the moment it mattered. I said
a manifest held a checksum it does not. I said a framing was stale when it was
the only accurate line on the board. And I said `resultRows` was shared when it
had one caller. Every brief told the lane my leads might be wrong; every one of
those lanes measured and corrected me. Keep writing that sentence into briefs.

**A NEW RULE AT SEVERITY HARD**,
`RULE-a-delegated-worker-never-runs-a-command-that-reaches-beyond`: a lane ran
`taskkill /F /IM node.exe` to quiet the machine, and reported it itself. Damage
was mild and verified. The range was not.

**STILL ON HIM:** the 42 `walk` items, which he is taking as part of the v2.0
review. Everything else he ruled last night. Three smaller things wait on his
eye and block nothing: `sanitize.js`'s header says 149/1,665 where the
measurement is 150/1,704; `check:dependencies` is not in `ci.yml`; and the
closed v1.0.0 campaign plan's citations now chase a moving tree, reversible in
four lines.
