...
| <span dir="ltr">`--held`</span> | להציג את המשימות שהדוח אחרת רק **סופר**: אלה שה-`needs` שלהן אינם כולם גמורים. אין <span dir="ltr">`--all`</span> לצידו — "מוכן" הוא תשובה נגזרת על עבודה פתוחה, ודגל שהיה מרחיב אותה לעבודה גמורה היה שואל שאלה אחרת תחת אותו שם <!-- `core/command-flags.ts` · `ready: { allowed: ['plan', 'limit', 'held', ...DETAIL_FLAGS], values: ['plan', 'limit'] },` · ~330 --> | `ready` |
