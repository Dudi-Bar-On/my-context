...
**שום מסלול <span dir="ltr">`/api`</span> אינו מגיע לפונקציה שמשנה, והמנגנון הוא הסיבה שאפשר
להאמין לזה ולא רק להבטיח אותו.** בדיקה סטטית של גרף הייבוא
אוכפת את זה: היא טוענת על **הקבוצה המדויקת** של סמלי הכתיבה הכבולים בכל מקום תחת
<span dir="ltr">`src/ui/`</span> — לא שהקבוצה ריקה, אלא שהיא בדיוק השניים שהבעלים אישר בשמם,
כך שכתיבה חדשה מפילה את הסוויטה באותה עוצמה שבה מפילה אותה כתיבה שנמחקה. כל כתיבה שהממשק
מציג לך **מולחנת להדבקה ב-shell שלך**, ולעולם אינה מורצת: כלומר כללי ההרשאות של Bash שלך
ממשיכים להתאים למחרוזות פקודה, בדיוק כפי שהם עושים כשאתה מקליד את הפקודה בעצמך.

יש כתיבה אחת, והיא על מסלול הסירוב. **בקשה שהשער מסרב לה מוסיפה רשומת
...
שנשלחו. קריאה שהוגשה אינה משנה ולו בית אחד בקורפוס; בקשה שסורבה נרשמת, מפני ששווה לדעת על
בקשה שנדחתה לשרת מקומי. לא האסימון ולא שום חלק ממנו נמצא ברשומה הזאת.

**מודל האסימון, בשני משפטים.** כל הרצה מטביעה אסימון חדש, שהדף שולח בכותרת ייעודית בכל בקשה
ושאינו מגיע לא לדיסק ולא לשורת פקודה של תהליך; מה שמופיע בכתובת הוא **nonce חד-פעמי במקטע**,
שנסחר פעם אחת ב-<span dir="ltr">`POST /api/handoff`</span> תמורת האסימון עצמו ומת אחר כך.
בקשה בלי הכותרת מקבלת <span dir="ltr">`401`</span> לפני שרץ מטפל כלשהו של מסלול.
...
- **קריאה** — התיעוד, המדריך, ומסך Learn המקושר לקורפוס הזה.
...
משכתבת את מה ש**נמסר** מהקורפוס — מיקוד שבתוקף מסתיר כל פריט כשיר שאינו תואם לו, בכל פגישה
...
מעטפת, כך ששום טקסט שמישהו מקליד אינו הופך לפקודה. האישור טובע **אסימון חד־פעמי הקשור בדיוק
לפקודה שהוצגה**, כך שדף שמעולם לא הראה לכם אישור אינו יכול להריץ דבר. וכל הרצה נכתבת ליומן
...
> הוא חד-פעמי וחי שלושים שניות; החשיפה שהשארית הזאת נוקבת בה היא **היכולת הקבועה לבקש**
