...
ולא פער בשער הזה; שום קורא משום מקור אינו יכול להזיז אותן, ולכן אין מדיניות שאפשר לעקוף.
...
במשך סבב אחד ארבעה ערכים שתיארו יכולות שהפרויקט כבר בנה — ההסגר התהפך, והפרק שנכתב כדי
...
פעולה רשאית להתבצע.** הדבר היחיד שהתוסף הזה אי פעם מונע הוא כתיבה לתוך
...
"מועמד לאכיפה עתידית" שבטבלת הדגלים הוא הקריאה המדויקת, והערך הזה הוא המקום שבו הפער
...
<span dir="ltr">`always: false`</span> ועם scope ריק, ול-`add` אין דגל שמשנה אחד מהם.
...
`mycontext_help` הוא הכלי שסוכן פונה אליו, ושני הנושאים שסשן זקוק להם בתדירות הגבוהה ביותר
בעיצומה של משימה — איך לשאול את הקורפוס, ומה מפתח תצורה עושה — הם השניים שהוא אינו יכול
לענות עליהם.
...
המצוטט בפרקים 3, 4 ו-6 הוא מה שה-hooks פולטים; שסיכום היכולות שבראש המסמך מקשר לכל פרק
שתוכן העניינים מקשר אליו; וששני המסמכים נושאים את אותו רצף כותרות ואת אותן דוגמאות באותו
...
<span dir="ltr">`parity.test.ts`</span> מחזיק את רצף הכותרות של הפרק הזה מול המראה העברית.
הפסקה הזאת אמרה "שום בדיקה אינה בודקת את הפרק הזה" עד שהוא נבדק.

להיבדק אינו להיות מאומת, וכדאי לנקוב במגבלות אחת-אחת. בדיקת המקבילות משווה מבנה ולעולם לא
משמעות: עברית שנשארה מאחור אחרי עריכה אנגלית עוברת כל טענה בחבילה, והקובץ ההוא מדגים את
