...
| **המשכיות** | בתחילת כל סשן ושוב אחרי כיווץ | כל פריט פעיל שמסומן `continuity: true`, במלואו — הדרג היחיד שאינו שואל אם הפריט נורמטיבי, ולכן גם `reference` יכול לנסוע בו |
...
- **כל פריט מגיע פעם אחת בחלון הקשר.** my_context רושם מה כבר הזריק, כך שעריכה של עשרה
  ...
  הדרג הנעוץ ואת האינדקס ברגע שהוא משוגר, מ-hook משלו — ראו
  [פרק 8](#תת-סוכן-אינו-מקבל-את-הזרקת-תחילת-הסשן).
...
<span dir="ltr">`Agent`</span> שאינו נוקב בפריט משימה שיש לקורפוס הזה. [פרק 2](#2-הרעיון) מתאר ידע נורמטיבי כמה
...
כותרת הסעיף נשארת נכונה — תת-סוכן עדיין אינו מקבל את הזרקת *תחילת הסשן*. מה שהשתנה הוא
שקיים כעת hook שבו my_context יכול לענות, ו**הוא עונה בו**.

**מה תת-סוכן מקבל בלידתו.** <span dir="ltr">`SubagentStart`</span> מספק את הדרג הנעוץ
במלואו ועוד את האינדקס — במכוון אותה בחירה שתחילת סשן עושה, כי ל"מה מוזרק" חייבת להיות
תשובה אחת בדיוק, ועותק שני של ההחלטה הזאת הוא בדיוק הסטייה שהתכנון הזה קיים כדי למנוע.
האינדקס שלו נושא את [הנשיאה בין הסשנים](#האינדקס--כדי-ששום-דבר-לא-יהיה-בלתי-נראה) ככל
אינדקס אחר, עם כלל אחד משלו: תת-סוכן נושא **מהסשן ששיגר אותו**, לפי מזהה. כל אירוע אחר
מבקש את הסשן העדכני ביותר שאינו הוא עצמו; לתת-סוכן אין סשן משלו — הוא יורש את
ה-`session_id` של ההורה מילה במילה — ולכן הכלל ההוא היה מוסר לו פריטים של חלון לא קשור,
תחת תווית שנוקבת בסשן שאיש מהמשתתפים בשיגור לא היה בו.

**הבלוק אומר מאין הגיע, וזה לא קישוט.** ההזרקה של תת-סוכן נפתחת במסגרת שנוקבת בשם התוסף,
אומרת שהבלוק נוסף לפני התור הראשון שלו ואינו חלק מההודעה ששיגרה אותו, ומצביעה על קובצי
ה-Markdown שהוא יכול לקרוא בעצמו. טקסט לא מיוחס שמגיע לחלון הקשר דווח בידי תת-סוכן אמיתי
להורה שלו כמתקפה אפשרית מחוץ לתחום — וזו הקריאה הנכונה של טקסט לא מיוחס שמגיע לחלון הקשר.
ולכן הבלוק של תת-סוכן אינו זהה בייט-בייט לזה של תחילת סשן; הבחירה שמאחוריו כן.

הוא מקבל גם את [דרג הבדיוק-בזמן](#בדיוק-בזמן--אלה-שחלים-על-מה-שאתה-נוגע-בו):
...
הזה קיים כדי לבודד במקומות אחרים. הזרקת הלידה מבצעת דה-דופליקציה תחת אותו מפתח מורכב,
ולכן קריאת הכלי הראשונה של התת-סוכן אינה מספקת שוב את מה שהוא בדיוק קיבל. כשמטען מגיע בלי
`agent_id` אין מפתח לכתוב תחתיו, ו-my_context אינו כותב **שום** רשומת seen במקום לכתוב את
של ההורה: זה עולה מסירה חוזרת אחת לתת-סוכן, בעוד שכתיבה לקובץ ההורה הייתה מדכאת את ההזרקה
הבאה של ההורה עצמו.

**הפער שנותר שינה צורה: הוא כעת פער של שהות והריגה, לא פער של מסירה.**
<span dir="ltr">`SubagentStart`</span> חוסם את השיגור שעליו הוא נורה — נמדד: hook שלקח
3,018 מילישניות עיכב את קריאת הכלי הראשונה של התת-סוכן עד שחזר — ולכן כל שיגור בפרויקט
משלם על זה לפני התור הראשון שלו, ושום דבר בבחירה אינו חסום בתוך התהליך. הדבר היחיד שמסיים
את ה-hook הוא Claude Code שהורג אותו אחרי חמש השניות ש-<span dir="ltr">`hooks/hooks.json`</span>
מכריז עליהן, ותת-סוכן שה-hook שלו נהרג מתחיל בלי שום ידע של הפרויקט הזה. וזה לא שקט: רשומת
הביקורת נכתבת **לפני** העבודה ואומרת <span dir="ltr">`delivery=attempted`</span> עם
ה-`agent_id`, ורשומה שנייה אומרת <span dir="ltr">`delivery=complete`</span> אחריה. שתיהן
נושאות את ה-`session_id` של ההורה, ולכן <span dir="ltr">`mycontext audit --session <parent>`</span>
מציגה את הזוג זה לצד זה, וניסיון בלי השלמה עבור אותו `agent_id` הוא בדיוק תת-סוכן שהתחיל
...
דרך פקודה. **הדרישה הזאת מקוימת היום, ונאכפת בבדיקה ולא בסקירה.** לכל אחד מעשרים ושמונה כלי
...
ל-20 מתוך 49 פקודות שורת הפקודה אין אחת, לכל אחת מסיבה שרשומה לידה
ב-<span dir="ltr">`CLI_WITHOUT_SLASH`</span>:

- <span dir="ltr">`contribution`</span> מדווחת כמה פעמים כל פריט נמסר בפועל, והקורא היחיד
  שאסור שיהיה לה הוא המודל שאת המסירות שלו היא סופרת: קורפוס שעורך את עצמו בתגובה למספרי
  התרומה של עצמו הוא בדיוק הכשל שהמדידה קיימת כדי **לגלות**, לא כדי להזין. היא אינה כותבת דבר
  ואינה קובעת דבר, ולכן פקודת סלאש רק הייתה מציבה טבלה מול מי שהטבלה עוסקת בו.
- <span dir="ltr">`conversation`</span> מסדרת ומציגה את התמלילים ש‑Claude Code כבר כתב
  בדיסק. למודל אין בהן צורך — השיחה שעליה היה שואל היא זו שהוא מנהל — והעיון שלשמו הן
  קיימות הוגדר כתכונת ווב, במסך השיחות. מי שמריץ אותה הוא אדם שממלא את האינדקס לפני
  שהוא פותח את המסך הזה.
- <span dir="ltr">`init`</span> ו-<span dir="ltr">`rebuild`</span> רצות לפני סשן, או מחוצה
  לו, כך שאין סשן שיישא פקודת סלאש.
- <span dir="ltr">`carry`</span> היא אותה הכרעה של אדם, שלב אחד מעל: **אדם** הביט במה
  שנשפך בהזרקה האחרונה והכריע שפריט אחד נדרש *עכשיו*. פקודת סלאש, כמו כלי, הייתה נותנת
  למודל לקבל את ההכרעה הזאת בעצמו — בדיוק מה שהפקודה הזאת קיימת כדי לשמור בידיים של אדם.
- <span dir="ltr">`repair`</span> נמצאת ברשימת הסירוב המומלצת, והתצוגה המקדימה שלה היא עמוד
  ...
  תיתן לי לעשות את זה".
- <span dir="ltr">`config`</span> נמצאת ברשימת הסירוב המומלצת מאותה סיבה: גם
  <span dir="ltr">`--delete`</span> וגם <span dir="ltr">`--disable`</span> נושאות תצוגה
  מקדימה הנוקבת במספר הפריטים האמיתי שהקטגוריה כבר נושאת, ופקודת סלאש עבור מי מהן הייתה
  אותה הנחיה שכל תוכנה הכן היחיד הוא "אל תיתן לי לעשות את זה".
...
- <span dir="ltr">`pack`</span> מייבאת קורפוס של זר אל תוך שלך, מאחורי שני אישורים —
  שהשני שבהם הוא המסלול היחיד שבו משהו שאתה כתבת מוחלף במשהו שמישהו אחר כתב. פקודת סלאש
  שהייתה מריצה אותה הייתה המודל נוטל את המעשה הזה בשמך. זו עבודה עתידית מכוונת ולא דלת
  סגורה: פקודת סלאש שרק *הציגה תצוגה מקדימה* של ייבוא ואז הדפיסה את
  ...
  ממתינה לו.
- <span dir="ltr">`rules`</span> קוראת את הקבועים שנשלחים בתוך החבילה — לא את הקורפוס שלכם —
  ובודקת אותם מול הסיכומים שנשלחו יחד איתם. פקודת סלאש הייתה בעצם המודל ששואל אם הכללים
  שניתנו לו הם האמיתיים, וזו שאלה שאי אפשר לסמוך על תשובתו לה: הקורא הכן של בדיקת שיבוש הוא
  האדם שהתקין את הכלי. אין גם כלי MCP, ומאותה סיבה.
- <span dir="ltr">`restore`</span> מסכמת שיחה קודמת מתוך התמליל שלה עצמה, ו—ברגע שאתם
  מאשרים—מוסרת את הסיכום לסשן הבא שייפתח. האישור הוא אדם שמכריע מה חלון הקשר שלם מקבל מתוך
  תיעוד מילולי של שיחה, ופקודת סלאש הייתה המודל שלוקח את ההכרעה הזאת בעצמו: בדיוק הסיבה של
  <span dir="ltr">`carry`</span>. החצי האוטומטי, <span dir="ltr">`--build`</span>, אינו דורש
  אישור כלל ומחזיק הצעה שאינה מגיעה לשום סשן עד שתפעלו. אין גם כלי MCP, ומאותה סיבה.
...
הסיבה שהיא פקודה אחת לצד ה-29 ולא במקומן. אותו דבר נכון לארבעת הערכים שמשנים כל הזמן:
<span dir="ltr">`/mycontext:pin`, `/mycontext:unpin`, `/mycontext:harden`</span>
...
סדר. מתוכם, <span dir="ltr">`counts.test.ts`</span> מחשב מהתוכנית הרצה את היחס "20 מתוך 49
