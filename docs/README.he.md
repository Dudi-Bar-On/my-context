...
**אף אחד לא הקליד כלום.** לא רצה חיפוש, לא נקרא שום כלי, אף אחד לא הדביק כלל ואף אחד לא
ביקש אחד. **אף אחד לא זכר כלום** — לא אתם, ולא המודל, שאין לו זיכרון מיום שני ואין לו שום
...
בחר לפי הנתיב הזה והזריק לפני שהכלי רץ. לשלושת הפריטים האחרים אין `scope` כלל, ולכן דבר
אינו מגביל אותם והם מגיעים בקובץ הראשון שהסשן נוגע בו — פעם אחת כל אחד, הקשים תחילה, בתוך
[תקציב](#התקציב-ומה-קורה-כשלא-נכנסים-בו) שנוקב במפורש בכל מה שלא נכנס.
...
- **הוא בלי scope** — כל כלל חל על כל קובץ. כאן `scope` הוא רשימת globs, והקובץ ש-Claude
  עומד לגעת בו הוא זה שמחליט אילו פריטים הוא מקבל.
- **הוא לא מבחין** — העדפה יושבת ליד חשיפה משפטית בלי ששום דבר מבדיל ביניהן. כאן הדרג של
  פריט מחליט אם מותר לו בכלל לכוון את המודל (טקסט נורמטיבי מוזרק במלואו; נימוקים רק
  נספרים, מאונדקסים וניתנים לחיפוש), והחומרה שלו מחליטה אילו פריטים מגיעים ראשונים
  לתקציב מלא.
- **הוא גדל עד שמדלגים עליו** — שום דבר בו אינו מתעד מתי הוא היה רלוונטי לאחרונה. כאן לכל
...
- **החילוץ מעוגן בציטוט, ול-my_context אין מודל משלו.** כל מועמד שנשלף ממסמך חייב לשאת
  מקטע שהועתק מילה במילה מהקטע שממנו הוא בא; המקטע נבדק בהתאמה מדויקת אחרי כיווץ רווחים,
  ופרפרזה נדחית. הבדיקה נגד המצאה היא מכנית ולא הנחיה שמבקשת יפה, ואין בשום מקום בתהליך
  מפתח API ולא עלות היסק.
  ← [ממסמך לפריטי טיוטה](#ממסמך-לפריטי-טיוטה)
- **גבול האמון הוא דרג בחירה, לא מדיניות.** פריט נורמטיבי ש-Claude לוכד נוחת כ-`draft`,
  ו-`draft` אינו מתקבל לאף דרג הזרקה — הבורר מפיל כל פריט שהסטטוס שלו אינו `active` עוד
...
- **הקורפוס הוא Markdown שבבעלותכם ומסד הנתונים הוא בר-השלכה.** קובץ אחד לכל פריט במאגר
  שלכם, כשכל אחד נושא checksum שנחתם מחדש בכל כתיבה; אינדקס ה-SQLite נגזר מהקבצים האלה
  ו-<span dir="ltr">`mycontext rebuild`</span> בונה אותו מחדש מאפס.
...
[שני פרקים מכאן](#שלוש-הקטגוריות-שרק-full-מפעילה).
