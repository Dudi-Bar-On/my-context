...
8. [מילון מונחים](#8-מילון-מונחים) — כל מונח שהמסמך הזה נותן לו משמעות מסוימת

**שום דבר כאן אינו מתאר התנהגות שאין לפרויקט הזה.** היכן שיכולת מוגבלת, המגבלה כתובה לצד
אותה יכולת; היכן שהוחלט נגד משהו, זה כתוב כהחלטה במקום שבו מתוארת היכולת שהיא מגבילה. אין
פרק של התנהגות שלא נבנתה, כי קורא אינו יכול להבחין בין פרק כזה למפת דרכים.
[איך המסמך הזה נבדק](#איך-המסמך-הזה-נבדק) נוקב בבדיקות שמחזיקות את שני המסמכים מול התוכנית,
ואומר מה הן אינן יכולות לראות.
...
> מונח שהמסמך הזה נותן לו משמעות מסוימת מוגדר ב[מילון המונחים](#8-מילון-מונחים). כל
...
אל תסמכו על משפט כאן שעודכן. הריצו `mycontext help` לרשימת הפקודות האמיתית,
...
שה-hooks פולטים; שלכל פרק שתוכן העניינים מקשר אליו יש שורה בסיכום היכולות שלמטה, או שהוא
מנוי — עם נימוק — כמשהו שהמוצר אינו *עושה*; וששני המסמכים נושאים את אותו רצף כותרות ואת
אותן דוגמאות באותו סדר. מתוכם, <span dir="ltr">`counts.test.ts`</span> מחשב מהתוכנית הרצה
את היחס "20 מתוך 49 פקודות שורת הפקודה" שב[פרק 5](#מה-שאתה-מקליד-פקודות-הסלאש) ונכשל
ב**שתי** השפות אם אחד מחצאיו סוטה — הוא סטה פעמיים לפני שהבדיקה נולדה — והוא מחשב באותה
דרך גם את מניין הקבצים שבפסקה הזאת עצמה. <span dir="ltr">`parity.test.ts`</span> מחזיק את
רצף הכותרות של המסמך הזה מול המקור האנגלי.
...
פקודה ושכל אי-סימטריה שהמסמך הזה מצהיר עליה מוצהרת גם בתוכנית,
ו-<span dir="ltr">`test/plugin/write-commands.test.ts`</span> מריצה את ההרצה היבשה שכל
פקודת כתיבה נוקבת ומוודאת שהיא מציגה תצוגה מקדימה, מסרבת, ואינה כותבת דבר.
...
קוראים עכשיו — מדגים את העיוורון הזה במקום רק לטעון אותו. בדיקת המצאי קוראת את הקובץ
האנגלי כולו, ולכן פקודה שנקובה ב[טבלת הדגלים](#כל-הדגלים-במקום-אחד) או
ב[מילון המונחים](#8-מילון-מונחים) נבדקת שהיא קיימת כמו כל פקודה אחרת — אבל שום בדיקה בשום
מקום אינה בודקת אם *דגל* מתנהג כפי שהשורה שלו אומרת. וכל עיגון שעובד בדרך של דרישת ביטוי
מסופק בשלילה שמוצבת לפני אותו ביטוי; רק בלוקי הדוגמאות, שמושווים בשלמותם, חסינים. קריאת
המסמך הזה מול עץ עבודה היא הדרך היחידה לדעת שהוא נכון.
...
כל מה שכתוב כאן עובד היום, וכל שורה מקשרת לפרק שמכסה אותה במלואה. היכן שאחת מהן מוגבלת,
המגבלה כתובה בפרק שמכסה אותה — למסמך הזה אין פרק של התנהגות שאינה קיימת, כי קורא אינו יכול
להבחין בין פרק כזה לתוכנית אספקה.
...
- פריט **שולט** כשהוא כשיר להזרקה וגם מנוסח כהוראה — משהו שהמודל אמור לציית לו, ולא רק
  לדעת עליו. אם קוראים את זה כמכניקה זו טעות: **שום דבר כאן אינו אוכף פריט.** שום hook,
  שום כלי ושום פקודה אינם קוראים פריט — או את החומרה שלו — כדי להחליט אם פעולה רשאית
  להתבצע, ושני ה-hooks שכן חוסמים קוראים נתיב ושער שיגור במקומו. שליטה עוסקת במה שמגיע
  למודל, ולעולם לא במה שכלי רשאי לעשות; הפער נאמר במלואו במקום שבו
  [`severity` מוסבר](#צעד-2--זה-נשמר-כ-markdown-שאפשר-לקרוא-להשוות-ולסקור).
- **דרג** היא המילה לחלוקה בין נורמטיבי לנימוקים. כל קטגוריה נושאת דרג אחד —
  ...
  [פרק 4](#4-מתי-זה-חוזר-ומה) קורא לחמשת מסלולי האספקה שלו *דרגי הזרקה*. במקומות שבהם
...
נימוקים אינם נכנסים לסשן בדרך הזאת אלא אם ביקשתם זאת. בתחילת סשן הם תורמים ספירה — "2
decision · 1 lesson" — ולא יותר. הם מאונדקסים, ניתנים לחיפוש ונשלפים לפי בקשה, ואינם מנסחים
את עצמם כפקודה. המסלול היחיד שדרכו הם מגיעים במלואם הוא זה שאתם מדליקים לכל פריט בנפרד:
[דרג ההמשכיות](#4-מתי-זה-חוזר-ומה) אינו מתייעץ בחלוקה בין נורמטיבי לנימוקים, ולכן פריט
שסימנתם <span dir="ltr">`continuity: true`</span> נישא אל הסשן הבא לאיזה דרג שלא ישתייך.
...
| `status` | <span dir="ltr">`draft`, `active`, `superseded`, `deprecated`</span> או `validated`. **רק `active` מוזרק אי פעם.** מה אומרים ארבעת האחרים כתוב ב[מילון המונחים](#8-מילון-מונחים) |
...
> [!IMPORTANT]
> **שום דבר אינו אוכף <span dir="ltr">`severity: hard`</span>.** הוא משנה בחירה ותו לא:
> פריטים קשים מתקבלים לתקציב של דרג לפני רכים, ופריט קשה פטור ממיקוד סשן — מיקוד לעולם אינו
> מסתיר אחד כזה. **שום hook, שום כלי ושום פקודה אינם קוראים את החומרה של פריט כדי להחליט אם
> פעולה רשאית להתבצע.** hook כאן חוסם בדיוק שני דברים, ואף אחד מהם אינו קורא חומרה: כתיבה
> לתוך <span dir="ltr">`.my_context/`</span> עצמה, ו — רק בפרויקט שהדליק את
> [`dispatchGate`](#dispatchgate--לדרוש-משיגור-שינקוב-בפריט-שלמענו-הוא-נעשה) — שיגור
> <span dir="ltr">`Agent`</span> שאינו נוקב בפריט משימה שיש לקורפוס הזה.
> [פרק 2](#2-הרעיון) מתאר ידע נורמטיבי כמה ש*חייב להתקיים* ושואל "מה אסור לי לטעות בו כאן?",
> וקורא יכול בהחלט לקרוא את זה כמכניקה. "מועמד לאכיפה עתידית" שבסכימת
> <span dir="ltr">`create_item`</span> הוא הקריאה המדויקת, וכאן הפער נאמר במפורש ולא נרמז
> דרך הסתייגות.
...
פריט אחד הגיע במלואו, מפני שהוא נעוץ. ארבעה הם פריטים מכווני-נורמה שהגיעו ככותרת בלבד —
כל אחד מהם נקוב בגילוי שמעל האינדקס, כי כותרת אי אפשר לציית לה — וכל אחד מהם מופיע גם
כשורת אינדקס אחת, כך ש-Claude יודע
...
יש חמישה **דרגי הזרקה** — חמישה מסלולים שדרכם טקסט של פריט יכול להגיע לסשן. (זהו המובן
...
|---|---|---|
| **נעוץ** | בתחילת כל סשן, בכל שיגור תת-סוכן, ושוב אחרי כיווץ | כל פריט נורמטיבי פעיל שמסומן `always: true`, במלואו |
...
| **המשכיות** | בתחילת כל סשן, בכל שיגור תת-סוכן, ושוב אחרי כיווץ | כל פריט פעיל שמסומן `continuity: true`, במלואו — הדרג היחיד שאינו שואל אם הפריט נורמטיבי, ולכן גם `reference` יכול לנסוע בו |
| **אינדקס** | בתחילת כל סשן, בכל שיגור תת-סוכן, ואחרי כיווץ | שורה אחת לכל פריט נורמטיבי שנותר, ועוד ספירות לשאר |
...
- **בלי scope פירושו בלי הגבלה, אלא אם הקטגוריה אומרת אחרת.** פריט בלי תבניות scope חל על
  ...
  בקובץ עצמו.
- **כל פריט מגיע פעם אחת בסשן — כיווץ אינו מאפס את הספירה.** my_context רושם מה כבר הזריק, כך שעריכה של עשרה
  ...
  אינה מרעיבה ממנו תת-סוכן, וכל תת-סוכן מקבל אותו לכל היותר פעם אחת. תת-סוכן מקבל גם את
  הדרג הנעוץ, דרג ההמשכיות ואת האינדקס ברגע שהוא משוגר, מ-hook משלו — ראו
  [פרק 4, "תת-סוכן, בלידה"](#תת-סוכן-בלידה).
  הרישום שמאחורי זה הוא קובץ seen פר-סשן —
  <span dir="ltr">`.my_context/state/<session>.seen.jsonl`</span>, מצב מיוצר מקומי למכונה,
  שנגזם בחלון שמירה של 30 יום בידי `mycontext rebuild` ושוב בתחילת כל סשן — ולא אינדקס
  ה-SQLite. כשאי
...
> [!WARNING]
> **הזרקה בדיוק-בזמן סומכת על כל אינדקס שהיא מצליחה לקרוא, כולל מיושן.** ה-hook של
> בדיוק-בזמן מגיש מה-Markdown עצמו בשני מקרים בדיוק: הפתיחה לקריאה בלבד של
...
> תחילת הסשן אינה מושפעת: היא מזריקה מה-Markdown עצמו ורק מרעננת את האינדקס אחר כך, על בסיס
> מיטב-המאמץ. <span dir="ltr">`mycontext doctor`</span> מדווחת על טריות האינדקס, אבל רק
> כשמישהו מריץ אותה. נרשם ל-1.1 (<span dir="ltr">`docs/ROADMAP.md`</span>, E21) ולא תוקן
> ב-1.0.0.
...
**`/clear` הורס חלון הקשר, ו-my_context הורס את מה שהחזיק על אותו חלון.** תחילת סשן
שמדווחת על חלון שנוקה מסירה את קובץ ה-seen של אותו סשן, את קובצי האחים הפר-תת-סוכניים
שלצדו ואת תמונת המצב לשחזור שלו — *לפני* שהיא קוראת מהם משהו, וזה כל העניין שבענף: חלון
חדש וריק שהיה מקבל את מצב הדה-דופליקציה של החלון ההרוס היה מקבל פחות ממה שמגיע לו בעוד
מאגר הידע מאמין שהוא מלא, וכיווץ מאוחר יותר באותו סשן היה משחזר פריטים שהחלון הזה מעולם
לא החזיק. מה שבאמת הוסר נאמר במשפט אחד בתוך הבלוק המוזרק. מחיקה שנכשלת גורמת להזרקת יתר,
שהוא הכיוון הבטוח; מה שאסור לה להיות הוא שקטה.

שני שומרים על הענף הזה, ואף אחד מהם אינו קישוט. <span dir="ltr">`/mycontext:LoadMyContext`</span>
לעולם אינה יכולה להפעיל אותו, יהיה אשר יהיה מה שנמסר לה, כי למסלול הידני אין מזהה סשן
לנקות. גם תת-סוכן אינו יכול: מטען <span dir="ltr">`SubagentStart`</span> נושא את
ה-`session_id` של *ההורה* ואין לו `source` משלו, ולכן בלי השומר ערך תועה היה מאפשר לילד
למחוק את מצב המסירה של הסשן ששיגר אותו.

**מה שלא נמדד כאן, ונאמר במפורש במקום להישתמע.** הענף ממופתח על ערך ה-`source` שהוא
`clear`, שכבר מנוי במסנן ה-`SessionStart` של התוסף הזה. האם `/clear` אמיתי אכן שולח את
הערך הזה, והאם מזהה הסשן שורד אותו — לא נבדק במאגר הזה: זה דורש אדם ליד מסוף אינטראקטיבי,
ו-`claude -p` אינה יכולה לייצר `/clear`. כל מה שלמעלה מתאר מה הקוד עושה כשהערך הזה מגיע —
לא ירי שמישהו צפה בו.

**מצב סשן ישן נסרק ומנוקה בתחילת כל סשן.** <span dir="ltr">`state/`</span> צובר קובץ לכל
סשן, וכעת גם אחד לכל תת-סוכן, בעוד ש-`mycontext rebuild` הייתה המנקה היחידה שלו — פרויקט
שהקורפוס שלו יציב לעולם אינו בונה מחדש, ולכן לעולם לא ניקה. הניקוי רץ *אחרי* שההזרקה כבר
נכתבה, ולכן אינו יכול לעכב אותה, והוא מיטב-מאמץ: ניקוי שאינו יכול לרוץ לעולם אינו סיבה
להכשיל תחילת סשן. השמירה היא 30 יום לפי זמן השינוי. כשהוא מסיר קובץ דה-דופליקציה הוא אומר
זאת ב-stderr, ונוקב בכמה קבצים הוסרו וכמה מהם היו קובצי דה-דופליקציה, כי זה הרגע היחיד שבו
אפשר לומר את התוצאה — בהזרקה הבאה קובץ seen שנוקה אינו ניתן להבחנה מסשן טרי, ולכן סשן מובטל
שיתחדש יקבל שוב פריטים שכבר ראה. כפילויות, לעולם לא החמצה.
...
**מה שסשן קודם החזיק יכול להינשא אל הסשן הזה.** תחילת סשן קוראת את רישום המסירה של סשן
קודם אחד ומסמנת את פריטיו באינדקס של הסשן הזה: הם מורמים לראש הרשימה, וכל אחד מהם נושא
סימון <span dir="ltr">` · carried`</span>. המודל רואה במה החלון האחרון באמת עבד לפני שהוא
רואה את השאר. אלה שורות אינדקס בלבד — שום דבר אינו נמסר במלואו רק משום שנמסר פעם — וזה לא
קונה מקום נוסף: שורות נישאות חולקות את <span dir="ltr">`budgets.index`</span> עם כל שורה
אחרת.

| פקודה | מה היא עושה |
|---|---|
| <span dir="ltr">`mycontext session carry --show`</span> | מה סשן חדש היה נושא היום, והאם זו בחירה של מישהו או ברירת המחדל. השניים נאמרים בנפרד, כי הם מתנהגים אותו דבר עד שמופיע סשן חדש יותר |
| <span dir="ltr">`mycontext session carry <id>`</span> | לשאת מאותו סשן מכאן ואילך. מזהה ש-<span dir="ltr">`mycontext session list`</span> מסמנת כלא-<span dir="ltr">`carryable`</span> **נדחה** במקום להישמר: הוא לא היה מספק דבר בתחילת הסשן הבאה ולא היה אומר דבר על הסיבה |
| <span dir="ltr">`mycontext session carry --none`</span> | לא לשאת דבר. זה מצב בפני עצמו, לא אותו דבר כמו "מעולם לא בחרתי" — אחרת כיבוי הנשיאה היה נופל חזרה לברירת המחדל ולא הייתה שום דרך לכבות אותה בכלל |

כשלא נרשמה בחירה, המקור הוא **הסשן העדכני ביותר בסביבת העבודה הזאת שאינו הסשן הנוכחי**,
והוא זז ככל שסשנים חדשים רצים. תת-סוכן הוא היוצא מן הכלל: הוא נושא מהסשן ששיגר אותו, נקוב
לפי מזהה ולא מדורג לפי עדכניות, כי לתת-סוכן אין סשן משלו וההורה שלו הוא בדיוק החלון שהוא
ממשיך.

שני דברים שהנשיאה אינה עושה. **פריט שסשן המקור ראה רק כשורת אינדקס אינו נישא** — מה שנוסע
הוא מה שאותו סשן באמת החזיק במלואו, וזו ממילא הראיה החזקה יותר. ושורה נישאת אינה פטורה
מהתקציב: היא יושבת בראש סדר האינדקס ולא מחוצה לו, ולכן באינדקס שכבר מלא שורה נישאת
**דוחקת** שורה משל הסשן הזה.

שני אלה מדווחים במקום שבו הם קורים. שורת הנשיאה מתחת לכותרת האינדקס נוקבת בסשן המקור — בשמו
אם נתת לו אחד, בשמונת תוויו הראשונים אם לא — במספר השורות שבאמת הגיעו, וב**כל מזהה נישא
שלא קיבל שורה, עם הסיבה**: <span dir="ltr">`delivered in full this session`</span>,
<span dir="ltr">`unknown id`</span>, <span dir="ltr">`not a normative category`</span>,
<span dir="ltr">`no longer eligible`</span>, <span dir="ltr">`hidden by the active focus`</span>
או <span dir="ltr">`over the index budget`</span>. כשהאינדקס היה מוצה היא נוקבת גם במזהי
השורות של הסשן הזה שנדחקו כדי לפנות מקום. הסעיף האחרון אינו נוחות: הערת הגלישה לעולם אינה
מדווחת על גלישה של דרג האינדקס בלבד, ולכן השורה הזאת היא המקום היחיד שבו קורא לומד ששורה
נדחקה ולא סתם לא נכנסה בתקציב.
...
> **אין פקודה שיוצרת שכבה גלובלית, ואין פקודה שכותבת אליה.** השכבה נקראת בכל פקודה ובכל
> הזרקה, ושום דבר אינו יוצר אחת או כותב אליה.
...
> <span dir="ltr">`~/.my_context`</span> — האיות עם הקו התחתון, שאיש אינו קורא. הדגל היחיד
> ש-`init` מקבלת הוא <span dir="ltr">`--pack <path>`</span>:
...
> ו-<span dir="ltr">`mycontext repair`</span> נוקבת בשם הפריטים הגלובליים שסירבה להחתים
> מחדש ואומרת להריץ אותה "מסביבת העבודה של השכבה הגלובלית עצמה" — סביבת עבודה שאף פקודה
> אינה מייצרת. **זה פער, לא תכנון.** <span dir="ltr">`mycontext init --global`</span>, ודרך
> לכוון לכידה או עריכה אל השכבה הגלובלית, היו סוגרים את זה. אף אחד מהם אינו קיים.
...
עבודה נוסעים איתה; אף אחד מהם אינו נקרא מהשורש הגלובלי, ואף אחד מהם אינו מזיק. זהו מסלול
אמיתי, וכל פריט שהוא מייצר נכתב בידי הקוד שכותב כל פריט — אבל העברה אינה משטח נתמך, ויכולת
שנקראת בכל פקודה ובכל הזרקה לא אמורה להזדקק לה.
...
<!-- `core/config.ts` · `pinned: 6000, jit: 6000, restored: 8000, continuity: 2000, index: 1200,` · ~94 -->
...
**אלה אינם חינם, וכדאי לומר בפירוש מה המחיר.** הדרגים מצטברים: תחילת סשן מריצה `pinned`,
`continuity` ו-`index`, עד כ-9,200 טוקנים משוערים אם שלושתם מלאים, לפני שהקלדתם משהו — וכל הזרקה נפרדת שנורתה מקובץ
משלמת עד `jit` נוסף — לכל היותר פעם אחת לפריט בכל **סשן**, ולא בכל חלון הקשר, ולכל תת-סוכן
סשן משלו לעניין זה. רישום הדה-דופליקציה הוא קובץ פר-סשן והוא שורד כיווץ: כיווץ בונה את החלון
מחדש, ומה שמחזיר פריט לחלון הבנוי-מחדש הוא דרג `restored` ולא מסירת `jit` שנייה. הדבר היחיד
שכן מציע פריט שוב בתוך אותו סשן הוא **עריכה** של הפריט עצמו — השער פוטר ממסירה חוזרת רק כל עוד
ה-checksum הרשום עדיין תואם והמסירה הקודמת הייתה שלמה. מול חלון הקשר של 200,000 טוקנים, תקרת
הפתיחה הזאת היא כ-4.6%, ופרויקט שאין בו פריט מסומן `continuity` משלם 7,200 מתוכה, כ-3.6%.
...
פריטים מתקבלים **קודם כול לפי שליטה**, ודירוג זה נמצא מעל החומרה ולא מתחתיה: פריט
<span dir="ltr">`rule`, `constraint`, `invariant`, `instruction`, `requirement`</span> או
<span dir="ltr">`standard`</span> — וכן <span dir="ltr">`task`</span> או
<span dir="ltr">`plan`</span> שטרם הושלמו — נכנס לפני כל דבר אחר, מפני שכל פריט שולט שנמדד
בקורפוס הזה כנשפך היה <span dir="ltr">`severity: soft`</span>, ודירוג השליטה במקום השני היה
מותיר כל אחד מהם מפסיד ל-<span dir="ltr">`pattern`</span> קשיח. מתחת לדירוג הזה הסדר הוא
<span dir="ltr">`severity: hard`</span> לפני <span dir="ltr">`severity: soft`</span>, ואז
...
תת-סוכן — החלון הנפרד של כלי <span dir="ltr">`Task`</span> — לעולם אינו רואה
<span dir="ltr">`SessionStart`</span>: הוא יורש את <span dir="ltr">`session_id`</span> של
ההורה כלשונו, ורק <span dir="ltr">`agent_id`</span> במטען ה-hook מבדיל בין השניים.
<span dir="ltr">`SubagentStart`</span> הוא מה שבא במקומו, ובוחר בדיוק כמו תחילת סשן רגילה:
**דרג נעוץ, דרג המשכיות ואינדקס**, בתוספת ה-carry החוצה-סשנים שנפתר מהסשן של *ההורה* לפי
מזהה (ולא "הסשן העדכני ביותר", מה שהיה מוסר לתת-סוכן פריטים של חלון לא קשור). הבלוק שלו
אומר זאת עם ההגעה — פתיח שנוקב בשם התוסף ומצביע על קובצי ה-Markdown שמאחוריו — משום שטקסט
לא מיוחס שמגיע לחלון הקשר טרי נקרא כהתקפה, ותת-סוכן דיווח בדיוק את זה על ההזרקה של עצמו
לפני שזה נוסף.

**הפער האמיתי שנשאר הוא זמן תגובה, לא מסירה.** <span dir="ltr">`SubagentStart`</span> חוסם
את השיגור שהוא נורה בשבילו — נמדד: hook שרץ 3,018 מ"ש עיכב את קריאת הכלי הראשונה של
תת-הסוכן באותו זמן — ו-Claude Code הורגת hook שרץ מעבר לחמש השניות ש-<span dir="ltr">`hooks/hooks.json`</span>
מצהירה. תת-סוכן שה-hook שלו נהרג מתחיל בלי שום ידע של הפרויקט הזה, ושום דבר בבחירה עצמה
אינו חסום בתוך התהליך כדי להבטיח שהיא מסתיימת קודם. זה חשוף, לא שקט: יומן הביקורת רושם
<span dir="ltr">`delivery=attempted`</span> לפני העבודה ו-<span dir="ltr">`delivery=complete`</span>
אחריה, תחת <span dir="ltr">`session_id`</span> של ההורה, כך שניסיון בלי השלמה תואמת הוא
בדיוק תת-סוכן שהתחיל בלי כלום.
...
**המודל** קורא לעשרים ושמונה כלי ה-MCP. שני המשטחים קוראים וכותבים לאותם קובצי Markdown תחת
...
| <span dir="ltr">`SubagentStart`</span> | תת-סוכן משוגר | נבחר כתחילת סשן, ומזריק את הדרג הנעוץ, דרג ההמשכיות ואת האינדקס לחלון הריק של התת-סוכן, ממוסגר בהסבר מאין הבלוק הגיע | 5 |
...
| <span dir="ltr">`PostCompact`</span> | אחרי כיווץ, ברגע שהתקציר שלו קיים | שורת ביקורת אחת שאומרת שהכיווץ הסתיים, עם ה-<span dir="ltr">`trigger`</span> שלו ושלושה מספרים: מה <span dir="ltr">`PreCompact`</span> לכד, מה דרג השחזור סיפק מחדש, וכמה מהמזהים האלה התקציר עדיין נוקב בהם בעצמו. אינו מזריק דבר — הפלטפורמה אינה מגדירה מעטפת פלט לאירוע הזה | 5 |
...
הריצה של עצמו ברגע שהמטען בידיו — הבחירה סינכרונית, ושום טיימר בתוך התהליך אינו יכול
להפסיק עבודה סינכרונית. שניים מהם ראויים לתשומת לב מהסיבה הזאת. ל-<span dir="ltr">`SessionStart`</span>
יש תקציב שהות של 500 מילישניות, שנשמר בידי בדיקת ביצועים ולא בידי חיתוך בזמן ריצה.
ו-<span dir="ltr">`SubagentStart`</span> **חוסם את השיגור שעליו הוא נורה** — נמדד: hook
שלקח 3,018 מילישניות עיכב את קריאת הכלי הראשונה של התת-סוכן עד שחזר — ולכן מה שהוא עולה
משולם בכל שיגור, לפני התור הראשון של התת-סוכן.
...
[פקודות הסלאש](#מה-שאתה-מקליד-פקודות-הסלאש)). אף אחת מהן אינה כותבת רשומת ביקורת: שתיהן משנות מטא-נתון
של סשן, לא דבר שפריטי הפרויקט הזה מצהירים.

**מתן שם לסשן הוא צעד ידני, וזה נדחה במכוון ולא נותר לא-גמור.**
<span dir="ltr">`UserPromptExpansion`</span> נורה ברגע שאחת מפקודות
<span dir="ltr">`/mycontext:*`</span> של התוסף הזה מוקלדת, והאירוע נושא
<span dir="ltr">`command_name`</span> ו-<span dir="ltr">`session_id`</span> — כך שה-hook
כותב רשומת ביקורת אחת שמתעדת איזו פקודה רצה ובאיזה סשן. **זהו זיהוי, לא מתן שם.** ה-hook
אינו קורא ל-<span dir="ltr">`mycontext session name`</span> במקומכם, ואין מסלול מ"בדיוק
הרצתי פקודת סלאש" לשם שאפשר להקליד במקום קידומת הקסדצימלית שמדלג על שני הצעדים הרגילים:
<span dir="ltr">`mycontext session list`</span> כדי למצוא את הסשן, ואז
<span dir="ltr">`mycontext session name <id> <name>`</span>, כשהמזהה מועתק ביד. הדבר היחיד
שכתיבה אוטומטית הייתה עושה הוא בדיוק מה ש-<span dir="ltr">`mycontext session name`</span>
כבר עושה במפורש, כך שהרווח הוא חיסכון בהעתקת מזהה ולא הפעלת משהו חדש — והמחסן שאליו היא
הייתה כותבת, <span dir="ltr">`.my_context/state/session-names.json`</span>, הוא אותו מחסן
שאיבד כתיבות תחת מקביליות עד השבוע הזה. כל hook כאן כתוב גם תחת האינווריאנטה שהוא לעולם
אינו חוסם הנחיה: הוא נכשל בפתיחה. כתיבה שנכשלת בפתיחה היא כתיבה שלפעמים אינה מתרחשת, ושם
סשן שלפעמים נקבע הוא תכונה גרועה יותר משם שתמיד מוקלד ביד.
...
<span dir="ltr">`/mycontext:handover`</span> מבקשת את פתק ה-handover עכשיו, בכל תפוסה
שחלון ההקשר מחזיק כרגע, במקום להמתין לסף — וכשאינה יכולה לומר באיזה סשן היא נמצאת, כמה
החלון מלא, או האם לסשן הזה עדיין רצות שרשראות עבודה, היא אומרת זאת ואינה מבקשת דבר.
...
**29 פקודות ה-<span dir="ltr">`/mycontext:add-<type>`</span> ו-29 פקודות
ה-<span dir="ltr">`/mycontext:list-<type>`</span> *הן* בורר הקטגוריה.** לכן הן נוצרות לפי
קטגוריה ולא מקבלות ארגומנט <span dir="ltr">`<type>`</span>: ההשלמה האוטומטית מסננת את
הרשימה תוך כדי הקלדה, ושום דבר בתוסף אינו יכול לשים תפריט על ארגומנט.
<span dir="ltr">`/mycontext:add`</span> מקבלת את הארגומנט במקום, וזו אינה נסיגה מזה —
מתן שמות עובד רק לקטגוריות שהמחולל הכיר, ולכן צורת הארגומנט היא מה שנשאר
ל[קטגוריה שהגדרתם בעצמכם](#קטגוריות-שאתם-מגדירים-בעצמכם), פקודה אחת לצד ה-29 ולא במקומן.

יש <span dir="ltr">`add-<type>`</span> אחת ו-<span dir="ltr">`list-<type>`</span> אחת לכל
קטגוריה **מופעלת** — 58 היום — ועוד 32 שאינן לפי קטגוריה:
...
`harden`,
`soften`, `supersede`, `refresh`, `link`, `unlink`, `handover`, `ingest`, `lesson`, `lesson-stage`</span>.
הזוגות שלפי קטגוריה
...
כל 90 אלה נושאות <span dir="ltr">`disable-model-invocation: true`</span>, וזה בתוקף — הן
...
לכל שורה חד-צדדית יש נימוק, ולכל פקודת שורת פקודה בלי פקודת סלאש יש נימוק משלה. **זה מתקיים
עכשיו, ונאכף על ידי בדיקה ולא על ידי סקירה**: לכל אחד מעשרים ושמונה כלי ה-MCP יש פקודת שורת
פקודה, פקודת סלאש, או שתיהן.

מה שנותר הוא אי-סימטריה בכיוון ההפוך — פקודות בלי פקודת סלאש — והוא **רשום ולא ממתין
לגילוי**: <span dir="ltr">`CLI_WITHOUT_SLASH`</span> ב-<span dir="ltr">`src/plugin/parity.ts`</span>
אומר של-20 מתוך 49 פקודות שורת הפקודה אין אחת, ונוקב, לכל אחת, בסיבה. כל ערך הוא אחת משתי צורות: פקודה
שרצה פעם אחת, לפני שיש סשן שפקודת סלאש יכולה לרוץ בתוכו (<span dir="ltr">`init`</span>,
<span dir="ltr">`rebuild`</span>), או פקודה שכתיבתה פקודת סלאש הייתה מאפשרת למודל לבצע
במקום אדם — בדיוק מה שהפקודה קיימת כדי לשמור בידי האדם. הרשימה המלאה, עם הנימוק לכל אחת,
היא במקור.

### מה שאתה מריץ: שורת הפקודה

49 פקודות. `mycontext help` מדפיס את אותה רשימה מהתוכנית עצמה,
...
**שני נושאים אינם קיימים**: <span dir="ltr">`mycontext help query`</span>
ו-<span dir="ltr">`mycontext help config`</span> מסורבות שתיהן בשמן. אף אחד משני הנושאים
אינו בלתי מתועד — [הסכמה של האינדקס](#הסכמה-של-האינדקס-ואיך-לתשאל-אותה) נושאת שאילתות
...
הקורפוס ומה מפתח תצורה עושה, הם אלה שהוא אינו יכול לענות עליהם. הספירה עברה מארבעה נושאים
לשבעה והפער לא זז, וזו הנקודה ששווה לשמור: שלושת הנושאים שנוספו מאז הם שלושת *משטחי
ההפעלה*, ואף אחד משני הנושאים האלה אינו משטח הפעלה.
...
**מחיקה לא תתווסף כלל.** <span dir="ltr">`NOGOAL-no-agent-hard-delete`</span> הוא פריט פעיל
בקורפוס של המאגר הזה עצמו, שמתעד את זה כלא-מטרה מכוונת ולא כעבודה שאיש לא הגיע אליה. פרישה
היא החלפה — <span dir="ltr">`mycontext supersede <id> --by <id>`</span>, שקיימת — והיא
משאירה את הפריט, גופו והיסטוריה שלו על הדיסק, היכן שסוקר עדיין יכול לקרוא אותם.
...
| <span dir="ltr">`mycontext show <id>`</span> | פריט אחד במלואו, בדיוק כפי שהוא על הדיסק. <span dir="ltr">`--json`</span> מדפיס את אותו פריט כמסמך JSON אחד, ובו כל הערה שהצורה ה-Markdown מדפיסה לצידו — תקציר שכבר אינו מתאר את הפריט, רישום ביומן שנכשל, קובץ שלא ניתן היה לקרוא |
...
| <span dir="ltr">`mycontext ready`</span> | משימות פתוחות שכל מה שרשום ב-<span dir="ltr">`needs`</span> שלהן כבר <span dir="ltr">`done`</span>, לפי סדר עדיפות. <span dir="ltr">`--plan`, `--held`, `--limit`</span>. היא עונה על "במה אפשר להתחיל עכשיו" בפרויקט שקטגוריה שלו מכריזה על <span dir="ltr">`plan`, `seq`</span> ו-<span dir="ltr">`state`</span>; פרויקט בלי קטגוריה כזו מקבל את המשפט הזה ולא רשימה ריקה. המוכנות **נגזרת בכל הרצה** מ-<span dir="ltr">`needs`</span> וממצבי מה שהוא מציין, ולכן אין מצב <span dir="ltr">`ready`</span> שיכול להתיישן, וכל משימה פתוחה שאי אפשר לשחרר נספרת לפי הסיבה ומוצגת עם <span dir="ltr">`--held`</span> |
| <span dir="ltr">`mycontext path`</span> | לכל נושא: כמה מהפריטים שלו הושלמו, מה מוכן להתחלה עכשיו, מה תקוע בגלל חסם שעוד לא נחת, ו — הקטגוריה שאף פקודה אחרת אינה רואה — מה **ממתין לך**. <span dir="ltr">`--d`, `--all`</span>. נושא הוא מספר D; אילו תוכניות הוא מחזיק נקרא מתוך הבלוק <span dir="ltr">`[D-MAP]`</span> שבפריט המרשם של הפרויקט, וכל מספר לידו **נגזר בכל הרצה** ממצבי הפריטים שאותו בלוק מציין. שום דבר אינו נשמר, ולכן אין קובץ התקדמות שיכול להתיישן. פרויקט שאינו מחזיק מרשם כזה מקבל את המשפט הזה ויוצא באפס |
...
מדובר — מהיכן הגיע ומתי. שתי חבילות שקוראות לעצמן אותו שם הן שתי רשומות, שנשמרות בנפרד, ולכן
<span dir="ltr">`--pack <name>`</span> עשוי להתאים לשתיהן; במקרה כזה הוא אינו מקדם אף אחת מהן,
נוקב בשתיהן, ו-<span dir="ltr">`--source <path>`</span> אומר באיזו מהן מדובר. ייבוא חוזר של
*אותו* מוצר מחליף את הרשומה שלו עצמו, וזה מה שהופך ייבוא חוזר לעדכון ולא לרשומה שנייה של חבילה
אחת.
...
| <span dir="ltr">`mycontext ack <id> <code>`</span> | רישום שאדם קרא ממצא אחד של <span dir="ltr">`doctor`</span> על פריט אחד והכריע בו. הממצא עדיין מחושב, עדיין מדווח ועדיין נספר — הוא מדווח כ**מאושר** במקום כפתוח, וזה כל מה שהפועל הזה עושה. האישור נשמר על הפריט יחד עם גיבוב של התוכן שמולו הוא ניתן, ולכן עריכה של אותו פריט **מפקיעה** אותו והממצא נפתח שוב; <span dir="ltr">`--list`</span> מדפיס כל ממצא על הפריט ואם הוא מאושר, מופקע או פתוח, ו-<span dir="ltr">`--clear`</span> מבטל אישור. הפקודה מסרבת לקוד ש-<span dir="ltr">`doctor`</span> אינו מדווח כרגע על אותו פריט, ומסרבת לכל מקור שאינו אדם: אין כלי MCP ואין פקודת סלאש, משום שאישור שמכונה כתבה על עצמה הוא דיווח על מה שהמכונה החליטה להפסיק להזכיר |
| `mycontext decay` | פריטים שלא הוזרקו לאחרונה |
| `mycontext contribution` | כמה פעמים כל פריט **נמסר** בפועל לתוך סשן, נקרא לאחור מתוך יומן הביקורת — שורה לכל מקור (<span dir="ltr">`human`</span>, <span dir="ltr">`agent`</span>, <span dir="ltr">`ingest`</span>), ואחריה הפריטים שנמסרו הכי מעט. אין כאן מסלול כתיבה חדש ואין שינוי בשום הוק: <span dir="ltr">`recordAudit`</span> תמיד כתב רשומת הזרקה אחת לכל מסירה, ובה מה שהוזרק ומה שהתקציב שפך — ולכן זה עונה על היסטוריה שכבר קרתה. קראו את שני הסייגים שהפקודה מדפיסה לפני שפועלים על מספר: היומן רושם **הזרקה, לא קריאה ולא הישענות**, ולכן פריט שנפתח כ‑Markdown או נשלף ב-<span dir="ltr">`show`</span> אינו מותיר בו עקבות; והמניינים אינם מנורמלים לפי גיל. <span dir="ltr">`--full`</span> לכל הפריטים עם המסירה הראשונה והאחרונה, <span dir="ltr">`--json`</span> לצורה המכונתית |
...
| <span dir="ltr">`mycontext carry <id>`</span> | מסמנת **פריט אחד** להזרקה הבאה בלבד, ואז שוכחת אותו — הכרעה על עכשיו, לא הבחירה העומדת של <span dir="ltr">`session carry`</span> באיזה סשן להמשיך, ולא הנצח של `pin`. משתמשת באותו גילוי בראש-תור, ונשכחת ברגע שההזרקה הבאה רצה — בין אם השורה התקבלה בתקציב ובין אם לא; סימון שאיש לא מנצל פשוט ממתין, גלוי בכל רגע עם <span dir="ltr">`--show`</span>, ולעולם אינו מתרחב מעצמו. סימון פריט שכבר נמצא בהקשר שלך עולה לכל היותר שורה מבוזבזת בראש התור — לפקודה הזאת אין דרך לדעת מה מכיל חלון של סשן מסוים כרגע, ולכן היא אינה מסרבת בשמך. <span dir="ltr">`--clear`</span> מבטלת את כל התור הממתין |
| <span dir="ltr">`mycontext handover ask`</span> | מבקשת את פתק ה-handover **עכשיו**, בכל תפוסה שחלון ההקשר מחזיק כרגע, במקום להמתין ל-<span dir="ltr">`handover.thresholdPercent`</span> ש-<span dir="ltr">`Stop`</span> משגיח עליו. היא יורה את אותה בקשה שהוו יורה, ולכן כל קורא שלה נשאר כשהיה — שורת הסטטוס, עקבות הביקורת, וההשוואה בין זמני הכתיבה שמכריעה אם נענתה. אפשר להריץ אותה **רק מתוך סשן של Claude Code** — היא מבקשת מהסשן שאתה נמצא בו לכתוב את ה-handover שלו, ולכן מחוצה לו אין ממי לבקש, ואין בכוונה שום דגל שנוקב בסשן ביד. היא **מסרבת במקום לנחש** גם בשאר המקרים, וכל סירוב נוקב במה שלא ניתן היה לבסס ואומר ששום דבר לא נכתב: כשאינה יכולה לקרוא כמה החלון מלא — שום אחוז אינו מומצא, וההודעה אומרת אם גשר שורת הסטטוס חסר, שותק או מיושן; וכששרשראות עבודה של הסשן הזה עדיין רצות, והיא נוקבת בשמן אחת-אחת, כך שהבחירה בין להמתין, לעצור אותן בעצמך, ולהמשיך עם <span dir="ltr">`--anyway`</span> היא שלך. שום דבר כאן אינו כותב את הפתק, ושום דבר כאן אינו עוצר שרשרת עבודה: ל-my_context אין שליטה שמסיימת תת-סוכן |
| <span dir="ltr">`mycontext rules [verify\|list\|show]`</span> | **מאגר כללי המוצר** — אוסף קטן של קבועים על my_context עצמה (עובדות, איסורים, נהלים, תקנים, הגדרות) שנשלח בתוך החבילה ואינו חלק מהקורפוס שלכם. הוא לעולם אינו מופיע ב-<span dir="ltr">`list`</span>, <span dir="ltr">`ready`</span>, <span dir="ltr">`doctor`</span> או בהזרקה שנבחרה מהפריטים שלכם, הוא אינו מוציא דבר מהתקציב שלכם, ודבר כאן אינו כותב לתוך <span dir="ltr">`.my_context/`</span>. כל רשומה מסומנת <span dir="ltr">`product`</span> — נכונה לכל מי שמתקין את הכלי — או <span dir="ltr">`developer`</span>, שחלה רק כשהפרויקט שעובדים עליו הוא my_context עצמה, כך שכלל על אופן העבודה של המאגר הזה לעולם אינו נשלח כחוק של הכלי. <span dir="ltr">`list`</span> מונה את הרשומות שבתוקף במקום שבו אתם עומדים; <span dir="ltr">`show <id>`</span> מדפיסה אחת בשלמותה, כולל הבקשה שהולידה אותה במילים של מי שביקש; <span dir="ltr">`verify`</span> בודקת כל רשומה מול הסיכום שנשלח איתה ומנקבת בשם את אלה שחסרות, שונו, או לא נשלחו מעולם. מאגר שאינו מאומת מסרב **כתיבות** ועדיין מתיר **קריאות** — חסימת קריאות הייתה מענישה אתכם על התקנה שעדיין אפשר להתאושש ממנה. <span dir="ltr">`--restore`</span> מחזירה את מה שנשלח, מהחבילה המותקנת ולעולם לא מהרשת. <span dir="ltr">`--restore`</span>, <span dir="ltr">`--json`</span> |
| `mycontext ui` | ממשק הרשת לקריאה בלבד, מוגש על <span dir="ltr">`127.0.0.1`</span> — <span dir="ltr">`--port N`</span>, <span dir="ltr">`--no-open`</span> מדפיסה את הכתובת במקום לפתוח דפדפן, ו-<span dir="ltr">`--idle-ms N`</span> מזיזה את החלון שאחריו שרת שלא נגעו בו יוצא. לולאה מקומית בלבד: הוא מסרב לעלות בכל כתובת אחרת במקום להזהיר. הדף מחליף nonce חד-פעמי שבמקטע הכתובת באסימון שאינו מגיע לא לדיסק ולא לשורת פקודה, והשרת יוצא מעצמו אחרי שמונה שעות של חוסר פעילות כברירת מחדל. [ממשק הרשת](#ממשק-הרשת--mycontext-ui) מתאר את המסכים ואת מה שאף אחד מהם אינו יכול לעשות |
| `mycontext statusline` | הגשר האופציונלי אל שורת הסטטוס של Claude Code, והדבר היחיד כאן שכותב מחוץ ל-<span dir="ltr">`.my_context/`</span>. <span dir="ltr">`mycontext statusline install`</span> מדפיסה את הגדרת ה-<span dir="ltr">`statusLine`</span> שיש לך עכשיו ואת מה שיחליף אותה בדיוק, ואינה כותבת **דבר** בלי <span dir="ltr">`--yes`</span>; <span dir="ltr">`--settings <path>`</span> בוחרת את הקובץ, וברירת המחדל היא זה של Claude Code (<span dir="ltr">`CLAUDE_CONFIG_DIR`</span>, ואחרת <span dir="ltr">`~/.claude/settings.json`</span>). לאחר ההתקנה <span dir="ltr">`mycontext statusline`</span> רצה בכל הודעה של העוזר: היא מדפיסה את המודל, את ההקשר שבשימוש וכמה ממנו הגיע מידע הפרויקט, ומעתיקה את המטען של Claude Code לקובץ אחד לכל סשן שממשק הרשת קורא. <span dir="ltr">`mycontext statusline uninstall --yes`</span> מחזירה את ההגדרה שהוחלפה — הקובץ כולו נשמר, לא רק המפתח, ולכן קובץ שלא השתנה מוחזר **בית אחר בית**, וקובץ שערכת מאז שומר על העריכה שלך ומקבל בחזרה רק את ה-<span dir="ltr">`statusLine`</span> שלו. היא מסרבת לחלוטין כאשר ה-<span dir="ltr">`statusLine`</span> שבקובץ אינו הגשר הזה, משום שהגדרה שקבעת אחרי ההתקנה אינה שלנו לדרוס ביציאה. [גשר שורת הסטטוס](#גשר-שורת-הסטטוס-בבחירה) |
...
- היא אינה יכולה לענות על *"מה הפריט ההוא אמר באותו רגע?"*. על כך עונה קובץ הפריט עצמו,
  ...
  לבקרת גרסאות.
- היא הדבר היחיד שיכול לענות על *"מתי הפריט הזה נוצר, או שונה לאחרונה?"*. **פריטים אינם
  נושאים שדות <span dir="ltr">`created_at`/`updated_at`</span> ב-frontmatter**, ולכן היומן
  יודע מתי כל שינוי קרה וה-Markdown של פריט בודד לא.
  <span dir="ltr">`REQ-changes-are-timestamped-and-audited`</span> — היסטוריית פעולות שאינה
  תלויה ב-git — ממומשת על ידי היומן הזה, והסעיף האחד הזה שלה עדיין אינו מקוים; הפריט
  בקורפוס אומר זאת בגופו.
...
שלו זהים בית-בית למה שהיו — ה-<span dir="ltr">`- [x]`</span> שאתה רואה
...
**היומן מצהיר עכשיו <span dir="ltr">`my_context/audit@2`</span> בכל שורה שהוא כותב.** הבנייה
הזאת קוראת גם <span dir="ltr">`@1`</span> וגם <span dir="ltr">`@2`</span>, ולכן יומן שכבר יש
לך ממשיך לעבוד אחרי השדרוג בלי שום הגירה. כל דבר אחר מסורב כפער גרסאות, והסירוב נוקב
ב**גרסה**:
...
> <!-- `pack/history.ts` · `return filterAudit(readAudit(root), { kind: 'mutation' })` · ~452 -->
> היסטוריה
...
>
> **הפטורים הם שלושה ולא אחד, וכל אחד מהם נחשף באותה דרך.** פריט שמסומן
> <span dir="ltr">`always: true`</span> פטור מאותה סיבה — מיקוד הסתיר פעם שישה פריטים נעוצים
> רכים במשך שלושה ימים — וכך גם פריט שמסומן <span dir="ltr">`continuity: true`</span>, שכל
> ההבטחה שלו היא שהסשן הבא לא יתחיל מאפס. הבלוק המוזרק מדפיס שורה לכל פטור שהופעל, ולכן אף
> אחד משלושתם אינו הישרדות שקטה.
...
#### הבאת אחת פנימה — `mycontext pack import`

**מהיכן מגיעות חבילות: [<span dir="ltr">`docs/TEMPLATES.md`</span>](TEMPLATES.md), ומשום מקום
אחר.** הגילוי הוא רשימה מנוהלת ביד במאגר הזה — אין מרשם, אין הבאה מחדש, אין ערוץ עדכון ואין
...
<span dir="ltr">`config.json`</span> שהיה משנה דרג של קטגוריה שכבר יש לכם, פריט שנושא
<span dir="ltr">`valid_until`</span>, נקודת קוד מסוננת, שני קבצים שתובעים מזהה אחד. כל אחד
מהם נוטש את הייבוא כולו בלי שנכתב דבר — הבטחה שמחצית התכנון יכולה להבטיח מפני שהיא אינה
כותבת דבר בשום מסלול.

**דוח ההתנגשויות מודפס ראשון, בכל מסלול — גם עם <span dir="ltr">`--yes`</span>.** הוא ממיין
את הפריטים המגיעים לשלושה דליים ומדפיס את שלושתם, ריקים או לא:

- **new** — שום פריט כאן אינו מחזיק את המזהה הזה. אלה מה שייבוא יוצר;
- **changed** — פריט כאן מחזיק את המזהה הזה, עם תוכן אחר. לכל אחד מהם הדוח נוקב בשדות
  שנבדלים, בשני גיבובי התוכן, במה שדריסה הייתה עולה, ובשאלה אם דריסה יכולה להגיע אליו
  בכלל: לארבעה מהשדות שהגיבוב מרכיב אין מסלול כתיבה, ולכן פריט שנבדל רק בהם נקוב כ*לא ניתן
  לדריסה כאן* במקום שיובטח וידולג בשקט;
- **identical** — אותו מזהה, אותו תוכן. אין מה לעשות.

**ואז שתי שאלות, לא אחת.** הראשונה מאשרת את הייבוא, ו-<span dir="ltr">`--yes`</span> עונה
עליה. השנייה נשאלת רק כשדלי ה-<span dir="ltr">`changed`</span> מחזיק משהו שדריסה יכולה
באמת להגיע אליו, והיא שואלת אם להחליף את הפריטים ש*אתם* כתבתם בגרסאות של החבילה —
ו**<span dir="ltr">`--yes`</span> אינו עונה על זו**. בלי אינטראקציה האישור הזה נכתב
<span dir="ltr">`--overwrite-changed`</span>. הפיצול הוא כל העניין:
<span dir="ltr">`--yes`</span> הוא הסכמה לייבוא שתיארתם, לא הסכמה להחליף כלל שאתם כתבתם.
סירוב לשאלה השנייה אינו שגיאה ואינו נוטש את הראשונה — הפריטים החדשים עדיין נוחתים, וכל
פריט שנבדל ונשאר כפי שהוא נקוב בשמו בסיכום.

פריט שנדרס נוחת <span dir="ltr">`draft`</span> כמו כל השאר, כך שמה ששלט מפסיק לשלוט עד
שתקדמו אותו שוב, והגרסה שהוחלפה נמצאת ביומן:
<span dir="ltr">`mycontext audit --item <id>`</span>.

<span dir="ltr">`--name <text>`</span> מתייקת את החבילה תחת שם שאתם בוחרים במקום זה
שהמניפסט שלה מצהיר עליו, וכך שתי חבילות שקוראות לעצמן אותו דבר נבדלות זו מזו **בשם**
...
מהן נדחה בשורה הראשונה שלו במקום להתמזג להיסטוריה שלכם. רשומה שהפעולה שבה לא מוכרת לבנייה
הזאת היא הפרש גרסאות ולא נזק: היא מוצבת בצד תחת
<span dir="ltr">`.audit/imported/unknown/`</span>, מילה במילה, ו**נספרת**, מפני שהסגר
שאיש אינו מסופר עליו הוא השמטה שקטה עם צעדים נוספים.

<span dir="ltr">`mycontext pack list`</span> נוקבת בכל חבילה שיובאה לכאן — הגרסה שלה, כמה
פריטים הביאה, מתי, והנתיב שממנו הגיעה. הרשומה הזאת היא גם מה
ש-<span dir="ltr">`mycontext review promote --all --pack <name>`</span> קוראת, וכך חבילה בת
ארבעים פריטים הופכת למעשה אנושי אחד שנעשה אחרי שהקורפוס גלוי ולא לפניו.
...
> <div dir="rtl">
>
> **מניפסט שמאמת אינו מחבר שנבדק.** הוא רשימה של גיבובי SHA-256 שנכתבה בידי מי שיצר את
> הארטיפקט, ולכן הוא תופס הורדה קטועה ועותק שהושחת ולא יותר מזה — אין חתימה ואין מולה דבר
> לבדוק. כל מה שחבילה מביאה נוחת <span dir="ltr">`draft`</span> בדיוק מהסיבה הזאת: הבדיקה
> שחשובה היא אדם שקורא.
>
> </div>

<div dir="rtl">

#### ממשק הרשת — `mycontext ui`

<span dir="ltr">`mycontext ui [--port N] [--no-open] [--idle-ms N]`</span> מגישה תצוגת דפדפן
לקריאה בלבד של סביבת העבודה הזאת על <span dir="ltr">`127.0.0.1`</span>. לולאה מקומית בלבד —
בקשה לעלות על כתובת אחרת היא סירוב בעלייה, לא אזהרה — ו-<span dir="ltr">`--no-open`</span>
מדפיסה את הכתובת במקום לפתוח דפדפן, וזה מה שאתה רוצה כששורת הפקודה אינה על המכונה שממנה אתה
קורא.

**שום מסלול <span dir="ltr">`/api`</span> של קריאה אינו מגיע לפונקציה שמשנה, והמנגנון הוא
הסיבה שאפשר להאמין לזה ולא רק להבטיח אותו.** בדיקה סטטית של גרף הייבוא
אוכפת את זה: היא טוענת על **הקבוצה המדויקת** של סמלי הכתיבה הכבולים בכל מקום תחת
<span dir="ltr">`src/ui/`</span> — לא שהקבוצה ריקה, אלא שהיא בדיוק השבעה שהבעלים אישר בשמם,
כך שכתיבה חדשה מפילה את הסוויטה באותה עוצמה שבה מפילה אותה כתיבה שנמחקה. הטענה הצרה היא
הנכונה, וההצרה מכוונת: כל מודול קריאה נותר ללא נגיעה, וכל כתיבה נקובה סמל-סמל ב-<span
dir="ltr">`test/ui/no-writes.test.ts`</span>. כתיבה שהממשק מרכיב אפשר **להעתיק ל-shell שלך**,
שם כללי ההרשאות של Bash שלך מתאימים למחרוזת הפקודה בדיוק כפי שהם עושים כשאתה מקליד אותה
בעצמך — או **להריץ אותה בשרת עצמו, מאחורי אישור**, וזה מסלול אחר עם שער אחר ובלי שום כלל Bash
לפניו. [הממשק יכול להריץ את מה שהוא מרכיב](#הממשק-יכול-להריץ-את-מה-שהוא-מרכיב) הוא המקום שבו
המסלול הזה מתועד, וכדאי לקרוא אותו לפני שמשאירים את הפורט הזה פתוח.

הכתיבות מעטות, וכל אחת מהן על מסלול שאפשר לנקוב בו. **בקשה שהשער מסרב לה מוסיפה רשומת
<span dir="ltr">`access`</span> אחת** ליומן הביקורת — <span dir="ltr">`op: ui-refused`</span>
— שנוקבת בבדיקה שסירבה ובערכי <span dir="ltr">`Host`</span> ו-<span dir="ltr">`Origin`</span>
שנשלחו; הטבעת nonce מוסיפה <span dir="ltr">`op: nonce-minted`</span>; והרצה דרך
<span dir="ltr">`POST /api/execute`</span> מוסיפה <span dir="ltr">`op: execute`</span> *לפני*
שהתהליך מתחיל ו-<span dir="ltr">`op: execute-done`</span> אחרי שהוא חוזר. קריאה שהוגשה אינה
משנה ולו בית אחד בקורפוס. לא האסימון ולא שום חלק ממנו נמצא באף אחת מהרשומות האלה.

**מודל האסימון.** כל הרצה מטביעה אסימון חדש, שהדף שולח בכותרת ייעודית בכל בקשה
ושאינו מגיע לשורת פקודה של תהליך; מה שמופיע בכתובת הוא **nonce חד-פעמי במקטע**,
שנסחר פעם אחת ב-<span dir="ltr">`POST /api/handoff`</span> תמורת האסימון עצמו ומת אחר כך.
דף שנטען מחדש כבר בזבז את ה-nonce שלו, ולכן אותו אסימון נקבע גם כעוגיית
<span dir="ltr">`HttpOnly`, `SameSite=Strict`</span>, והשער מקבל את הכותרת **או** את העוגייה
הזאת — בקשת <span dir="ltr">`/api`</span> שאינה נושאת אף אחת מהן מקבלת
<span dir="ltr">`401`</span> לפני שרץ מטפל כלשהו של מסלול, כאשר
<span dir="ltr">`POST /api/handoff`</span> ו-<span dir="ltr">`POST /api/nonce`</span> פטורים
מבדיקת האסימון בלבד, מפני שהם הדרך שבה קורא שאין לו אסימון משיג אחד.
<span dir="ltr">`Host`</span> ו-<span dir="ltr">`Origin`</span> עדיין נבדקים בשניהם.

**איות של לולאה מקומית מנותב מחדש, וכל דבר אחר מסורב במקום להיות מוגש.** בקשה שה-<span
dir="ltr">`Host`</span> שלה הוא <span dir="ltr">`localhost:<port>`</span>,
<span dir="ltr">`[::1]:<port>`</span> או כל <span dir="ltr">`127.x.x.x:<port>`</span> מקבלת
<span dir="ltr">`302`</span> אל <span dir="ltr">`http://127.0.0.1:<port>`</span> — הדפדפן נושא
את המקטע מעבר לקפיצה, ולכן קישור עם nonce שורד אותה — בעוד שכל <span dir="ltr">`Host`</span>
אחר, כולל שם לולאה מקומית על פורט **אחר**, מסורב. זה מה שסוגר את מארח ה-rebinding שאחרת היה
מקבל את הדף של השרת הזה.

**הוא בן חלוף, וכברירת מחדל הוא אינו חוזר.** השרת יוצא מעצמו אחרי שמונה שעות של חוסר פעילות
כברירת מחדל — <span dir="ltr">`--idle-ms`</span> מזיזה את זה לשני הכיוונים, עד תקרה של יממה
— כאשר "חוסר
פעילות" פירושו שאין בקשת <span dir="ltr">`/api`</span> שאינה זרם. **זרם פתוח אינו נחשב
פעילות, במכוון**: לשונית Watch שנשארה רצה אינה יכולה להחזיק את התהליך פתוח. מה שכן מחזיק
אותו פתוח הוא דף שאתה באמת רואה, שפועם פעם בדקה כל עוד הוא גלוי ומפסיק כשהחלון יורד לרקע.
כשהשרת אכן יוצא, הדף אומר זאת בבאנר ו**לעולם אינו מתחבר מחדש** — התחברות שקטה מחדש הייתה
אותו דמון שהתכנון הזה מסרב לו, בשם אחר.

**נקיבת פורט היא הדרך לבקש שרת שכן חוזר, ועד שלא עשיתם זאת לא קורה דבר.**
<span dir="ltr">`ui.port`</span> ב-<span dir="ltr">`.my_context/config.json`</span> היא ההצטרפות המפורשת: כשהמפתח חסר לא נכתב
שום קובץ, לא נבדק שום פורט ולא מופעל שום תהליך — תוסף אינו מריץ שרת רקע במכונה שלא ביקשה
אותו. קבעו אותו למספר שלם בין 1 ל-65535, וה-hook <span dir="ltr">`Stop`</span> — שכבר רץ בסוף כל תור של
העוזר — בודק את הפורט הזה, לכל היותר פעם בדקה, ומפעיל שרת מחליף כשאיש אינו עונה. **כך שרת
שמת חוזר בתוך דקה**, או בתוך תור אחד כשהתורים ארוכים מזה. המחליף מופעל עם <span dir="ltr">`--no-open`</span>:
שום hook אינו פותח דפדפן, לעולם. הערך <span dir="ltr">`0`</span> מסורב יחד עם הערכים שמחוץ לטווח ואינו נקרא
כ"כל פורט פנוי" — הקורא היחיד של המפתח הזה הוא hook, שאין לו למי למסור כתובת, ופורט שאיש לא
בחר הוא כתובת שאיש לא יכול לסמן. הדגל <span dir="ltr">`--port`</span> עדיין מנצח כשמעבירים אותו;
<span dir="ltr">`ui.port`</span> היא ברירת המחדל שמתחתיו, והיא מה שה-hook משתמש בו, כי אין לו שורת פקודה
לקרוא.

ההפעלה מחדש מוגבלת בקצב בנפרד מהבדיקה: **ניסיון הפעלה אחד לכל היותר בכל חמש דקות**, ואחרי
שלושה כישלונות רצופים סביבת העבודה נסוגה ואומרת זאת פעם אחת ב-stderr, נוקבת בפורט ובדרך
לכבות. הבדיקה נמשכת בכל מקרה, ולכן התור הראשון שבו שרת כן עונה מרים את הנסיגה מעצמו.

**<span dir="ltr">`ui.enabled`</span> הוא מתג הכיבוי, ומאז 27 באוגוסט 2026 הוא מכריע משהו.** הוא ויתור מפורש
— היעדרו פירושו כן, ורק <span dir="ltr">`false`</span> מפורש הוא סירוב — ועד לתאריך הזה הוא אומת, הוצג, ולא
נאכף על ידי דבר. עכשיו <span dir="ltr">`"enabled": false`</span> מסרב את <span dir="ltr">`mycontext ui`</span> לפני שנתפס
משהו, נוקב במפתח ובקובץ שהוא יושב בו, ומכבה את התחזוקה שלמעלה **בלי לבטל את הפורט שהגדרתם**.
שני המפתחות הם שני חצאים של שאלה אחת: <span dir="ltr">`ui.port`</span> אומרת *היכן*, <span dir="ltr">`ui.enabled`</span> אומר
*האם*, ואף אחד מהם אינו נגזר מהשני.

```json
{ "ui": { "port": 58888 } }
```

**לשונית שכבר הייתה פתוחה שורדת את ההפעלה מחדש.** השרת המחליף מכבד את האישורים שההתקנה הזאת
כבר הנפיקה: <span dir="ltr">`ui-sessions.json`</span> שומר <span dir="ltr">`sha256(token)`</span> של 64 האחרונים מהם, שלושים
יום כל אחד, תחת הספרייה הגלובלית ולא בתוך שום קורפוס. זהו digest ולעולם לא אסימון, ולכן
הקובץ אינו אישור כלל — גם בפלטפורמה שמתעלמת ממצב ההרשאות שאיתו נכתב. דף שנשאר פתוח לאורך
הפעלה מחדש על אותו פורט מתאושש אפוא ברענון פשוט, במקום להינעל עד שתדביקו nonce טרי מהמסוף.
המנגנון הזה אינו חדש — זהו <span dir="ltr">`ui-sessions.json`</span> מתנהג כפי שנבנה, עכשיו עם קורא שני.

**המסכים** מקובצים בסרגל בארבע קבוצות, ו-<span dir="ltr">`mycontext ui`</span> עצמה היא
הרשימה הכנה — הם עדיין נוספים, ולכן מנייה כאן הייתה מתיישנת לפני שהייתה מועילה:

- **הזרקה** — התצוגה המקדימה של תחילת הסשן (עם <span dir="ltr">`seen`</span> לכל סשן, כך
  שהיא עונה מה סשן **מסוים** היה מקבל), מפת כיסוי ה-scope, פערי כיסוי כמסך משלהם, סימולטור
  התקציב, ומה מוזרק ברגע זה;
- **ראיות** — Watch ו-Ask (להלן), <span dir="ltr">`doctor`</span>,
  <span dir="ltr">`decay`</span>, גרף הקשרים, ו-<span dir="ltr">`status`</span>;
- **שינוי** — Work, ה-Composer, Configure, לכידה, נהלים, פורטים וחבילות;
- **קריאה** — מסך Library, שמונה כל מסמך וכל מדריך לפי כותרת ופותח אחד בלשונית משלו, ומסך
  Learn המקושר לקורפוס הזה.

שתי טענות על המסכים האלה נושאות את התנאי שלהן באותו משפט, כי בלעדיו הן שקריות. התצוגה
המקדימה שווה לבחירה של ה-hook **כשנותנים לה את אותו סשן**; האפשרות הקרה עונה על שאלה אחרת
ומסומנת ככזאת. וסימולטור התקציב מדווח מה היה נשפך **תחת התקציבים המועמדים, עבור האירוע והסשן
שבחרת** — הוא מריץ את הבורר האמיתי ולא מעריך אותו.

##### Work, ה-Composer ו-Configure

**Work** הוא תור הרוויזיות הממתינות. הוא מציג כל הצעה כ**הפרש לכל שדה מול הטקסט שבתוקף**,
רשומה אחת לכל שדה שההצעה באמת נוגעת בו, ומסמן התיישנות **לכל שדה בנפרד** — שדה שהטקסט
שמתחתיו זז מאז שההצעה הועמדה מסומן כמיושן במקום להיות מצויר בשקט כאילו הוא עדיין חל. הצעת
כותרת לצד הצעת גוף מיושנת עדיין קריאה, ועדיין ניתנת לקידום.

האישור הוא **שורה מולחנת אחת**, והעתקתה היא כל האינטראקציה:
...
**הממשק אינו מקדם.** אין נקודת קצה של הכרעה שהוא יכול לקרוא לה: ההפרש הוא היכולת שהדפדפן
מוסיף, וההחלטה נשארת במקום שבו כל החלטה אחרת במוצר הזה נשארת.
<span dir="ltr">`--revision`</span> נמצא בשורה המולחנת במכוון — בלעדיו שורת הפקודה מסרבת
ברגע שלפריט יש יותר מרוויזיה ממתינה אחת — וכלל המניעה
<span dir="ltr">`Bash(mycontext review promote-revision *)`</span> מתאים לצורה הארוכה באותה
מידה.

**ה-Composer** בונה פקודות מבוררים אמיתיים — הפריטים, התצורה והתורים שיש לסביבת העבודה הזאת
בפועל, ולא תיבת טקסט. הקטלוג שלו מפצל כל פקודה לשניים, והפיצול מבוטא כצורה ולא כבדיקה:
פקודת **קריאה** נוקבת במסך או בנקודת קצה והממשק מריץ אותה; פקודת **כתיבה** אינה נוקבת
באף אחד מהם, ולכן אין לממשק מה להריץ גם אם היה רוצה. כתיבה מולחנת מוצגת עם ההערה *"This is a
write. Run it in your own shell."* המנגנון נאמר באותו משפט עם הטענה: כללי ההרשאות של Bash
שלך מתאימים למחרוזות פקודה, ופקודה מודבקת נשארת מחרוזת אחת — הארגומנטים מצוטטים לשורה אחת
ולא נמסרים ל-shell כאן. במקום שבו ארגומנט מכיל תחביר של החלפה ב-shell ההעתקה **מסורבת
לחלוטין** ולא מצוטטת, כי מרכאות כפולות אינן מנטרלות <span dir="ltr">`$(…)`</span>, ופקודה
שנראית בטוחה גרועה מפקודה שאי אפשר להעתיק.

**Configure** קורא את <span dir="ltr">`.my_context/config.json`</span> ומאפשר לך לבנות עריכה
מועמדת שלו. שלוש תכונות חשובות, וכל אחת מהן מדויקת:

- **הסירובים הם מילותיו של <span dir="ltr">`resolveConfig`</span> עצמו** — נקודת הקצה מעלה
  את ההודעה ששורת הפקודה הייתה זורקת, מילה במילה ולא בניסוח מחדש, ובדיקה משווה את השתיים מול
  הזריקה האמיתית;
- **התצוגה המקדימה מחושבת בידי הבורר האמיתי וכללי ההזרקה האמיתיים מול הקורפוס שלך** —
  <span dir="ltr">`governing`</span> הוא מה שמתחיל ומפסיק להיות מוזרק,
  <span dir="ltr">`agentEdits`</span> ו-<span dir="ltr">`scopePolicy`</span> נוקבים בשמות
  הפריטים המושפעים ולא רק סופרים אותם, ו-<span dir="ltr">`selection`</span> הוא הבורר עצמו
  שרץ פעמיים על אותו הקשר;
- **הממשק לעולם אינו כותב את הקובץ.** הוא מלחין עבורך את ה-JSON כדי שתדביק אותו בעורך שלך —
  אותו כלל שה-hook של המניעה אומר לסוכנים (*"changes to
  <span dir="ltr">`.my_context/config.json`</span> are the user's to make — ask, do not
  edit"*), מוחל על הממשק עצמו.
...
> <div dir="rtl">
>
> **מסך Configure עדיין אינו קורא לתצוגה המקדימה.**
> <span dir="ltr">`POST /api/config/check`</span> ו-<span dir="ltr">`POST /api/config/preview`</span>
> חיים, בדוקים וניתנים להשגה, אבל המסך קורא כרגע רק
> <span dir="ltr">`GET /api/config`</span>, והלוח "מה משתנה" שלו מצויר ריק ומושבת. התצוגה
> המקדימה שלמעלה היא תיאור נכון של **נקודת הקצה**; היא עדיין אינה תיאור של המסך. קרא את
> המסך כמציג תצורה עם מלחין צמוד, עד שזה ינחת.
>
> </div>
...
<!-- `ui/watch-model.ts` · `import { AUDIT_KINDS, type AuditKind, type AuditRecord } from '../core/audit.ts';` · ~3 --> שורת הזרקה נושאת את **מספר** הפריטים שאותה הזרקה
...
<span dir="ltr">`mycontext audit --item <id> --json`</span> וכלי ה-MCP
<span dir="ltr">`audit_log`</span> הם המקום שבו המזהים נמצאים. רשומות הזרקה שנכתבו לפני
שספירת הטוקנים הייתה קיימת מצוירות כ**לא נרשמו, לעולם לא כאפס** — היעדר אינו מדידה.

**לשונית Watch גלויה כן מחזיקה את השרת בחיים, והזרם אינו מה שעושה זאת.** הזרם במכוון אינו
נחשב פעילות; הפעימה של הדף פעם בדקה כן, והיא נפסקת כשהלשונית מפסיקה להיות גלויה. כך שלשונית
שאתה מסתכל בה מחזיקה את השרת פתוח, ולשונית ברקע לא. כשהשרת אכן יוצא תחת זרם פתוח, הזרם מדווח
שהוא סירב להמשיך, הבקשה הכושלת הבאה מעלה את הבאנר "השרת יצא", ושום דבר אינו מתחבר מחדש.

**Ask** בונה שאילתות מובנות מעל הקורפוס ומעל היסטוריית הביקורת. בשתי לשוניות הסינון הוא מציג
את ה-SQL **המדויק** ואת הפרמטרים שכל תשובה הריצה; הדוחות המוכנים שלצידן אינם נושאים את
ה-SQL שלהם, והכרטיס מוסתר במקום להישאר ריק. שורות הקורפוס נקראות **כפי שה-hooks השאירו
אותן** — Ask לעולם אינו בונה מחדש את האינדקס, ו-<span dir="ltr">`updated_at`</span> הוא זמן
הכתיבה לאינדקס ולא חותמת תוכן, ולכן הוא מתארך את השורה ולא את הטענה שבתוכה.

**תשובות הביקורת מגיעות מההיטל דרך דלת קריאה בלבד — שום דבר אינו מעדכן אותו, מפני שסנכרון
הוא כתיבה וזה משטח קריאה.** מכאן שלוש תוצאות ולא שתיים, ואסור למזג אותן: היטל שהוא עדכני
עונה כרגיל; היטל שהוא **מפגר, מסועף או פגום הוא סירוב** שנושא את מצבו ומצביע על
<span dir="ltr">`mycontext audit`</span> כדי לבנות אותו מחדש, מפני שתשובה מהיסטוריה חלקית
הייתה מציגה אותה כשלמה; והיטל ש**מעולם לא נבנה אינו סירוב כלל** — הוא עונה ריק ואומר שההיטל
נעדר.

נקודת הקצה מקבלת ארבעה דוחות מוכנים — <span dir="ltr">`ops`</span>,
<span dir="ltr">`items`</span>, <span dir="ltr">`sessions`</span> ו-<span dir="ltr">`tasks`</span>.
האחרון מצרף פריטי משימה למה שיומן הביקורת רושם שקרה להם, והוא **ניתן להשגה דרך ה-API בלבד**:
הכפתורים במסך Ask היום הם דוח הפעולות, דוח הסשנים, ודוח הפריטים בשני התפקידים
<span dir="ltr">`spilled`</span> ו-<span dir="ltr">`injected`</span>.

##### גשר שורת הסטטוס (בבחירה)

הממשק אינו יכול למדוד את ההקשר של Claude בעצמו. Claude Code מוסר את המספר הזה ל*פקודת שורת
סטטוס*, וזה הגשר אליה.

<span dir="ltr">`mycontext statusline install`</span> מדפיסה את הגדרת
<span dir="ltr">`statusLine`</span> שיש לך עכשיו ואת מה שיחליף אותה בדיוק, ו**אינה כותבת דבר
עד שתריץ אותה שוב עם <span dir="ltr">`--yes`</span>**. <span dir="ltr">`--settings <path>`</span>
בוחרת את הקובץ, וברירת המחדל היא זה של Claude Code. **התקנת mycontext לעולם אינה נוגעת בשורת
הסטטוס שלך; רק הפקודה הזאת, שאתה מריץ, נוגעת.**

ההסרה הפיכה, והיקפה נאמר במקום להיות מובטח.
<span dir="ltr">`mycontext statusline uninstall --yes`</span> מחזירה את **הקובץ המקורי כולו**,
בית אחר בית — כולל הזחה, סדר מפתחות ושורה מסיימת — **כאשר הקובץ לא השתנה מאז ההתקנה**. אם כן
השתנה, רק מפתח <span dir="ltr">`statusLine`</span> מוחזר ושאר הקובץ נשאר כפי שהוא עכשיו, כי
החזרת הקובץ כולו הייתה מוחקת את השינוי שלך; הפקודה אומרת במה משתיהן בחרה. אם לא היה קובץ
הגדרות לפני ההתקנה, ההסרה מוחקת את זה שהיא יצרה. ואם ה-<span dir="ltr">`statusLine`</span>
שבקובץ כבר אינו הגשר הזה, היא **מסרבת לחלוטין** במקום לדרוס הגדרה שקבעת אחר כך.

לאחר ההתקנה <span dir="ltr">`mycontext statusline`</span> רצה בכל הודעה של העוזר — ובנוסף
ברענון של שישים שניות, כך שהנתון אינו מתיישן בזמן שסשן יושב בחוסר מעש. היא מדפיסה את המודל,
את ההקשר שבשימוש, וכמה ממנו הגיע מידע הפרויקט, ומעתיקה את המטען של Claude Code לקובץ אחד לכל
סשן.

**לנתון ההקשר יש בדיוק שלושה מצבים, ואף אחד משני השליליים אינו מוצג כאפס.** אחרי כיווץ הוא
קורא **"not yet known"** עד הקריאה הבאה ל-API, מפני שהמטען נושא חלון בלי שימוש בתוכו עדיין.
כשהמטען אינו נושא <span dir="ltr">`context_window`</span> שמיש כלל — Claude Code ישן יותר, או
כל צורה שהקוד הזה אינו מזהה — הוא קורא **"unknown"**. אחרת זהו המספר האמיתי, מחושב על הקלט
בלבד מול גודל החלון, בהתאמה למה ש-Claude Code עצמו מציג. החלק של "מידע הפרויקט" מחושב מיומן
הביקורת, שהוא מקומי למכונה הזאת, ולכן הוא מכסה את הסשנים של המכונה הזאת בלבד.
...
> <div dir="rtl">
>
> **היום המספר הזה מגיע לשורת הסטטוס של Claude Code ולא לממשק הרשת.** קובץ ההעתקה נכתב
> ו-<span dir="ltr">`GET /api/watch/context`</span> מגיש אותו, אבל שום מסך אינו קורא את
> המסלול הזה עדיין: רצועת המצב של הממשק קבועה כרגע לומר שלא מותקן גשר, בין אם מותקן ובין אם
> לא. התקן את הגשר בשביל שורת הסטטוס במסוף שלך; אל תתקין אותו בציפייה שתצוגת הדפדפן תשתנה.
>
> </div>
...
עשרים ושמונה כלים, מוגשים מעל stdio על ידי `src/mcp/server.ts`. המודל מגיע אליהם בלי shell, וכל
...
| `list_items` | מפקד הקורפוס לפי קטגוריה — ספירות, לא פריטים. עם <span dir="ltr">`category`</span>, מניית הפריטים של אותה קטגוריה, וזה המקום היחיד שבו יש חפיפה עם <span dir="ltr">`query_items({type})`</span> |
...
| `mycontext_help` | קריאת הדרכה על נושא אחד: <span dir="ltr">categories, scope, capture, workflow, tools, slash</span>. אלה כל הנושאים ש-<span dir="ltr">`mycontext help`</span> מגישה חוץ מ-`cli` — מנוע משום שהרישום של שורת הפקודה עצמה לעולם אינו נטען על המשטח הזה, כך שהנושא היה מציג מקטע פקודות שאינו נוקב באף פקודה — והאנומרציה בסכמה נגזרת מהרשימה ולא נשמרת ביד <!-- `core/teach.ts` · `export const MCP_HELP_TOPICS: HelpTopic[] = HELP_TOPICS.filter((t) => t !== 'cli');` · ~36 --> |
...
| `ready` | מניית עבודה פתוחה שכל ה-<span dir="ltr">`needs`</span> שלה הושלמו, בסדר עדיפות יורד; עבודה מוחזקת נספרת, ועם <span dir="ltr">`held`</span> גם מוצגת עם הסיבה שלה |
| `doctor` | הרצת בדיקת התקינות של הקורפוס: רעננות אינדקס, יתומים, סחיפה, גלובים מתים, הרשאות, מזהי סשן |
| `decay_report` | מניית פריטים נורמטיביים פעילים שלא הוזרקו בחלון הסשנים האחרון, הקרים ראשונים — סימן לבדוק, לעולם לא פסק דין שמשהו אינו בשימוש |
| `list_ingest_sessions` | מניית כל סשן קליטה, ההתקדמות שלו לפי עוגן, וכל מועמד שנדחה |
| `create_lesson` | רישום לקח, או גזירה מחדש מלקח קיים לפי מזהה, וקבלה בחזרה של בקשת גזירת הכללים. מחתים תמיד <span dir="ltr">`origin: "agent"`</span> — קריאת כלי היא מבצע לא-אנושי מעצם בנייתה, כך שאין ארגומנט הטוען אחרת |
| `stage_rule_candidates` | הצבת מועמדי כלל שנגזרו מלקח, לאישור או דחייה של אדם. לא כותב דבר לקורפוס — רק <span dir="ltr">`mycontext lesson-accept`</span>, פקודת אדם, יוצר את הכלל |
| `preview_pack_import` | עם <span dir="ltr">`path`</span>, תצוגה מקדימה של ייבוא ארטיפקט — אותו דוח התנגשויות ש-<span dir="ltr">`mycontext pack import`</span> מדפיס לפני האישור הראשון שלו, ועוד הפקודה שאדם מריץ אחר כך. בלי <span dir="ltr">`path`</span>, מניית החבילות שכבר יובאו לכאן. לעולם אינו מייבא בעצמו |
| `status_report` | לוח המחוונים המורכב: ספירות, תור סקירה, התקדמות קליטה, דעיכה ותקינות |
| `list_todos` | מניית תיבת הדואר הנכנס — פריטים שנלכדו כ-<span dir="ltr">`todo`</span> — ומה המשמעות של הדרג שלהם |
| `ask_handover` | בקשת פתק ה-handover **עכשיו**, בכל תפוסה שחלון ההקשר מחזיק כרגע, במקום להמתין ל-<span dir="ltr">`handover.thresholdPercent`</span>. הוא יורה את אותה בקשה ש-<span dir="ltr">`Stop`</span> יורה, כך שדבר במורד הזרם אינו משתנה: ההשוואה שמכריעה אם נענתה היא אותה השוואה. הוא מסרב במקום לנחש — כששרת זה לא הופעל על ידי Claude Code ולכן אינו עונה לשום סשן, כשאינו יכול לקרוא כמה החלון מלא (שום אחוז אינו מומצא), וכששרשראות עבודה של הסשן עדיין רצות, ואז הוא נוקב בשמן כדי שאדם יבחר בין להמתין לבין להמשיך עם <span dir="ltr">`anyway`</span> |
| `list_rules` | קריאת מאגר כללי המוצר שנשלח בתוך החבילה: כל רשומה שבתוקף במרחב העבודה הזה, או רשומה אחת במלואה עם <span dir="ltr">`id`</span>. זהו חצי-הכלי של <span dir="ltr">`mycontext rules list`</span> / <span dir="ltr">`mycontext rules show`</span>, והוא קיים משום ש-<span dir="ltr">`missedDoorLine`</span> — טקסט שנכתב עבור המודל — מורה לו לבדוק את המאגר כשדלת לא הספיקה למסור, וסוכן בלי גישה למעטפת לא יכול היה לעשות זאת |
| `verify_rules` | תשובה לשאלה אם מאגר הכללים עדיין תואם לסכומי הביקורת שנשלחו איתו, תוך נקיבה בכל רשומה שאינה תואמת. אין בו <span dir="ltr">`restore`</span>: החזרת בתים היא כתיבה, היא חיה ב-<span dir="ltr">`mycontext rules verify --restore`</span>, והסירוב נוקב בה במקום להתעלם מהארגומנט |
| `read_procedure` | חצי הקריאה של <span dir="ltr">`mycontext procedure`</span>: מניית כל נוהל לפי שלב, הצגת אחד עם הסימונים שלו, או סימון/ביטול סימון של צעד. <span dir="ltr">`activate`</span> ו-<span dir="ltr">`done`</span> אינם כאן — אלה מחתימים מקור אנושי בקוד ונשארים מעשה אנושי, אותה חלוקה שבין <span dir="ltr">`list_drafts`</span> ל-<span dir="ltr">`review promote`</span> |
...
| <span dir="ltr">`--request "<text>"`</span> | המילים ש**אדם** כתב כשביקש את הפריט הזה, מועתקות מילה במילה — האיות שלו, הפיסוק שלו, שבירות השורה שלו. זהו תיעוד בלבד: הוא לעולם אינו מוזרק לשיחה, אינו חלק ממה שהתקציר נמדד מולו, והוא מחוץ לגיבוב של הפריט — ולכן רישום בקשה אינו הופך דבר למיושן, ומחיקתה מחזירה את הקובץ בית אחר בית. **אל** תנסחו מחדש, אל תסדרו ואל תתרגמו, ולעולם אל תכתבו בקשה ממה שהפריט אומר: בקשה חסרה אומרת באמת שאיש לא רשם אחת, ובקשה מומצאת היא ציטוט של דבר שמעולם לא נאמר, ששום דבר אחר כך לא יוכל להבחין בינו לבין האמיתי. ב-<span dir="ltr">`edit`</span> היא רושמת בקשה על פריט שאין לו **אף אחת**, <span dir="ltr">`--request=`</span> מוחקת אחת, ופריט שכבר רושם בקשה — מסורב: תיקון הוא בקשה חדשה, ולא בקשה ישנה משופרת. אותו שדה הוא הארגומנט <span dir="ltr">`request`</span> של <span dir="ltr">`create_item`</span> ו-<span dir="ltr">`update_item`</span> <!-- `core/validate.ts` · `export function validateRequest(request: string): void {` · ~578 --> | <span dir="ltr">`add`, `edit`</span> |
...
| <span dir="ltr">`--held`</span> | להציג את המשימות שהדוח אחרת רק **סופר**: אלה שה-`needs` שלהן אינם כולם גמורים. אין <span dir="ltr">`--all`</span> לצידו — "מוכן" הוא תשובה נגזרת על עבודה פתוחה, ודגל שהיה מרחיב אותה לעבודה גמורה היה שואל שאלה אחרת תחת אותו שם <!-- `core/command-flags.ts` · `allowed: ['plan', 'limit', 'held', 'questions', ...DETAIL_FLAGS], values: ['plan', 'limit'],` · ~425 --> | `ready` |
...
| דגל | מה הוא עושה | היכן הוא עובד |
|---|---|---|
| <span dir="ltr">`--port <n>`</span> | פורט ה-loopback להגשה. הכתובת אינה דגל כלל: היא <span dir="ltr">`127.0.0.1`</span>, ובקשה להאזין לכל כתובת אחרת מסורבת בהפעלה ולא זוכה לאזהרה. בהיעדרו הפורט נופל אל <span dir="ltr">`ui.port`</span> ב-<span dir="ltr">`.my_context/config.json`</span>; כשהוא ניתן, הדגל מנצח אותה | `ui` |
...
| <span dir="ltr">`--nonce`</span> | לייצר nonce מסירה חד-פעמי טרי משרת ש**כבר רץ**, ולהדפיס את הכתובת שלו, במקום להפעיל שרת שני. זהו מתג, הוא אינו מקבל ערך, והוא סותר את שלושת האחרים — העברתו לצידם מסורבת ולא נזרקת בשקט <!-- `core/command-flags.ts` · `ui: { allowed: ['port', 'no-open', 'idle-ms', 'nonce'], values: ['port', 'idle-ms'] },` · ~347 --> | `ui` |
...
#### תיבת הנכנס — `todo` ו-`note`

כל קטגוריה אחרת בקטלוג מצפה שכבר תדעו איזה סוג של ידע יש לכם. ברגע שמחשבה באמת מגיעה, בדרך
כלל אינכם יודעים — והמחיר של לדרוש ממישהו לסווג מחשבה לפני שהוא רושם אותה הוא המחשבות שלעולם
אינן נרשמות. `todo` ו-`note` קיימות כדי שהלכידה לא תעלה דבר: `todo` הוא משהו לבנות או לתקן
מאוחר יותר, ו-`note` הוא כל דבר שצץ ואסור שיאבד.

**שתיהן יושבות על דרג הנימוק, וזה מה שהופך אותן לתיבת נכנס ולא לדרך נוספת לשלוט בפרויקט.**
`todo` לעולם אינו מוזרק לסשן במלואו, ואינדקס הסשן מצמצם את כל הקטגוריה לספירה חשופה במקום
לנקוב באיזשהו פריט שלה — כך שמאה לכידות עולות לסשן מספר אחד, לא מאה שורות. גם שום דבר אינו כופה על
לכידה להיות <span dir="ltr">`draft`</span>, ולכן סוכן יכול לכתוב אחת בלי להעמיד לפניכם משהו
להכריע בו: אף אחת מהשתיים אינה נכנסת לתור הסקירה, מפני
ש-<span dir="ltr">`mycontext review`</span> שואל מה צריך לשלוט בפרויקט הזה, ותיבת נכנס אינה
חלק מהשאלה הזאת.

יש להן משטחי רשימה משלהן מאותה סיבה. <span dir="ltr">`mycontext todo`</span> מדפיסה את תיבת
הנכנס בסדר המזהים שכל רשימה אחרת משתמשת בו, עם <span dir="ltr">`--tag`</span>,
<span dir="ltr">`--all`</span> ו-<span dir="ltr">`--limit`</span>; לכידות שפרשו מוסתרות
ונספרות ולא נזרקות. הערות חוזרות מ-<span dir="ltr">`mycontext search --type note`</span>.

**הדרך החוצה היא <span dir="ltr">`mycontext inbox-promote <id> --to <category>`</span>.** היא
יוצרת פריט אמיתי מהקטגוריה שנקבתם בה, מעבירה את הכותרת, הגוף והתגיות, כותבת על הפריט החדש
קשר <span dir="ltr">`derived_from`</span> שמצביע חזרה על הלכידה, ומפרישה את המקור
כ-<span dir="ltr">`deprecated`</span> במקום למחוק אותו — הקובץ, הגוף שלו, התצפיות והקשרים
שלו כולם נשארים, והוא נשאר בר-חיפוש ונספר. ה-<span dir="ltr">`origin`</span> של הלכידה
**נישא הלאה ולא מוטבע מחדש**, ולכן קידום אינו הלבנה: הערה שסוכן כתב עדיין נוחתת כטיוטה
כשהיא הופכת לפריט נורמטיבי, בדיוק כפי שלכידה ישירה של סוכן הייתה נוחתת.
...
> <div dir="rtl">
>
> **<span dir="ltr">`inbox-promote`</span> ו-<span dir="ltr">`review promote`</span> הן
> פקודות שונות ועושות עבודות שונות.** <span dir="ltr">`inbox-promote`</span> מוציאה לכידה
> מתיבת הנכנס אל קטגוריה אמיתית. <span dir="ltr">`review promote`</span> מעבירה טיוטה
> לשליטה. אף אחת מהן אינה איות ארוך יותר של השנייה.
>
> </div>

<div dir="rtl">

הדוגמה המעובדת הברורה ביותר היא באג. לכדו אותו ברגע שנתקלתם בו, לפני שהבנתם אותו —
<span dir="ltr">`mycontext add note "The sandbox drops connections on Fridays" --tags bug`</span>.
ברגע שאתם כן מבינים אותו, הוא <span dir="ltr">`known_issue`</span>,
ו-<span dir="ltr">`mycontext inbox-promote NOTE-… --to known_issue`</span> היא הצעד האחד
שביניהם.
...
> <div dir="rtl">
>
> **מחיר ידוע, שנרשם במקום להיפתר.** כל פריט אחר בקורפוס נכון עד שהוא מוחלף. `todo` נכון עד
> שמישהו עושה אותו, ולקורפוס אין שום דרך ללמוד שזה קרה — שום דבר אינו צופה בקומיטים שלכם,
> ואין פקודה שמסמנת אחד כבוצע. קידום, או הפרשה ביד, הוא ההקלה; הוא אינו תיקון, ותיבת נכנס
> שלא מטפלים בה תתמלא בשקט בעבודה שכבר הסתיימה.
>
> </div>

<div dir="rtl">

#### מחזור החיים של `procedure`

`procedure` היא הקטגוריה האחת בקטלוג שיש לה מחזור חיים,
ו-<span dir="ltr">`mycontext procedure [list|show|activate|done|step]`</span> היא המקום שבו
מחזור החיים הזה חי. שום דבר לא נוסף ל-<span dir="ltr">`status`</span> כדי לשאת אותו — השלבים
הם הסטטוסים שכבר נשלחו:

| שלב | מה הוא על הדיסק | מה הוא מזריק |
|---|---|---|
| proposed | <span dir="ltr">`status: draft`</span> | כלום |
| ready | <span dir="ltr">`status: draft`</span>, ובנוסף התגית <span dir="ltr">`ready`</span> | כלום, גם לא שורת אינדקס |
| active | <span dir="ltr">`status: active`</span> **וגם** <span dir="ltr">`always: true`</span> | הגוש המלא, בכל תחילת סשן |
| done | <span dir="ltr">`status: deprecated`</span> | כלום; נספר ב"פרשו" |
| abandoned | <span dir="ltr">`status: superseded`</span> | כלום |

**הפעלה היא שתי כתיבות, ו-<span dir="ltr">`mycontext procedure activate`</span> עושה את
שתיהן** — וכדאי לומר זאת במפורש, כי זה הדבר שקורא אחרת מבין לא נכון.
<span dir="ltr">`status: active`</span> רק הופך פריט לכשיר להיבחר בכלל;
<span dir="ltr">`always: true`</span> הוא מה שמוסר אותו במלואו בכל תחילת סשן ולא כשורת אינדקס
אחת. אלה תכונות שונות, ונוהל שיש לו אחת בלי השנייה אינו רץ. הפקודה מדפיסה את שני המעברים
לפני שהיא מבקשת מכם לאשר.

**<span dir="ltr">`done`</span> מפרישה את הפריט ל-<span dir="ltr">`deprecated`</span> ולא
ל-<span dir="ltr">`validated`</span>, וההבדל הוא כל העניין.** נוהל
<span dir="ltr">`validated`</span> היה ממשיך לשלוט, ונוהל שכבר בוצע הוא בדיוק הדבר שצריך
להפסיק להגיע. שום דבר אינו נמחק: הקובץ, הגוף שלו והצעדים שלו נשארים, והוא נספר במניין
שפרשו של באנר הסשן. הפקודה גם אינה מסיקה דבר במקומכם — היא מדפיסה את ההתקדמות שהיא רואה
ומסיימת את הנוהל מפני שאמרתם, בין אם כל צעד מסומן ובין אם לא.

**מה שנוהל <span dir="ltr">`ready`</span> עושה היום הוא כלום.** הוא טיוטה שנושאת תגית: הוא
אינו מוזרק, והוא גם אינו נקוב באינדקס, ולכן המודל אינו לומד שהוא קיים עד שמישהו מפעיל אותו.
זו שאלה פתוחה במכוון ולא פליטה — "שורת אינדקס בלבד" אינו מצב שקיים בבנייה הזאת —
ו-<span dir="ltr">`mycontext procedure list`</span>, שמקבצת כל נוהל לפי שלב, היא המקום שבו
נוהל <span dir="ltr">`ready`</span> גלוי בינתיים.

##### `## Steps`, והיכן ההתקדמות נשמרת

הצעדים של נוהל הם תכונה של פורמט הקובץ: מקטע <span dir="ltr">`## Steps`</span> של שורות
<span dir="ltr">`- [ ]`</span>, ב-Markdown, כמו <span dir="ltr">`## Relations`</span>
ו-<span dir="ltr">`## Observations`</span> לצידו. הם **ניתנים ליצירה בלבד** —
<span dir="ltr">`mycontext add procedure "…" --step "…" --step "…"`</span> כותבת אותם, ואין
דגל שעורך אחד מהם אחר כך. תיקון צעד פירושו עריכת ה-Markdown והרצת
<span dir="ltr">`mycontext repair`</span> כדי להטביע מחדש את ה-checksum. פריט בלי צעדים אינו
משתנה ולו בבית אחד, ולכן שום דבר בקורפוס קיים אינו חייב להגר.

**ההתקדמות לעולם אינה נכנסת לקורפוס.**
<span dir="ltr">`mycontext procedure step <id> <n>`</span> כותבת רשומת
<span dir="ltr">`progress`</span> אחת ליומן הביקורת ואינה נוגעת בשום פריט: אחרי סימון, הקובץ,
הגוף שלו, הצעדים שלו וה-`checksum` שלו זהים בית-בית למה שהיו, והפקודה אומרת זאת בשורה שהיא
מחזירה. סימוני <span dir="ltr">`- [x]`</span> ב-<span dir="ltr">`mycontext procedure show`</span>
נספרים מהרשומות האלה בזמן ההצגה ו**מצוירים מעל הרשימה השמורה** — ה-Markdown שעל הדיסק עדיין
קורא <span dir="ltr">`- [ ]`</span>, והוא נשאר הרישום. המגבלה שייכת לאותה נשימה: ההתקדמות
נרשמת **לכל סביבת עבודה, לא לכל סשן**, ולכן שני מסופים שעובדים על אותה סביבת עבודה חולקים
מערכת סימונים אחת. כל משטח שמציג התקדמות אומר זאת.

נוהל מבוצע **פעם אחת**, וזו בדיוק הסיבה שהוא הקטגוריה שנושאת מחזור חיים: הוא מפסיק להיות
מוזרק כשהוא הסתיים. `runbook` לעולם אינו מסתיים, ולכן אין לו ממה להפסיק להיות — ולכן
<span dir="ltr">`mycontext procedure activate`</span> מסרבת ל-`runbook` בשמו במקום להתייחס
לזה כאל יכולת שטרם הגיעה. הבחירה בין השתיים מכוסה במקום שבו הקטלוג מתאר אותן, למעלה.
...
**שתי קטגוריות יכולות לחלוק תחילית מזהה, ושום דבר אינו אומר זאת.** שני שמות שחולקים את שש
האותיות והספרות הראשונות שלהם — <span dir="ltr">`standard_ops`</span>
ו-<span dir="ltr">`standardize`</span> — מגיעים לאותה קידומת, ותחילית `prefix` מפורשת
מתנגשת באותה שתיקה: תנו ל-`rule` ול-`invariant` שתיהן
<span dir="ltr">`{"prefix": "POLICY"}`</span> והפריט השני שנטבע הוא
<span dir="ltr">`POLICY-…-2`</span>, בלי שגיאה, בלי אזהרה ובלי ממצא מ-`doctor`. המזהים אז
מפסיקים לומר לקורא מאיזו קטגוריה פריט הוא, וזה רוב מה שתחילית מזהה קיימת בשבילו. התיקון
הוא לסרב להתנגשות בטעינת התצורה, והוא לא נבנה — ולכן קבעו `prefix` במפורש, ובדקו אותו מול
התחיליות שכבר בשימוש. **`prefix`
...
ל-`create_item` — המשטח הזה מקבל כל סוג מופעל, ולכן הקטגוריה שמישה לגמרי ומה שחסר הוא
המשטח היחיד שנוצר מראש. סגירת זה משמעה ייצור הפקודות מהתצורה של הפרויקט עצמו, וזו שאלה של
אריזת התוסף ולא של תצורה, והיא לא נבנתה.
...
לאותו קטלוג — עשרים קטגוריות אז, עשרים ותשע היום — ושם שני לאותה קבוצה הוא דבר שצריך
להסביר לקורא שאין לו משמעות.
...
והסימפטום היחיד היה פריטים שנעדרו בשקט מהסשנים. רמה אחת למעלה התשובה שונה, ובכוונה: מפתח
עליון שהקונפיגורציה אינה מכירה (<span dir="ltr">`"budget"`</span>,
<span dir="ltr">`"watched_docs"`</span>) **מדולג בשמו ושאר הקונפיגורציה נטענת**, מפני שמפתח
עליון הוא גם הדרך שבה my_context חדש יותר מגיע, ודחיית הקובץ כולו בגללו הייתה הופכת את "הבנייה
הזאת ישנה מהקונפיגורציה שלך" ל"התוסף אינו עושה דבר". הדילוג לעולם אינו שקט —
<span dir="ltr">`mycontext doctor`</span> מעלה אזהרת <span dir="ltr">`config_key_skipped`</span>
שנוקבת בכל מפתח שלא נקרא, ובקבוצה התקינה.
...
### `dispatchGate` — לדרוש משיגור שינקוב בפריט שלמענו הוא נעשה
...
**כבוי אלא אם הדלקתם אותו**, והיעדרו פירושו כבוי: התקנת my_context לעולם אינה מתחילה לסרב
לפרויקט את תת-הסוכנים שהוא תמיד שיגר. משתדליקים אותו, ה-hook
<span dir="ltr">`PreToolUse`</span> קורא את ה-<span dir="ltr">`prompt`</span> של כל שיגור
<span dir="ltr">`Agent`</span> ומחפש בו אסימון בצורת מזהה של הקורפוס הזה — רצף אותיות גדולות
לפני המקף הראשון, <span dir="ltr">`TASK-<slug>`</span> וכדומה. הוא קורא את
<span dir="ltr">`prompt`</span> ולא את <span dir="ltr">`description`</span>, מפני שלתווית בת
שלוש עד חמש מילים אין מקום לשאת מזהה באופן אמין.

**צורה אינה מספיקה; המזהה חייב להתקיים.** כל מועמד נבדק מול האינדקס, ושיגור שנוקב רק במזהים
שאין לקורפוס הזה מסורב תוך ציון המזהים שנקב בהם, כך שטעות הקלדה נתפסת ולא מתקבלת. שיגור שאינו
נוקב בדבר בצורת מזהה מסורב תוך ציון הצורה שחיפשו.

**יש פתח מילוט נקוב בשם, והוא מתועד ולא שקט.** כתבו
<span dir="ltr">`no-item: <reason>`</span> בכל מקום ב-prompt והשיגור ימשיך, כשהסיבה נכתבת
ליומן הביקורת כ-<span dir="ltr">`op: agent-item-waived`</span>. סירוב כותב שורה משלו,
<span dir="ltr">`op: deny`</span>. תחת המפתח הזה מתקבל רק
<span dir="ltr">`enabled`</span>; כל איות אחר נדחה בזמן טעינה ונוקב במה שהמפתח כן מבין.
...
ה-hook <span dir="ltr">`Stop`</span> מבקש מכם לכתוב תחתיו מה עשיתם, מה הוחלט, ומה על
הסשן הבא לעשות ראשון. הוא מבקש **לכל היותר פעמיים בכל חלון הקשר**: פעם אחת כשהסף נחצה, ופעם
נוספת רק אם הבקשה הראשונה לא נענתה. חלון שנבנה מחדש בעקבות כיווץ מקבל תקציב משלו של שתיים.
<span dir="ltr">`SessionStart`</span> מוסר אז את הקטע הזה (עד
...
**המחסן שומר הכול, במקום אחד, בלי השגחה.** שלוש עובדות על
<span dir="ltr">`.my_context/.revisions/`</span>, רשומות ולא מתוקנות. היומן שלו רק מוסיף
ולעולם אינו נגזם, כך שפרויקט שמחזיק ומיישב הרבה רוויזיות צובר קובץ שרק גדל.
...
מקומית למכונה שהוחזקה בה, בלתי נראית לסוקר בכל checkout אחר, והיומן שלעולם אינו מוחק הצעה
אינו בבקרת גרסאות כלל. [יומן הביקורת](#יומן-הביקורת--מה-ש-my_context-באמת-עשה) חולק את
הראשונה ואת השלישית מהן וסוגר את השנייה: הוא מתגלגל ב-8 MiB ועדיין לעולם אינו מוחק, ולכן
גם הצמיחה הכוללת שלו בלתי חסומה, אבל בניגוד למחסן הרוויזיות יש לו בדיקת
<span dir="ltr">`doctor`</span> שמדווחת על גודלו. למחסן הרוויזיות עדיין אין אף אחת.

**יומן רוויזיות שניתן להכניס ל-commit נשקל ונדחה.** היומן הוא קובץ JSONL אחד שרק מוסיף,
שריפוי הזנב הקטוע שלו מניח כותב יחיד על מכונה אחת, וכל יישוב — כל קידום, כל דחייה — מוסיף
לו שורה. אילו היה בבקרת גרסאות, הוא היה פוגש את התוספות של מכונה אחרת כקונפליקט מיזוג,
ויישוב קונפליקט מיזוג פירושו שכתוב היסטוריה בתוך המחסן האחד שהבטחתו היא שהצעה שנרשמה
לעולם אינה משוכתבת. מה שסוקר ב-checkout אחר באמת צריך כבר נוסע: רוויזיה שקודמה היא הטקסט
החדש של הפריט, שעובר commit כמו כל פריט אחר. כך הצעה מוחזקת נשארת שיחה עם האדם שליד המכונה
שבה הוחזקה. זה גם אינו שינוי קטן — הוא דורש סיפור מיזוג ל-JSONL שרק מוסיף, כללי
מזהי-רוויזיה בין מכונות, וריפוי שמבחין בין זנב קטוע לשריד מיזוג, ואף אחד מאלה אינו קיים.
...
שבע-עשרה פקודות בשורת הפקודה משנות את מה ששולט בפרויקט הזה בלי אדם בלולאה. שמונה מהן מעבירות
...
<span dir="ltr">`pack import --overwrite-changed`</span>, והיא היחידה מבין החמש-עשרה
...
ל-<span dir="ltr">`draft`</span>. <span dir="ltr">`--yes`</span> לבדו אינו מגיע לשם, מפני
שהדריסה היא אישור שני עם שאלה משלו; כלל המניעה שלמטה נכתב לפיכך מול
<span dir="ltr">`pack import`</span> ולא מול דגל, שכלל הרשאות ממילא אינו יכול להתאים לו.
`edit` פועלת בשני הכיוונים, וכל שאר מה שהיא עושה
...
`focus` היא אחת משתי חברות שאינן משנות אף פריט בודד — השנייה היא `config`, למטה — והיחידה
שמשנה את מה ש**נמסר** במקום את מה שהקורפוס מכיל. כל שאר הפקודות כאן משכתבות פריט; `focus`
משכתבת את מה ש**נמסר** מהקורפוס — מיקוד שבתוקף מסתיר כל פריט כשיר שאינו תואם לו ואינו פטור — קשיח, נעוץ או המשכיות — בכל פגישה
מאוחרת יותר, על פני כל סביבת העבודה ולא רק על הפגישה שקבעה אותו. מה ששולט ומה שהמודל באמת
נאמר לו אינם אותו דבר, וזו הפקודה שמפרידה ביניהם. שתיים מחמש
...
`config` היא החברה החדשה ביותר, וזו השנייה שאינה משנה אף פריט בודד:
<span dir="ltr">`mycontext config <name> --delete|--disable --yes`</span> פועלת על
**קטגוריה** שלמה ב-<span dir="ltr">`config.json`</span> ולא על הקובץ של פריט כלשהו.
<span dir="ltr">`--disable`</span> — חוקית על קטגוריה שמובנית או מותאמת אישית כאחת — עוצרת
לכידות חדשות מהסוג הזה ומוציאה כל פריט שכבר נושא אותה מבחירה להזרקה, בלי לערוך או למחוק אף
אחד מהם; קטגוריה מובנית לעולם אינה יכולה להפוך ללא-מוכרת, רק להיכבות בדרך הזאת, ו-<span
dir="ltr">`--delete`</span> מסרבת לה בשמה, ונוקבת ב-<span dir="ltr">`--disable`</span> כחלופה.
על קטגוריה **מותאמת אישית**, <span dir="ltr">`--delete`</span> מסירה את הערך כולו מ-<span
dir="ltr">`config.json`</span>, כך שסביבת העבודה הזאת מפסיקה להכיר את מה שהיא נוקבת, וכל
פריט שכבר נושא אותה נופל לברירות המחדל של קטגוריה נעדרת עבור מדיניות ה-scope ומדיניות
העריכה. כל כתיבה מגבה קודם את <span dir="ltr">`config.json`</span>, כשיש כזה, ומדפיסה
היכן; וכל כתיבה מזהירה עם מספר הפריטים האמיתי שהקטגוריה כבר נושאת לפני השער — המספר הוא
רדיוס ההשפעה של הכתיבה האחת שעומדת לקרות, לא הסכמה שנייה.
...
| <span dir="ltr">`mycontext carry <id> --yes`</span> | מסמנת פריט אחד למסירה בהזרקה **הבאה** בלבד, בלי קשר לתקציב הדרג שלו, ושוכחת אותו — בין אם השורה התקבלה ובין אם לא. עקיפה חד-פעמית, לא <span dir="ltr">`always: true`</span> שני. היא משתמשת מחדש בגילוי `carried` שכבר קיים ב-<span dir="ltr">`core/select.ts`</span> להעברה בין-פגישתית: המזהה מגיע כשורת **אינדקס** בראש התור — לעולם לא הטקסט המלא של הפריט, ולעולם לא שינוי במה שהפריט אומר. <span dir="ltr">`--clear`</span> מבטלת את כל התור הממתין מאחורי אותו אישור; <span dir="ltr">`--show`</span> מדווחת על התור ומסרבת ל-<span dir="ltr">`--yes`</span> בשמו, מפני שהיא אינה משנה דבר |
...
| <span dir="ltr">`mycontext review discard-revision <id> --yes`</span> | דוחה רוויזיה ממתינה — <span dir="ltr">`--revision REV-...`</span> נדרש באותם תנאים כשיותר מאחת ממתינה. היא אינה משנה דבר במה ששולט, ולכן אינה נספרת בין השבע-עשרה שלמעלה — אבל היא מיישבת, סופית, הכרעה שתור הרוויזיות קיים כדי לשמור לאדם, ואותה הצעה אינה יכולה להיות מוחזקת שוב מול אותו טקסט. ההצעה עצמה נשארת ביומן |
| <span dir="ltr">`mycontext config <name> --delete|--disable --yes`</span> | מכבה או מוחקת קטגוריה שלמה ולא פריט בודד. <span dir="ltr">`--disable`</span> — חוקית על קטגוריה מובנית או מותאמת אישית כאחת — עוצרת לכידות חדשות מהסוג הזה ומוציאה כל פריט שכבר נושא אותה מבחירה להזרקה, בלי לערוך או למחוק אף אחד מהם בדיסק. <span dir="ltr">`--delete`</span> — מסורבת בשמה על קטגוריה מובנית, שמופנית להשתמש ב-<span dir="ltr">`--disable`</span> במקום — מסירה ערך של קטגוריה **מותאמת אישית** מ-<span dir="ltr">`config.json`</span> כליל, כך שסביבת העבודה הזאת מפסיקה להכיר את מה שהיא נוקבת. מגבה את <span dir="ltr">`config.json`</span> קודם, כשיש כזה, ומדפיס את נתיב הגיבוי; מזהיר עם מספר הפריטים האמיתי לפני השער |
...
| <span dir="ltr">`mycontext restore --approve <key> --yes`</span> | משחררת סיכום של שיחה **קודמת** אל הסשן **הבא** שייפתח — תיאור מילולי של מה שהוכרע, מה שתוקן, מה שנמדד, מה שנשאר פתוח ומה שנוסה ונכשל, שנבנה מתמליל ששמור בדיסק. היא אינה עורכת שום פריט ואינה גורמת לדבר לשלוט, והיא כאן מסיבתה של `carry` ולא מסיבתה של `edit`: מה שהיא מכריעה הוא מה שחלון הקשר שלם מקבל, והחלון הזה הוא חלון **מאוחר יותר**, כך שמי שמאשר אינו נמצא בלולאה שקוראת אותו. שלוש הצורות האחרות אינן מגודרות ואינן צריכות להיות — <span dir="ltr">`--build`</span> מחזיקה הצעה אוטומטית ואינה מוסרת דבר, <span dir="ltr">`--show`</span> מדווחת, ו-<span dir="ltr">`--discard`</span> מושכת בחזרה. האישור עצמו קורא מחדש את הקובץ המוחזק מהדיסק ומסרב לומר שבטוח לנקות את החלון אלא אם מה שנמצא שם הוא מה שהוצג לך |
...
### הממשק יכול להריץ את מה שהוא מרכיב

`mycontext ui` מרכיב פקודות בכמה ממסכיו. מאז 27 באוגוסט 2026 השרת גם יכול **להריץ** אותן:
`POST /api/execute` פעיל, וכל פקודה בקטלוג ניתנת להרצה — הקריאות, הכתיבות, ואלה שמשנות את מה
ששולט בפרויקט הזה.

> [!NOTE]
> **המסכים מציעים את הכפתור.** ההערה הזאת אמרה פעם שאינם, ונשארה עומדת אחרי שהפקד נשלח —
> מסמך שסותר את הקוד, בדיוק הפגם שהפרויקט הזה קיים כדי למנוע, ודווקא במקטע שמתאר
...
> הזדמנויות לטעות בו.
> <!-- `command-actions.js` · `outcome = await ctx.post('/api/execute', { id, values, nonce: answer.nonce });` · ~603 -->

כל הרצה נמצאת מאחורי אישור.

איזה אישור פקודה מקבלת **נגזר מהמנתח האמיתי של הארגומנטים**, ולא נשמר כרשימה שמישהו מתחזק:

- **מתחת לגבול האישור** — `doctor`, `status`, `decay`, `search`, `rebuild` — אישור פשוט
  הנוקב בפקודה ובארגומנטים שנפתרו.
- **עליו או מעליו** — `add`, `edit`, `pin`, `supersede`, `review promote` והשאר — הפרש שדה
  אחר שדה, לפני ואחרי, הנוקב בכל שדה שהפקודה תשנה.

פקודה שנוספה מאוחר יותר ואיש לא סיווג אותה מקבלת את האישור **החזק יותר**. סיווג מיושן עולה
אפוא בטקס, לעולם לא בכתיבה שקטה.

שלוש תכונות תוחמות את זה, וכל אחת ניתנת לבדיקה ולא רק להבטחה. הדפדפן שולח **מזהה** מהקטלוג
וערכים, לעולם לא שורת פקודה — השרת בונה מחדש את הארגומנטים מהקטלוג שלו עצמו ומריץ אותם בלי
מעטפת, כך ששום טקסט שמישהו מקליד אינו הופך לפקודה. מסלול האישור טובע **אסימון חד־פעמי הקשור
בדיוק לפקודה שהוצגה**, כך שהרצה שלא אושרה אינה יכולה לקרות **בשקט** — זו התכונה שזה קונה,
והיא צרה יותר מ"אינה יכולה לקרות כלל": דף מקומי שכבר מחזיק את האסימון יכול לקרוא למסלול
האישור בלי לצייר אותו לעולם. וכל הרצה נכתבת ליומן
הביקורת **לפני** שהתהליך מתחיל: הרצה שאי אפשר לתעד אינה מתרחשת.

> [!WARNING]
> **זה רץ על המכונה שלכם, עכשיו. הממשק יודע לומר שזה הגיע מהדפדפן שלכם — לא שאתם ביקשתם.**
> שער ה־loopback מוכיח שבקשה הגיעה מדפדפן במחשב הזה. אין בו דרך להוכיח שאדם ביקש אותה, ושום
> דבר בתכנון הזה אינו משנה את זה. הריצו רק את מה שאתם מזהים.
>
> **אין מתג שמכבה את ההרצה.** רשומת הביקורת היא סיפור האחריות והאישור הוא השער. אם אינכם
> רוצים שדף מקומי יוכל להגיע לזה כלל — אל תריצו `mycontext ui`.

### שרת שכבר רץ יכול להטביע nonce חדש

מאז 28 באוגוסט 2026 השרת גם עונה ל-`POST /api/nonce`, ו-`mycontext ui --nonce` היא הפקודה
שקוראת לו: היא מטביעה nonce חד-פעמי טרי למסירה משרת שכבר **רץ**, במקום להפעיל שרת מחדש.
הסיבה לקיומה היא שהחלופה הייתה גרועה יותר. ללשונית דפדפן שאיבדה את האסימון שלה יש דרך חזרה
אחת בלבד — nonce — ו-nonce נהג להיות מודפס רק כשהשרת **מתחיל** לרוץ. שחזור לשונית נעולה אחת
חייב הפעלה מחדש של השרת, וזו מטביעה אסימון חדש ומפנה את הישן ביותר מ-64 ה-digests
ש-`ui-sessions.json` זוכר; הפעלה מחדש שמצילה לשונית אחת עלולה לנעול לשונית אחרת. `--nonce`
שובר את המעגל הזה ומדפיס את אותה שורת כתובת שמסלול ההפעלה מדפיס.

מסלול זה **פטור מאסימון**, בדיוק כמו `/api/handoff` שלמעלה, ומוחזק תחת אותו שער בדיוק:
`Host` לולאה מקומית, `Origin` תואם כשנשלח כזה, ו-POST בלבד. ההבדל הוא ממה הוא פטור.
`/api/handoff` **מחליף nonce באסימון** — הקורא צריך כבר להחזיק nonce כדי לקבל אסימון תמורתו
— כך שקורא שאין בידו אף אחד מהשניים אינו יכול להגיע אליו. `POST /api/nonce`, לעומת זאת,
**מייצר nonce משום דבר**, מלבד בדיקת Host/Origin עוברת.

> [!WARNING]
> **מרגע שהשרת מתחיל ועד שהוא יוצא, כל תהליך מקומי שיכול להגיע ל-127.0.0.1 בפורט שלו יכול
> לקבל אסימון בכל רגע — לא רק בשניות שאחרי ההפעלה, כש-nonce נהג להיות מודפס ואז נעלם.** זהו
> המחיר של סגירת הנעילה, שהתקבל במכוון ב-28 באוגוסט 2026 ולא הושאר בלי גילוי. כל הטבעה
> נכתבת ליומן הביקורת בכל מקרה — `op: nonce-minted`, ה-`Host` וה-`Origin` שנשלחו, לעולם לא
> ה-nonce עצמו — כך שהיווצרות אסימון תמיד משאירה עקבות, גם כשדבר לא סורב. ה-nonce שנמסר
> הוא חד-פעמי וחי שלושים שניות, או עשר דקות כאשר <span dir="ltr">`?ttl=printed`</span> מבקש
> את משך החיים המודפס ש-<span dir="ltr">`--nonce --no-open`</span> משתמש בו; החשיפה שהשארית
> הזאת נוקבת בה היא **היכולת הקבועה לבקש**
> אחד, לא אסימון בודד ששרד את החלון שלו.

שום דבר אחר בגבול אינו זז. אסימון שהתקבל בדרך הזאת פותח בדיוק את אותו משטח `/api` לקריאה
בלבד שכל אסימון אחר פותח — ומאז 27 באוגוסט 2026, גם את מסלול ההרצה שתואר למעלה, באותם
תנאים.
...
| **MCP** | Model Context Protocol — הממשק שדרכו Claude מגיע לכלים. my_context מגיש עשרים ושמונה מהם מעל stdio, והם המשטח היחיד של המודל אם אין לו shell |
...
| **rationale** (נימוקים) | הדרג של הסיבה שהפרויקט הוא כפי שהוא: החלטות, מסמכי ADR, לקחים, פשרות, הנחות, מקרי קצה, סיכונים. מאונדקס, ניתן לחיפוש, נשלף לבקשה — לעולם לא מוזרק בלי שביקשו, אלא אם סימנתם את הפריט `continuity: true`, הדרג היחיד שאינו מתייעץ בחלוקה הזאת |
...
| **tier** (דרג) | שני דברים שונים, תלוי במשפט. הדרג של *קטגוריה* הוא `normative` או `rationale` ([פרק 2](#2-הרעיון)). דרג *הזרקה* הוא אחד מחמשת מסלולי האספקה — נעוץ, בדיוק בזמן, משוחזר, המשכיות, אינדקס ([פרק 4](#4-מתי-זה-חוזר-ומה)) |
