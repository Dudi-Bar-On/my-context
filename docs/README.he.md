...
**המודל** קורא לשישה-עשר כלי ה-MCP. שני המשטחים קוראים וכותבים לאותם קובצי Markdown תחת
...
flowchart TB
  ...
  A(["<b>Claude</b>"]) --> TL["<b>כלי MCP</b><br/>שישה-עשר, מוגשים מעל stdio"]
...
שישה-עשר כלים, מוגשים מעל stdio על ידי `src/mcp/server.ts`. המודל מגיע אליהם בלי shell, וכל
...
דרך פקודה. **הדרישה הזאת מקוימת היום, ונאכפת בבדיקה ולא בסקירה.** לכל אחד משישה-עשר כלי
...
| **MCP** | Model Context Protocol — הממשק שדרכו Claude מגיע לכלים. my_context מגיש שישה-עשר מהם מעל stdio, והם המשטח היחיד של המודל אם אין לו shell |
