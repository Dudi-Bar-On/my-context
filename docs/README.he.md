...
**המודל** קורא לשלושה-עשר כלי ה-MCP. שני המשטחים קוראים וכותבים לאותם קובצי Markdown תחת
...
```mermaid
flowchart TB
  U(["<b>אתה</b>"]) --> SL["<b>/mycontext:…</b><br/>65 פקודות סלאש"]
  U --> CL["<b>mycontext …</b><br/>29 פקודות שורת פקודה"]
  A(["<b>Claude</b>"]) --> TL["<b>כלי MCP</b><br/>שלושה-עשר, מוגשים מעל stdio"]
...
29 פקודות. `mycontext help` מדפיס את אותה רשימה מהתוכנית עצמה,
...
שלושה-עשר כלים, מוגשים מעל stdio על ידי `src/mcp/server.ts`. המודל מגיע אליהם בלי shell, וכל
...
דרך פקודה. **הדרישה הזאת מקוימת היום, ונאכפת בבדיקה ולא בסקירה.** לכל אחד משלושה-עשר כלי
...
ל-9 מתוך 29 פקודות שורת הפקודה אין אחת, לכל אחת מסיבה שרשומה לידה
...
### שתי דרישות רשומות שהפרויקט הזה אינו מקיים

שתי אלה שונות מכל השאר בפרק הזה, וההבדל ראוי שייאמר בפירוש ולא ירוכך.

**שתיהן רשומות בקורפוס של המאגר הזה עצמו כדרישות עם `severity: hard`
...
הרשימה הזאת מנתה שלוש. <span dir="ltr">`REQ-changes-are-timestamped-and-audited`</span>
ירדה ממנה כש[יומן הביקורת](#יומן-הביקורת--מה-ש-my_context-באמת-עשה) נשלח, והפריט בקורפוס
סומן באותו שינוי ולא אחריו. סעיף אחד שלה עדיין אינו מקוים ורשום ככזה בפריט עצמו: הפריטים
עדיין אינם נושאים שדות <span dir="ltr">`created_at`/`updated_at`</span> ב-frontmatter.
היומן רושם מתי כל שינוי קרה, ולכן היסטוריית הפעולות שהיא מבקשת קיימת; חותמות הזמן ברמת
הפריט לא.
...
כל אחת משתי אלה צריכה החלטה מוצרית לפני שהיא צריכה מממש. הוצאת אחת מהן לגמלאות היא תוצאה
...
סדר. מתוכם, <span dir="ltr">`counts.test.ts`</span> מחשב מהתוכנית הרצה את היחס "9 מתוך 29
...
| **MCP** | Model Context Protocol — הממשק שדרכו Claude מגיע לכלים. my_context מגיש שלושה-עשר מהם מעל stdio, והם המשטח היחיד של המודל אם אין לו shell |
