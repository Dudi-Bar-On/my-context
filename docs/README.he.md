...
**אם מילה או ‎`--flag`‎ כאן אינם מובנים מאליהם, יש מקום שאפשר לקפוץ אליו ישירות.** כל מונח
שהמסמך הזה נותן לו משמעות מסוימת מוגדר ב[מילון המונחים](#9-מילון-מונחים), וכל אפשרות של
שורת הפקודה נמצאת בטבלה אחת: [כל הדגלים, במקום אחד](#כל-הדגלים-במקום-אחד). המונחים מוגדרים
גם בשפה פשוטה במקום הראשון שבו הם מופיעים, כך שקריאה מההתחלה לסוף לעולם אינה מחייבת אף אחד
מהשניים.
...
שלוש מילים במשפט האחרון משמשות במסמך הזה במובן מדויק, ולכן כדאי לקבע אותן לפני שנבנה
עליהן משהו.
...
- פריט **שולט** כשהוא כשיר להזרקה והוא מנוסח כהוראה — משהו שהמודל אמור לציית לו, ולא רק
  לדעת עליו.
- **דרג** היא המילה לחלוקה בין נורמטיבי לנימוקים. כל קטגוריה — `constraint`, `decision`,
  `rule`, `lesson` והשאר — נושאת דרג אחד, ואפשר לשנות איזה ([פרק 6](#6-תצורה)). שימו לב
  לשימוש שני, שאינו קשור, באותה מילה: [פרק 4](#4-מתי-זה-חוזר-ומה) קורא לארבעת מסלולי
  האספקה שלו *דרגי הזרקה*. במקומות שבהם ההבדל חשוב, המשפט אומר במה מדובר.

אוסף הפריטים של הפרויקט — כל מה שנמצא תחת `.my_context/items/`, בכל דרג ובכל סטטוס — הוא
ה**קורפוס** שלו.
...
כל מה שבא אחרי הכותרת הוא **אפשרות** — צמד ‎`--name value`‎ שקובע שדה אחד בפריט. ארבעה
דברים בפקודה הזאת חשובים.

- ‎`--body "…"`‎ הוא הטקסט של הפריט: הפסקה ש-Claude באמת יקבל. הכותרת אומרת מה הכלל; הגוף
  אומר למה, ו"למה" הוא מה שמונע מכלל להיות מיושם מכנית במקרה שהוא מעולם לא עסק בו.
- ‎`--scope "src/api/**"`‎ הוא מה שהופך את הכלל לממוקד במקום סביבתי. זהו **גלוב היקף** —
  תבנית של נתיב קובץ, שבה ‎`*`‎ תואם בתוך רמת תיקייה אחת ו-‎`**`‎ תואם על פני כמה רמות
  שצריך. האילוץ הזה נוגע לשכבת ה-API, ולכן הוא יחזור כשנוגעים בקוד של ה-API וישאר מחוץ
  לדרך בכל מצב אחר. כלל בלי היקף נשמר, מאונדקס וניתן לחיפוש, אבל לעולם לא מוזרק מעצמו —
  ראו [פרק 4](#4-מתי-זה-חוזר-ומה).
- ‎`--tags uploads`‎ מצמיד תוויות חופשיות. הן אינן משנות דבר לגבי מתי פריט מוזרק; הן שם
  כדי שתוכל למצוא אותו אחר כך.
...
הגוש שבין שורות ה-‎`---`‎ הוא ה-**frontmatter**: השדות ש-my_context משתמש בהם כדי להחליט
מתי הפריט הזה חוזר ועד כמה לסמוך עליו. כל מה שמתחתיו הוא הגוף, והגוף הוא מה ש-Claude באמת
קורא. שדה אחר שדה:

| שדה | מה המשמעות |
|---|---|
| `id` | שם הפריט, נגזר מהכותרת. המזהים הם הדרך שכל דבר אחר מפנה אליו |
| `type` | הקטגוריה שלו — `constraint`, `decision`, `rule` וכן הלאה. הקטגוריה קובעת את הדרג |
| `status` | ‏`draft`, `active`, `superseded`, `deprecated` או `validated`. **רק `active` מוזרק אי פעם**; מה משמעות ארבעת האחרים כתוב ב[מילון המונחים](#9-מילון-מונחים) |
...
| `scope` | גלובי הקבצים שהפריט נצמד אליהם. ריק פירושו שהוא לעולם לא מוזרק אוטומטית |
| `tags` | תוויות חופשיות למציאה מאוחרת. הן אינן משפיעות על ההזרקה |
| `origin` | מי כתב אותו: `human`,‏ `agent` (כלומר Claude, דרך כלי MCP) או `ingest` (חולץ ממסמך). על השדה הזה בנוי [גבול האמון](#7-גבול-האמון), ואף כלי אינו מאפשר לקורא לקבוע אותו |
| `source_file`,‏ `source_anchor`,‏ `source_checksum` | מהיכן הפריט הגיע כשהוא חולץ ממסמך: הנתיב, הכותרת בתוכו, וגיבוב של אותו טקסט כדי שאפשר יהיה לזהות סטייה |
| `valid_from`,‏ `valid_until` | היום שבו התחיל לחול, והיום שבו חדל. ‏`valid_until` ממולא כשפריט פורש |
| `checksum` | גיבוב של תוכן הפריט עצמו, מוחתם מחדש בכל כתיבה. כך `mycontext doctor` מבחין בקובץ שנערך ביד |

חלק מהקטגוריות מוסיפות שדה משלהן — פריט `rule`, למשל, נושא ‎`directive: do`‎ או
‎`directive: dont`‎. הפקודה ‎`mycontext examples <category>`‎ מדפיסה דגם תקין של כל סוג,
כולל השדות הנוספים.
...
הגלובלית, ואז לפי מזהה כדי שהתוצאה תהיה דטרמיניסטית. **שכבה** היא המקום שבו קובץ הפריט
חי: `.my_context/` בפרויקט שאתה עובד עליו היא שכבת ה*פרויקט*, ותיקיית `.my-context`
בתיקיית הבית שלך, אם קיימת כזאת, נקראת לצידה כשכבה *גלובלית*. הפרויקט מנצח בתיקו, כך
שפריט של הפרויקט מתקבל לפני פריט גלובלי שמתחרה על אותו מקום, ופריט של הפרויקט עם אותו
מזהה מסתיר את הגלובלי לגמרי. פריט גדול מדי למקום שנותר מדולג במקום לסיים את המעבר, כך
שפריט קטן יותר אחריו עדיין יכול להתקבל. על פריט שדולג כך אומרים ש**נשפך** (spill) — זו
המילה שבה משתמש הקוד, והפסקה שלמטה היא איך שפיכה נראית מבחוץ.
...
אין עמודת `title`, וזה בכוונה. מזהה הוא סלאג של הכותרת — `CONST-postgres-pool-capped-at-20`‎
עבור "Postgres pool capped at 20" — כך ששתי העמודות הרחבות ביותר בטבלה הזאת אמרו אותו דבר
פעמיים, וביחד הביאו את דוח ברירת המחדל ל-192 תווים בקורפוס של המאגר הזה עצמו, מול פריסה
ברוחב 100. בלי הכותרת הוא 97. הכותרת עדיין מופיעה במלואה ב-`mycontext show`, ב-`list --full`‎
וב-`list --json`; אותה הסרה נעשתה ב-`mycontext decay`‎ (מ-170 תווים ל-97), ב-`review list`‎,
ובטבלת הפריטים הקרים שבתוך `status --full`‎.
...
באותיות קטנות ועם מקפים — ‏`## Rate limits` הופך ל-`rate-limits` — וכך שני צידי השיחה
מסכימים על איזה חלק של המסמך מדובר. **מועמד** הוא פריט מוצע שעדיין לא קיים: חולץ, תואר
ב-JSON, ואין לו דבר על הדיסק עד שמחילים אותו. כל מועמד חייב לצטט את מקטע המקור שלו מילה
במילה — פרפרזה נדחית — וכל מה שמוחל
...
‎`MYCONTEXT_WIDTH` כדי לפרוס לרוחב אחר. כלל אחד גובר על התקציב: שום עמודה אינה מצטמצמת
מתחת לאסימון הבודד הארוך ביותר שבה, ולכן מזהה, גלוב או נתיב לעולם אינם נשברים בין שורות
ונשארים ניתנים להעתקה. לכן טבלה שהאסימונים שלה עצמם רחבים מהתקציב — קורפוס שבו המזהים
לבדם חורגים ממנו — נשארת ברוחבה הטבעי במקום להידחס לכיוון מספר שאינה יכולה להגיע אליו,
משום שדחיסה כזאת עולה בשורות שלמות ועדיין חורגת. כל הדוחות בקורפוס של המאגר הזה עצמו
נכנסים עכשיו: הרחב מכולם, `list`‎, הוא 97 תווים.
...
**דגל** — נקרא גם אפשרות או מתג — הוא ‎`--name`‎ שנכתב אחרי פקודה. יש כאן שני סוגים.
*מתג* הוא דלוק או כבוי ואינו מקבל דבר אחריו (‎`--yes`,‏ ‎`--json`‎). *דגל ערך* בא בליווי
הערך שיש לקבוע, ושני האיותים ‎`--name value`‎ ו-‎`--name=value`‎ שקולים בכל מקום בשורת
הפקודה הזאת.

עשרים ושניים אלה הם כולם. שום דבר כאן אינו חל על כל הפקודות: כל שורה אומרת בדיוק היכן
הדגל עובד, ופקודה שקיבלה דגל שאינה מכירה או מסרבת לו או — בכמה פקודות — מתעלמת ממנו,
ומי מהשתיים [מפורט למטה](#שלושה-כללים-שחלים-על-כולם). כלי ה-MCP מקבלים ארגומנטים
בשם ב-JSON ולא דגלים; אלה טבלת הכלים [שלמעלה](#מה-שהמודל-קורא-לו-כלי-ה-mcp).

**בחירת כמות הפירוט שדוח מדפיס.**

| דגל | מה הוא עושה | היכן הוא עובד |
|---|---|---|
| ‎`--short`‎ | שורה אחת לכל פריט, בטבלה מיושרת בעמודות. **זו ברירת המחדל** — אין צורך להקליד אותה לעולם | `list`,‏ `status`,‏ `decay`,‏ `doctor`,‏ `review list`,‏ `ingest-status` |
| ‎`--full`‎ | בית אחד לכל פריט, כל שדה בשורה מתויגת משלו. לא טבלה רחבה יותר | אותן שש |
| ‎`--summary`‎ | הצורה בלי השורות: ספירות כותרת ואזהרות בלבד | אותן שש |
| ‎`--json`‎ | מסמך JSON אחד במקום טבלה, כולל שגיאות טעינה של הקורפוס. הייצוג הנאמן היחיד של דוח מקונן | אותן שש, ובנוסף ‎`mycontext query`‎ |
| ‎`--quiet`‎ | ב-‎`mycontext doctor`‎ בלבד, איות ותיק יותר של ‎`--summary`‎. אם תעבירו גם ‎`--quiet`‎ וגם רמת פירוט, ‎`--quiet`‎ מנצח ואף אחד לא אומר זאת | `doctor` |
| ‎`--sessions <n>`‎ | כמה סשנים אחרונים נחשבים "לאחרונה" בדוח הדעיכה. ברירת מחדל 20; חייב להיות מספר שלם גדול מאפס | `decay` |
| ‎`--all`‎ | להציג גם את הפריטים ה*חמימים* — אלה שכן הוזרקו בתוך החלון, ושהדוח משמיט אחרת. ‎`--full`‎ כבר כולל אותם | `decay` |
| ‎`--limit <n>`‎ | מספר השורות המרבי ששאילתת SQL מחזירה. ברירת מחדל 1000, מינימום 1; אין הגדרה של "בלי הגבלה". כשהתקרה נוגסת, הדוח אומר זאת | `query` |
| ‎`--type <category>`‎ | להציג רק טיוטות מקטגוריה אחת. שם שאין לו קטגוריה פשוט לא תואם דבר — זו אינה שגיאה | `review list` |

**קביעת שדה בפריט.**

| דגל | מה הוא עושה | היכן הוא עובד |
|---|---|---|
| ‎`--body "<text>"`‎ | הטקסט של הפריט — הפסקה ש-Claude מקבל | `add` |
| ‎`--scope "<globs>"`‎ | תבניות הקבצים שהפריט נצמד אליהן, מופרדות בפסיקים | `add`,‏ `review promote`,‏ `lesson-accept` |
| ‎`--tags "<labels>"`‎ | תוויות חופשיות, מופרדות בפסיקים. אינן משפיעות על ההזרקה | `add` |
| ‎`--severity hard\|soft`‎ | פריטי `hard` מתקבלים לתקציב לפני `soft`. כל מילה אחרת מסורבת | `add`,‏ `review promote`,‏ `lesson-accept` |
| ‎`--always`‎ | לנעוץ את הפריט: להזריק אותו במלואו בתחילת כל סשן, בלי קשר לקבצים. זמין רק כל עוד הפריט טיוטה | `review promote` |
| ‎`--title "<text>"`‎ | להחליף את כותרת המועמד המבוים בניסוח שלך לפני שהכלל נוצר | `lesson-accept` |
| ‎`--directive do\|dont`‎ | האם הכלל שנוצר מורה או אוסר | `lesson-accept` |
| ‎`--by <id>`‎ | נוקב במחליף שתופס את מקומו של הפריט הפורש. **חובה** — פרישה בלי יורש אינה מוצעת | `supersede` |
| ‎`--reason "<text>"`‎ | למה הפרישה קרתה. זה נרשם כתצפית `supersession` על ה**מחליף**, בנוסח ‎`Replaces <old id>: <your text>`‎ | `supersede` |

**אישור שינוי, והזנת נתונים פנימה.**

| דגל | מה הוא עושה | היכן הוא עובד |
|---|---|---|
| ‎`--yes`‎ | לאשר בלי שישאלו. כל אחת מהפקודות האלה אומרת מה היא עומדת לעשות ואז ממתינה לכן; זה עונה מראש, וזה מה שהופך את הפקודה לשמישה בסקריפט. זה אינו אמצעי אבטחה — ראו [פרק 7](#7-גבול-האמון) | `add`,‏ `review promote`,‏ `review discard`,‏ `supersede`,‏ `repair` |
| ‎`--anchor <a>`‎ | לאיזה חלק של המסמך הכוונה. ב-`ingest` הוא מבקש מחדש מקטע מסוים במקום את הבא בתור; ב-`ingest-apply` הוא **חובה**, ואומר מאיזה מקטע הגיעו המועמדים שאתם מחזירים | `ingest`,‏ `ingest-apply` |
| ‎`--file <path>`‎ | לקרוא את מטען ה-JSON מקובץ במקום מהקלט התקני | `ingest-apply`,‏ `lesson-stage` |
| ‎`--stdin`‎ | לקרוא את מטען ה-JSON מהקלט התקני — האיות להזרמה פנימה. ‏`ingest-apply` דורשת אחד מבין ‎`--file`‎ ו-‎`--stdin`‎ ומדפיסה שימוש אם לא ניתן אף אחד; ‏`lesson-stage` קוראת מהקלט התקני בכל פעם ש-‎`--file`‎ נעדר, כך שבפקודה הזאת ‎`--stdin`‎ מתעד כוונה ולא מפעיל דבר | `ingest-apply`,‏ `lesson-stage` |

#### שלושה כללים שחלים על כולם

**חזרה על דגל או אוספת או מסרבת, ולעולם אינה שומרת אחד מהם בשקט.** ‏‎`--scope`‎ ו-‎`--tags`‎
הם רשימות, ולכן חזרה פירושה "וגם": ‎`--scope "src/api/**,src/db/**"`‎ ו-‎`--scope
src/api/** --scope src/db/**`‎ מייצרים בדיוק את אותו פריט. כל דגל ערך אחר מחזיק ערך יחיד,
ומסירתו פעמיים מסורבת מכול וכול במקום להיפתר — ‎`--body x --body y`‎ נעצר בהודעה שנוקבת
בשניהם. זו אינה קפדנות: שמירת הראשון בשקט היא מה ששורת הפקודה הזאת עשתה פעם, וזה תייג
בהיקף שגוי פריט אמיתי בקורפוס של המאגר הזה עצמו לפני שמישהו שם לב.

**‏‎`--yes=false`‎ פירושו לא.** מתג אינו רק צורתו החשופה. ‏‎`--yes`,‏ ‎`--yes=true`,‏
‎`--yes=yes`,‏ ‎`--yes=on`‎ ו-‎`--yes=1`‎ כולם מאשרים; ‎`--yes=false`,‏ ‎`--yes=no`,‏
‎`--yes=off`‎ ו-‎`--yes=0`‎ כולם מסרבים, ומשאירים את הפקודה בדיוק במקום שבו הייתה בלי
‎`--yes`‎ כלל — היא שואלת, או מסרבת אם אין טרמינל לשאול בו. כל דבר אחר, כמו ‎`--yes=maybe`,‏
מסורב ולא מנוחש, והעברת איות אמת ואיות שקר של אותו דגל מסורבת אף היא. כל זה חל על
‎`--json`,‏ ‎`--full`,‏ ‎`--all`‎ וכל מתג אחר, לא רק על ‎`--yes`‎.

**דגל שאינו מוכר מסורב — ברוב הפקודות.** ‏‎`mycontext status --ful`‎ נעצר ונוקב בשגיאת
ההקלדה במקום להדפיס את דוח ברירת המחדל ולצאת עם 0. הפקודות שבודקות הן `add`,‏ `list`,‏
`status`,‏ `decay`,‏ `doctor`,‏ `review` (כל תת-פקודה מול המערך שלה), `ingest-status`,‏
`query`,‏ `repair` ו-`supersede`. גם ‎`mycontext help`‎ מסרבת, אך בדרך אחרת: היא קוראת את
מה שבא אחריה כשם נושא, ו-‎`--anything`‎ אינו אחד מארבעת הנושאים שלה. הפקודות ש**אינן**
בודקות הן `init`,‏ `show`,‏ `rebuild`,‏ `examples`,‏ `ingest`,‏ `ingest-apply`,‏ `lesson`,‏
`lesson-stage`,‏ `lesson-accept` ו-`lesson-discard`: דגל שהן אינן מכירות נזרק בלי מילה.
נבדק בהרצה של כל אחת מהן; הפער אמיתי וכדאי להכיר אותו לפני שסומכים על כך שדגל אכן נכנס
לתוקף.
...
קבועה בת 284 תווים בלי שבירת שורות ב*כל* רמת פירוט. שניהם תוקנו קודם — ‎`--full` היא בית
לכל פריט וכל פסקה נשברת, והכול נפרס לרוחב 100 תווים ([פרק 5](#5-שימוש) מתאר את הצורות).

טבלת ברירת המחדל/‎`--short` החזיקה מעמד הכי הרבה זמן, ותוארה כאן כמגבלה אמיתית ולא כעבודה
שלא הושלמה: שתי העמודות הרחבות ביותר שלה היו המזהה והכותרת, המזהים בקורפוס הזה מגיעים
ל-64 תווים, ואסור לשבור אף אחת מהשתיים — `INV-a-validator-that-gates-writes-must-`‎ נקרא
כמזהה שלם, כך שקורא היה מעתיק חצי מזהה ומקבל תשובה שאין פריט כזה עבור שורה שנמצאת על המסך
שלו. המסקנה שנרשמה אז, שרק מזהים קצרים יותר יכולים לתקן את זה, שגתה באבחנה. המזהה **הוא**
הכותרת: `makeId` הופך את האחת לסלאג של השנייה, כך ששתי העמודות היו עובדה אחת שתפסה 156 מתוך
192 התווים של הטבלה. הסרת הכפילות — ולא קיצור המזהה, שהיה הופך את `RULE-014.md changed`‎
לחסר משמעות בהשוואת גרסאות — הביאה את `list`‎ ל-97 תווים ואת `decay`‎ ל-97. `review list`‎
וטבלת הפריטים הקרים ב-`status --full`‎ איבדו את אותה עמודה מאותו נימוק.
...
- **עדיין לא שוחררה ולא תויגה שום גרסה.** שיטת הגרסאות הוכרעה ונכתבה
  ...
  אין תגיות git, ולכן כל מה שנעשה עד כה יושב תחת `[Unreleased]`, וה-`0.1.0` שהמניפסטים
  נושאים הוא הגרסה שמוכנה לשחרור, לא כזו שפורסמה. עד שתהיה תגית, גיבוב הקומיט הוא עדיין
  התשובה המדויקת לשאלה "איזו בנייה זו".
...
קיימת כמו כל פקודה אחרת. מה ששום בדיקה אינה בודקת בשום מקום הוא אם *דגל* מתנהג כפי
שהשורה שלו אומרת: כל שורה נכתבה מתוך הרצה של הדגל וקריאה של מה שחזר, וזו חובה אנושית
בכל פעם שאחד מהם משתנה.
...
| **always** | שדה ה-frontmatter שנועץ פריט. ‏`always: true` פירושו הזרקה במלואו בתחילת כל סשן, בלי קשר לקבצים שאתה נוגע בהם |
| **anchor** (עוגן) | הכותרת שמעליה יושב מקטע של מסמך שנקלט, באותיות קטנות ועם מקפים: ‏`## Rate limits` הופך ל-`rate-limits`. שני צידי שיחת הקליטה משתמשים בו כדי לנקוב באותו מקטע |
| **budget** (תקציב) | מגבלת הגודל של דרג הזרקה אחד, בטוקנים משוערים. ארבעה כאלה, אחד לכל דרג, כולם ניתנים לתצורה. מה שלא נכנס — נשפך |
...
| **checksum** (סכום ביקורת) | גיבוב של תוכן הפריט עצמו, מוחתם מחדש בכל כתיבה. ‏`mycontext doctor` משווה אותו לקובץ כדי להבחין בעריכה ידנית |
| **compaction** (כיווץ) | ‏Claude Code שמסכם סשן ארוך וממשיך מהסיכום. בדרך כלל נזרק בו מה שהוזרק קודם, ובגלל זה קיים הדרג המשוחזר |
| **corpus** (קורפוס) | כל הפריטים של הפרויקט: כל מה שתחת `.my_context/items/`, בכל דרג ובכל סטטוס |
| **deprecated** (מיושן) | פורש בלי שנקבו במחליף. זה מה ש-‎`mycontext review discard`‎ קובעת על טיוטה. לא מוזרק; הקובץ נשאר במקומו |
...
| **frontmatter** | הגוש שבין שורות ה-‎`---`‎ בראש קובץ פריט: השדות שקובעים מתי הפריט חוזר ועד כמה לסמוך עליו. הפרוזה שמתחתיו היא הגוף |
| **governing** (שולט) | היות כשיר להזרקה *וגם* מנוסח כהוראה. פריטים נורמטיביים שולטים; פריטי נימוקים לעולם לא |
| **index** (אינדקס) | דרג ההזרקה הזול ביותר: שורה אחת — מזהה, סוג, כותרת — לכל פריט נורמטיבי שלא סופק במלואו, ועוד ספירות לכל השאר. זה גם השם של ‎`.index.db`, מטמון ה-SQLite הניתן להשלכה; ההקשר מבחין ביניהם |
...
| **JIT** / **just in time** (בדיוק בזמן) | דרג ההזרקה שנורה כש-Claude עומד לקרוא או לערוך קובץ שתואם להיקף של פריט. נכתב `jit` בתצורת התקציבים |
| **layer** (שכבה) | היכן חי קובץ הפריט. ‏`.my_context/` בפרויקט שאתה עובד עליו היא שכבת ה*פרויקט*; תיקיית `.my-context` בתיקיית הבית, כשקיימת כזאת, נקראת לצידה כשכבה *גלובלית*. פריטי הפרויקט מנצחים בתיקו ומסתירים פריט גלובלי עם אותו מזהה |
| **MCP** | ‏Model Context Protocol — הממשק שדרכו Claude מגיע לכלים. ‏my_context מגיש אחד-עשר מהם מעל stdio, והם המשטח היחיד של המודל אם אין לו קונכייה |
...
| **origin** (מקור) | מי כתב פריט: `human`,‏ `agent` או `ingest`. על השדה הזה בנוי גבול האמון |
| **pinned** (נעוץ) | דרג ההזרקה של פריטים שמסומנים `always: true`: מסופקים במלואם בתחילת כל סשן. ‏‎`mycontext review promote <id> --always`‎ היא הדרך היחידה להיכנס אליו כיום |
| **rationale** (נימוקים) | הדרג של הסיבה שהפרויקט הוא כפי שהוא: החלטות, ADRים, לקחים, פשרות, הנחות, מקרי קצה, סיכונים. מאונדקס, ניתן לחיפוש, נשלף לבקשה — לעולם לא מוזרק בלי שביקשו |
| **restored** (משוחזר) | דרג ההזרקה שנורה אחרי כיווץ ומספק מחדש את מה שהיה בהקשר לפניו |
| **scope glob** (גלוב היקף) | תבנית של נתיב קובץ על פריט, שנבדקת מול הקובץ ש-Claude עומד לגעת בו — `src/billing/**`. ‏‎`*`‎ נשאר בתוך רמת תיקייה אחת, ‎`**`‎ חוצה כמה שצריך. בלי היקף — לעולם לא מוזרק אוטומטית |
| **severity** (חומרה) | ‏`hard` או `soft`. זה משנה את סדר הקבלה לתקציב, ותו לא: קשיח קודם |
| **slash command** (פקודת סלאש) | משהו שאתה מקליד בתוך סשן Claude Code, באיות ‎`/mycontext:<name>`‎. שונה מפקודת שורת פקודה, שהיא ‎`mycontext <name>`‎ בטרמינל |
| **spill** (שפיכה) | מה שקורה לפריט שאינו נכנס לתקציב הדרג שלו: הוא מדולג, ונקוב בהערה מתחת להזרקה כדי שלעולם לא ייזרק בשקט. פריט קטן יותר אחריו עדיין יכול להתקבל |
...
| **watched docs** (מסמכים במעקב) | הגלובים שעריכה שלהם מייצרת שורת תזכורת אחת ללכוד את מה שהעריכה החליטה. מוגדרים תחת `watchedDocs`; הרשימה שאתה נותן מחליפה את ברירות המחדל |
