...
משלמת עד `jit` נוסף, פעם אחת לפריט בכל חלון הקשר (לכל תת-סוכן חלון משלו), מפני שיומן
ההזרקות אינו מוסר את אותו פריט פעמיים לאותו חלון. מול חלון הקשר של 200,000 טוקנים, עלות
...
קריאות הכלים שלו שנוגעות בקבצים מפעילות `PreToolUse` ככל האחרות. יומן ההזרקות ממפתח את
המסירות שלו על `session_id` בצירוף `agent_id`, כך שכל תת-סוכן הוא תחום דה-דופליקציה משל
...
לא קיבל *דבר* ממה שהסשן כבר ראה, בעוד היומן טוען למסירה — בדיוק מצב הכיסוי-הכוזב שהפרק
