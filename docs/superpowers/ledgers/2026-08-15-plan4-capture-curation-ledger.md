...
Task 16: complete (commits 400b881..<this round>, review clean after 1 fix round). 1177 tests.
The user's own five requirements, not a plan brief: the generated command surface (S2), the rename to
`mycontext`, `--yes=false`, tabular output with detail levels, and Task 9's approval-gate escalation.

**🔴 REQUIREMENT 5's ANSWER IS "A PLUGIN CANNOT DO THIS", VERIFIED FROM THE DOCS.** A Claude Code plugin's
own `settings.json` supports **only** the `agent` and `subagentStatusLine` keys — there is no
plugin-shippable `permissions` block — so this repository cannot ship a deny rule for
`Bash(mycontext lesson-accept*)` on the user's behalf. What shipped instead is the honest statement in
three places (`SKILL.md`, README, the `workflow` help topic), each pinned by an assertion, plus the deny
rules for the user's *own* `.claude/settings.json` **and what they do not cover**: Bash rules match the
command string, and `node …/src/cli/index.ts lesson-accept`, `npx`, `devbox run` and similar are different
strings that are **not** denied (the built-in wrapper-stripping list covers `timeout`/`nice`/`xargs`, not
those). Task 9's statement therefore stands unchanged and is now written down: **the gate holds if and only
if the agent's Bash surface excludes those commands.** Nothing anywhere describes `--yes` as protection.
- **Follow-up, and it may flip this answer: the plugin ALREADY ships a deny mechanism it did not consider.**
  `hooks/hooks.json` registers a `PreToolUse` hook and `pre-tool-use.ts` emits `permissionDecision: 'deny'`
  today. Adding `Bash` to that matcher and denying command strings **is** a plugin-shippable enforcement
  path. The counter-arguments are real — it inherits the same different-spelling hole, and
  `INV-hooks-fail-open` constrains what a hook may do — but that analysis was never performed. The README's
  "this repository cannot close the gap on your behalf" is narrowly true of plugin settings and **over-broad
  as written**. Recorded, not fixed.

**Two of the task's own premises were wrong, and the docs were checked rather than assumed:**
1. **`$1`/`$2` are not 1-based positionals.** The current docs define `$N` as `$ARGUMENTS[N]`, **zero-based**
   — "`$0` for the first argument or `$1` for the second". The 1-based reading is the older behaviour. The
   generated surface therefore uses `$ARGUMENTS` **only**, enforced by a test; a file that guessed wrong
   would have captured the wrong words silently, which is this project's signature failure. (Both templates
   want the whole string anyway, so it cost nothing.)
2. **`disable-model-invocation: true` was not verifiable as "keeps the description out of context"** from
   the page consulted; the flag is documented as preventing automatic loading and subagent preloading. The
   flag was used (correct for a user-only surface) and the code comment records exactly what is and is not
   established rather than restating the brief. A separate documentation query later returned *"Description
   not in context, full skill loads when you invoke"*, so the 37 descriptions most likely cost nothing —
   but the refusal to claim it before it was verified is the behaviour to keep. Fallback if a live session
   says otherwise: collapse to a generic `/mycontext:add <type> <text>`, a small edit to the generator.
Also verified and acted on: plugin commands **are** namespaced `<plugin.json name>:<command>` (hence the
rename), and `plugin.json`'s `commands` field **replaces** the default `commands/` scan rather than
supplementing it — so no such field was added, and a test asserts its absence. The locally installed
`plugin-dev` marketplace skill states the opposite on both points and was ignored where it disagreed.

**🔴 FOUR REPEAT DEFECTS IN ONE TASK, and all four were found by RUNNING THE OUTPUT AGAINST THIS REPO'S
REAL CORPUS AFTER EVERY TEST PASSED.** That practice — the S1 instinct applied to a screen rather than to a
capture — is the finding. Three were caught by the implementer doing it unprompted, the fourth by the
reviewer doing it again:
1. `status --full` printed "usage: no sessions recorded yet" and then **25 rows under a `cold id` header** —
   every scoped item in the corpus, `CONST-zero-runtime-dependencies` and `RULE-erasable-syntax-only`
   included, rendered as a decay list one line after the report said it had measured nothing. Task 13's
   defect (a list that reads as a recommendation the data cannot support) **and** Task 15's (a number
   asserted over a ledger that does not hold it), in a section written after both.
2. `decay` hedged an empty ledger with *"(only 0 session(s) recorded so far, so cold mostly means new)"* —
   a caveat on a signal, where there was no signal. Zero sessions now says so in its own words. And *"do not
   supersede anything **below on this list**"* pointed at nothing under `--summary`.
3. The usage banner's `padEnd(28)` ran every reporting command's usage string into its summary
   (`[--json]counts, review queue…`) — **the same collision `col` exists to prevent**, in the same commit
   that promoted `col` into a shared helper for exactly that reason.
4. (Reviewer) `decay --full` rendered a **pinned** item's scope as `(none)` — 7 of the 25 cold rows — in a
   report whose own summary said `unscoped 0` and whose own module defines unscoped as "no scope **and no
   pin**". `list --full` renders the identical field as `always`, so **two commands in one release disagreed
   about the same value**, and the obvious action the `(none)` invited ("give it a scope or delete it") was
   wrong for precisely the items CI enforces. `DecayRow` now carries `always`; the same category error in
   `status`'s "N **scoped** item(s) have never been injected" is fixed to "injectable (scoped or pinned)".
   The `scope` column was new and had no test; it has two now, one of which asserts the two commands agree.

**What the review verified independently, and is worth trusting later:** 38 command files for exactly the 17
enabled categories and none for the disabled three; the drift pin genuinely fails when a category is flipped
without regenerating; all nine `hasFlag` call sites safe now that it can throw; no column collision on the
63-character ids at any level; every `--json` parses; and **no number contradicts another command** —
`status --json` agrees with `decay --json`, `list --summary`, `review list` and `doctor`. That last property
is the one Task 15 failed, and it held here.

**27 mutants, 27 killed**, including every doc-boundary sentence (deleting the SKILL.md section, softening
the README's "they do not make one impossible", flipping the help topic's "not by enforcement"), the
generated-set-equals-enabled-set pin in both directions, and `--yes=false` end to end. One structural
weakness named rather than hidden: the semantic command-file tests read the **committed files**, so they
only bite once someone regenerates; byte-identity is what catches an unregenerated template change, and the
`the generated set is exactly the enabled set` test derives both sides from one config object, so it cannot
catch a **config** change on its own. The pair covers it; neither half does.

**Task 16 follow-ups (recorded, not fixed):** the `PreToolUse`-hook deny path above; `package-lock.json`
still says `"name": "my-context"` (cosmetic, `npm ci` passes, but the new identity test does not cover the
lockfile and the report's "every remaining hit is a path or a test" claim was wrong); `--yes=false` on an
**interactive** terminal falls through to the prompt instead of refusing outright; no end-to-end test
asserts `--json=false` prints text (only a unit test); and the generated commands carry no `allowed-tools`,
so `list-*`/`review`/`status` prompt for Bash the first time — a rule was skipped deliberately rather than
shipping one that might silently fail to match a `${CLAUDE_PLUGIN_ROOT}`-substituted path.
