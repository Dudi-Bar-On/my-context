...
than a table inside it — as `inject.ts:260-262` claims.
...
(`src/hooks/pre-tool-use.ts:252-273`):
...
| full note, `cross-layer duplicate id(s):` (`inject.ts:320`) | 123 | **178 — budget unreachable** |
| full note, `SNAPSHOT WRITE FAILED (<msg>)` (`pre-compact.ts:90`) | 123 | **209 — budget unreachable** |
...
```
src/hooks/pre-tool-use.ts:277   recordAudit(…, sessionId, …)      ← raw session_id
src/hooks/pre-tool-use.ts:302   appendSeen(root, dedupeKey, …)    ← session_id + "__" + agent_id
```

`ledgerKey` (`src/hooks/io.ts:29-45`) appends `agent_id` when present, because a subagent begins with
...
> is keyed on `session_id` + `agent_id`** (`ledgerKey`, `io.ts:29`), because dedupe is per context
...
**5.2 §0.5 of the same document** flagged that `src/core/audit.ts:532` cited a
