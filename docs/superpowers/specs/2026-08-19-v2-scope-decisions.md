...
> `status === 'draft' && layer === 'project'` (`src/core/select.ts` · `i.status === 'draft' && i.layer === 'project'` · ~914) and a rationale item is
...
(`src/core/categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~202) — and
...
> id**, and `src/core/focus.ts` · `What focus is scoped to: the WORKSPACE, not the session` · ~22 records the codebase hitting exactly this and conceding it —
...
1. **It amends the one decision it claimed to protect.** The original argument was that
   ...
   untouched. That decision's subject is not `select()`. Verbatim, `src/core/search.ts` · `The corpus filter behind BOTH` · ~151: *"The
...
2. **The motivating query missed on field coverage, not tokenisation.** The predicate **was**
   ...
   **The one-line widening under "Taken instead" below shipped 2026-08-19** and this reason is now
   ...
   every `extra` value (`src/core/search.ts` · `function searchableText(item: Item): string {` · ~225), and its docblock records exactly this argument.
...
> `src/core/focus.ts` · `What focus is scoped to: the WORKSPACE, not the session` · ~22 records the codebase already hitting and conceding, and which §6j sets
...
> `/^#{1,6}\s/` inside a body (`src/core/validate.ts` · `export function validateBody(body: string): void {` · ~513), so a `## Steps` section
...
- **`UPDATE_FIELD_POLICY` is untouched.** `FieldPolicy` stays `'content' | 'gated'`, and the four
  ...
  (`src/core/trust.ts` · `export const UPDATE_FIELD_POLICY = {` · ~456). The original "third thing — progress, neither content nor gated"
...
categories are *enabled* (`core/config.ts` · `const enabledByProfile = new Set(PROFILES[profile]);` · ~1499, read at `core/config.ts` · `enabled: enabledByProfile.has(def.name),` · ~1519); the `categories` block **also** carries `tier` and
`agentEdits`, and a `tier` override drags `agentEdits` with it (`core/config.ts` · `} else if (override.tier !== undefined) {` · ~1651). A pack
...
import (`core/config.ts` · `if (raw === undefined) return { ...DEFAULT_BUDGETS };` · ~1378 and `core/config.ts` · `if (raw === undefined) return [...DEFAULT_WATCHED_DOCS];` · ~1424), doing precisely what the next paragraph forbids, through the mechanism
...
never-injected, never-indexed and no draft gate through `isNormative` (`src/core/select.ts` · `function isNormative(item: Item, config: Config): boolean {` · ~513).
...
`validateBody` refuses any heading line (`src/core/validate.ts` · `export function validateBody(body: string): void {` · ~513), so steps must be a
...
`isEligible` admits exactly `'active'` (`src/core/select.ts` · `export function isEligible(item: Item, config: Config): boolean {` · ~495); everything else is `draft` or a
...
It is a **category** lookup — `isNormative(item, config)` (`src/core/select.ts` · `function isNormative(item: Item, config: Config): boolean {` · ~513), applied by `select()` for the full-text half (`src/core/select.ts` · `const injectable = eligible.filter((i) => isNormative(i, config));` · ~1378) and by `buildIndex` for the index half (`src/core/select.ts` · `.filter((i) => isNormative(i, config) && !chosenIds.has(i.id))` · ~1001).
...
`UPDATE_FIELD_POLICY` (`src/core/trust.ts` · `export const UPDATE_FIELD_POLICY = {` · ~456) types every updatable field as
...
(`src/core/vocabulary.ts` · `export const RELATION_TYPES = [` · ~102) is a closed vocabulary and **`derived_from` is the only honest
...
`src/core/focus.ts` · `What focus is scoped to: the WORKSPACE, not the session` · ~22 records the same problem being hit before: **no surface that can *set*
...
`pruneSnapshots` (`ledger.ts` · `export function pruneSnapshots(` · ~814) is age-based — 30 days by mtime — and when this was written its **only production
caller was `cmdRebuild`** (`cli/index.ts` · `function cmdRebuild(ws: Workspace, out: Emit): number {` · ~1419), with no hook pruning and no `clearSeen` anywhere.
**Both halves have since closed.** A hook prunes: `SessionStart` sweeps `state/` once per session, after the write to stdout (`hooks/session-start.ts` · `function sweepStaleState(cwd: string): void {` · ~163) — its docblock carries this finding's own measurement, "15 files one day and 47 the next". And `clearSeen` exists (`core/seen-file.ts` · `export function clearSeen(root: string, sessionId: string): ClearSeenReport {` · ~464), so pruning is no longer age-only.
...
`parseAudit` (`core/audit.ts` · `export function parseAudit(raw: string, file: string, windowed = false): AuditRecord[] {` · ~1574) **refuses unregistered ops**. Any new hook op must be added to
...
`src/core/audit.ts` · `**Gitignored, and the consequence is disclosed rather than left to be` · ~57, and **both READMEs**. §5 reverses that on the owner's explicit
...
the text of discarded proposals** (`src/core/revision.ts` · `export function revisionHistory(ctx: MutationContext, itemId: string): RevisionRecord[] {` · ~526) — an export built as a deny-list
...
joined it 2026-08-20 and `'progress'` 2026-08-21** (`src/core/audit.ts` · `export const AUDIT_KINDS: AuditKind[] = [` · ~647). It is stored per
record, derived from one total `KIND_OF` table (`src/core/audit.ts` · `const KIND_OF: Record<AuditOp, AuditKind> = {` · ~652), validated on read (`src/core/audit.ts` · `!AUDIT_KINDS.includes(row.kind as AuditKind)` · ~1342), with
`filterAudit(…, {kind})` (`src/core/audit.ts` · `export function filterAudit(records: AuditRecord[], filter: AuditFilter): AuditRecord[] {` · ~1639) already the shared filter. `MUTATION_OPS` (`src/core/audit.ts` · `export const MUTATION_OPS = [` · ~180) still matches §5's
...
the only two writers (`persist.ts` · `export function auditMutation(` · ~420, and the discard settlement at `revision.ts` · `const audited = auditFailureNote(recordAudit(ctx.root, {` · ~940) have no parameter for either. The type
...
(`core/config.ts` · `const enabledByProfile = new Set(PROFILES[profile]);` · ~1499, read at `core/config.ts` · `enabled: enabledByProfile.has(def.name),` · ~1519). The `categories` block **also** carries `tier`, `agentEdits`, `scopePolicy`
and `prefix` — and a `tier` override drags `agentEdits` with it (`core/config.ts` · `} else if (override.tier !== undefined) {` · ~1651).
...
2. every existing `rule` **stops being injected at all** — `isNormative` (`select.ts` · `function isNormative(item: Item, config: Config): boolean {` · ~513).
...
on import (`core/config.ts` · `if (raw === undefined) return { ...DEFAULT_BUDGETS };` · ~1378 and `core/config.ts` · `if (raw === undefined) return [...DEFAULT_WATCHED_DOCS];` · ~1424). §6h's own words: *"a pack that silently changed how much context
...
`src/core/search.ts` · `The corpus filter behind BOTH` · ~151:
...
`Status` has five members (`types.ts` · `export type Status = 'active' | 'draft' | 'superseded' | 'deprecated' | 'validated';` · ~2). `ready` cannot produce an index line — `buildIndex` (`select.ts` · `function buildIndex(` · ~991)
enumerates only `eligible`, and `isEligible` requires `active` (`select.ts` · `export function isEligible(item: Item, config: Config): boolean {` · ~495). `done` is
...
taken"* (`categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~202). The scan adds the decisive point: **the owner was naming the
...
- **F9** — `todo` can never reach the review queue: the queue is `status === 'draft'`
  (`select.ts` · `i.status === 'draft' && i.layer === 'project'` · ~914) and a rationale item is never forced to draft. §1's own §1.1 says "no draft
...
(`categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~202). R11b's own words were *"runbook (or to call it with different name)"*: the
...
1. `src/core/search.ts` · `The corpus filter behind BOTH` · ~151 — the recorded decision's subject is *"BOTH `query_items` … and `mycontext
...
is in — which `core/focus.ts` · `What focus is scoped to: the WORKSPACE, not the session` · ~22 records as unknowable.
...
(`src/core/config.ts` · `if (!override.tier || !override.description) {` · ~1548), so §6h and §6m.12's premise that pack-defined categories work could
never hold.
...
(`src/cli/commands/review.ts` · `const patch: UpdateInput = { id: item.id, status: 'active', origin: 'human' };` · ~1131), after a confirmation — and the export-and-packs plan defends
...
(`src/hooks/pre-tool-use.ts` · `is managed by my_context and must not be written` · ~117). A toggle command is not a door into the managed directory, and
...
> `{ profile: 'standard', categories: {}, budgets: {} }` (`src/cli/index.ts` · `const INIT_CONFIG = { profile: 'standard', categories: {}, budgets: {} } as const;` · ~150). What R14.1
...
**Shipped 2026-08-20, exactly as ruled.** `'ui'` is on the list (`src/core/config.ts` · `export const TOP_LEVEL_KEYS = [` · ~1025), and an unrecognised
top-level key is collected and disclosed rather than refused (`src/core/config.ts` · `const skippedKeys = Object.keys(input)` · ~1487). The paragraphs
...
symptom was items quietly missing from sessions"* (`src/core/config.ts` · `the only symptom was items quietly missing from sessions.` · ~1009). That stays exactly
