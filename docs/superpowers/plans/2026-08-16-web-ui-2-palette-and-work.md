...
| Fact | Where verified |
|---|---|
| `promoteRevision` applies through `updateItem` with `origin: 'human'` | `core/revision.ts` · `export function promoteRevision(` · ~783 |
| `RevisionRecord { revisionId; itemId; changes; base; origin; stagedAt; state; settledAt; reason }` | `core/revision-log.ts` · `export interface RevisionRecord {` · ~56 |
...
| `REVISION_FIELDS = ['title','body','tags','extra']` — **in `revision-log.ts` since Task 1 executed; `revision.ts` re-exports it** | `core/revision-log.ts` · `export const REVISION_FIELDS = ['title', 'body', 'summary', 'tags', 'extra'] as const;` · ~291 |
...
| **`changedFields` — `export`ed, formerly `fieldsOf`; two consumers**. `cli/commands/review.ts` imports it from `core/revision.ts`, which re-exports it since Task 1 moved it; `cli/commands/revision-view.ts` imports it from `core/revision-log.ts` directly | `core/revision-log.ts` · `export function changedFields(changes: RevisionChanges): RevisionField[] {` · ~376 |
| `decorate(ctx, record)` computes `current`/`changedSince`/`stale`/`itemMissing` | `core/revision.ts` · `function decorate(ctx: RevisionViewContext, record: RevisionRecord): PendingRevision {` · ~366 |
| `foldLog` is terminal-state folding; a settled revision never comes back pending | `core/revision-log.ts` · `export function foldLog(lines: LogLine[]): RevisionRecord[] {` · ~174 |
| `staleRefusal` names the moved fields and both values — **exported** | `core/revision.ts` · `export function staleRefusal(itemId: string, pending: PendingRevision): string {` · ~747 |
| `missingItemRefusal` names the gone item — **exported** | `core/revision.ts` · `export function missingItemRefusal(` · ~736 |
...
| `resolveConfig` throws on unknown profile, unknown category key, invalid prefix, invalid enum, invalid shape | `core/config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1071 |
| `AGENT_EDITS = ['allow','review']` — declaration order is user-facing | `core/config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~126 |
| `SCOPE_POLICIES = ['global','required','inert']` | `core/config.ts` · `export const SCOPE_POLICIES: ScopePolicy[] = ['global', 'required', 'inert'];` · ~127 |
...
| `agentEditsFor(config,type)` | `core/config.ts` · `export function agentEditsFor(config: Config, type: string): AgentEdits {` · ~201 |
| `scopePolicyFor(config,type)` | `core/config.ts` · `export function scopePolicyFor(config: Config, type: string): ScopePolicy {` · ~179 |
| `PROFILES` imported from `core/categories.ts` | `core/config.ts` · `  CATEGORIES, PROFILES,` · ~10 |
| Config file is `<projectRoot>/config.json` | `core/workspace.ts` · `const configPath = projectRoot ? path.join(projectRoot, 'config.json') : null;` · ~49 |
...
| **`STATUSES` lives in `core/validate.ts`** — whose graph reaches no mutator | `core/validate.ts` · `export const STATUSES: Status[] = ['active', 'draft', 'superseded', 'deprecated', 'validated'];` · ~23 |
...
| `mycontext search` already validates against the **moved** `STATUSES` | `cli/commands/search.ts` · `import { STATUSES } from '../../core/validate.ts';` · ~5 |
...
1. **`decorate` has not been a store lookup since 2026-08-16** — `c4fbd51` gave it `itemNow`, which
   ...
   is the SessionStart injection path: `core/inject.ts` · `        pendingRevisions({ root: ws.projectRoot ?? stateRoot, store: null, items, config: ws.config }),` · ~516
...
1. **Plan-2 server code lives in two modules, both inside the no-writes graph.** `src/ui/read-model-work.ts` and `src/ui/read-model-config.ts` are imported by `server.ts` (each exports a `register…Routes()` function), so `test/ui/no-writes.test.ts` covers them automatically. Neither may import `revision.ts`, `mutate.ts`, or `cli/commands/*` (the CLI command modules import `mutate.ts` and each other freely — e.g. `cli/commands/edit.ts` · `import { updateItem, type MutationContext, type UpdateInput } from '../../core/mutate.ts';` · ~5). `cli/commands/search.ts` was the example here until §0's `RELATION_TYPES` row: that import names `core/vocabulary.ts` now, and `search.ts` reaches a mutator through nothing but `./context.ts`'s type-only `MutationContext`.
2. **One staleness implementation.** The per-field staleness decoration is in `revision-log.ts` as `decoratePending(record, item)` (`core/revision-log.ts` · `export function decoratePending(record: RevisionRecord, item: Item | null): PendingRevision {` · ~401) — Task 1 executed 2026-08-21 and moved it, so this is a fact now rather than an intention. Its store dependency was never the single `ctx.store.get(id)` this line claimed: the lookup is `itemNow` (`core/revision.ts` · `function itemNow(ctx: RevisionViewContext, id: string): Item | null {` · ~350), which reads the store when there is one and the parsed corpus when there is not, so it stayed behind and the item — not the store — is the parameter. `revision.ts` calls the moved function (`core/revision.ts` · `function decorate(ctx: RevisionViewContext, record: RevisionRecord): PendingRevision {` · ~366); behaviour changes were none. See *Task 1 executed, 2026-08-21* above for the four things this plan said about that move that were wrong. Plan 1's Task 6 explicitly reserved this boundary for plan 2: "anything it needs beyond counts — decorated revisions, staleness — is plan 2's problem to solve on this same boundary."
...
11. **The glob tester composes `matchesAnyGlob` (`paths.ts` · `export function matchesAnyGlob(subject: string, patterns: string[]): boolean {` · ~70), deliberately.** Its question is "which files match this pattern" — a question about a *pattern*, which is exactly what `matchesAnyGlob` answers. The defect `matchesScope`'s comment documents is using it to answer "which items govern this file" (`select.ts` · `matchesAnyGlob(path, item.scope)` · ~245 — `query_items` re-derived it that way and kept hiding unscoped items from a path query); that question stays with `matchesScope`/`injection()` everywhere in this plan too.
...
**Interfaces:**
- Consumes: `filterItems` (`core/search.ts` · `export function filterItems(items: Item[], filters: ItemFilters, config: Config): Item[] {` · ~70), `anyFilterSet` (`core/search.ts` · `export function anyFilterSet(filters: ItemFilters): boolean {` · ~89), `matchesAnyGlob` (`core/paths.ts` · `export function matchesAnyGlob(subject: string, patterns: string[]): boolean {` · ~70), `listRepoFiles` (`doctor/checks.ts` · `export function listRepoFiles(repoRoot: string, limit: number = FILE_LIMIT): string[] {` · ~74), `injection`.
- Produces:
  - `apiSearch(ws, url): JsonResult` — `GET /api/search?text=&type=&tag=&path=&status=&relation=&limit=` → `{ items: { id; type; title; status; always; scope; injected; phrase }[]; total: number; truncated: boolean }`. The predicate is `filterItems` — the ONE filter behind `query_items` and `mycontext search` (`core/search.ts` · `The corpus filter behind BOTH` · ~7); this endpoint is its third caller, not a fourth spelling. At least one filter required (`anyFilterSet`, mirroring `cli/commands/search.ts` · `At least one filter is required. To list the whole corpus, that is` · ~63); default limit 50 (`cli/commands/search.ts` · `const DEFAULT_LIMIT = 50;` · ~68), truncation reported, never silent; `status`/`relation` validated against the mirrored enum lists below; `type` validated against `Object.keys(ws.config.categories)` (`Object.hasOwn`-safe: the map is null-prototype, `core/config.ts` · `const categories: Record<string, ResolvedCategory> = Object.create(null);` · ~1140).
  - `apiGlob(ws, url): JsonResult` — `GET /api/glob?pattern=a/**,b/**` → `{ patterns: string[]; total: number; sample: string[]; fileWalkTruncated: boolean }`. `pattern` is comma-separated exactly as `--scope` takes it (`cli/index.ts` · `[--scope "a/**,b/**"] [--tags "a,b"] ` · ~454); matching is `matchesAnyGlob(file, patterns)` over `listRepoFiles(repoRoot)`; `sample` is the first 200 matches with `total` the real count. **This is the one legitimate `matchesAnyGlob` call in the UI**: the question is "which files match this pattern" — a question about a pattern the user is composing, not about which items govern a file. The govern question stays with `matchesScope`/`injection()` (the defect `matchesScope`'s own comment names — `select.ts` · `matchesAnyGlob(path, item.scope)` · ~245), and the module comment says so at the call site.
...
**Interfaces:**
- Consumes: `resolveConfig` (`core/config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1071), `DEFAULT_BUDGETS` (`core/config.ts` · `export const DEFAULT_BUDGETS: Budgets = { pinned: 6000, jit: 6000, restored: 8000, index: 1200 };` · ~67), `AGENT_EDITS` (`core/config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~126), `SCOPE_POLICIES` (`core/config.ts` · `export const SCOPE_POLICIES: ScopePolicy[] = ['global', 'required', 'inert'];` · ~127), `PROFILES` (`core/categories.ts`), `node:fs` `readFileSync`/`existsSync`. <!-- historical-citation: quotes DEFAULT_BUDGETS as it was before the fifth budget, `continuity: 2000` (plan:live seq:9) -->
- Produces:
  ...
  - `apiConfigCheck(ws, url, body): JsonResult` — `POST /api/config/check` with body `{ candidate: unknown }` →
    ...
    - `dropped` is the editor's own strictness, labelled as such (Design decision 9) — **and the three cases this bullet was written around are refusals now, not silences.** An invalid `budgets` value is refused by name (`core/config.ts` · `function requireBudgets(raw: unknown): Budgets {` · ~1004) and a non-string `watchedDocs` entry is refused rather than filtered (`core/config.ts` · `function requireWatchedDocs(raw: unknown): string[] {` · ~1050), so both arrive as `ok: false` with the thrown message, not as a finding. The top level reads five keys, `ui` included (`core/config.ts` · `export const TOP_LEVEL_KEYS = [` · ~799), and an unknown one is skipped **and disclosed** on the resolved config (`core/config.ts` · `  skippedKeys: string[];` · ~352). So `dropped` carries `skippedKeys` — one finding per skipped key, e.g. `{ where: 'budget', message: 'not a key this build reads — nothing under it was loaded' }` — and nothing else until a later loader change opens a new silence. A config editor that let the loader's silences through would be INV-nothing-is-dropped-silently violated in the one screen built to prevent it; the loader having closed them is why the list is now short, not why it is gone.
...
- Produces (the palette screen renders this catalogue; nothing else defines a command):
  ...
  - The catalogue itself, one entry per verified command — the source citations are in this plan's Verified Facts table and each def carries the flag set exactly as the CLI declares it:
    - **write**: `add` (args category+title; flags body/file/note/**step**/scope/tags/severity/**extra**/yes — `cli/index.ts` · `const ADD_USAGE =` · ~452; **`--step` and `--extra` landed after this list was written** and belong in it, because the sentence above says a def carries the flag set exactly as the CLI declares it), `edit` (arg id; flags title/body/scope/tags/severity/always/status/extra/yes — `cli/commands/edit.ts` · `usage: 'edit <id> [--title|--body|--scope|--tags|--severity|--always|--status|--extra]',` · ~956), `pin`/`unpin`/`harden`/`soften` (arg id; flag yes — `cli/commands/edit.ts` · ``usage: `${entry.name} <id> [--yes]`,`` · ~1107), `supersede` (args id + `--by` id — `cli/commands/supersede.ts` · `usage: 'supersede <id> --by <id>',` · ~165), `refresh` (arg id — `cli/commands/refresh.ts` · `usage: 'refresh <id>',` · ~161), `repair` (flag yes — `cli/commands/repair.ts` · `usage: 'repair [--yes]',` · ~197), `lesson-accept` / `lesson-discard` (args id+key — `cli/commands/lesson.ts` · `usage: 'lesson-accept <id> <key>',` · ~449 and `cli/commands/lesson.ts` · `usage: 'lesson-discard <id> <key>',` · ~456), `review promote` (arg id; flags scope/always/severity/yes — `cli/commands/review.ts` · `mycontext review promote <id> [--scope "a/**,b/**"] [--always] [--severity hard|soft] [--yes]` · ~90), `review discard` (arg id; flag yes — `:85`), `review promote-revision` (arg id; flags revision/force/yes — `:87`), `review discard-revision` (arg id; flags revision/reason/yes — `:88`), and `rebuild` (no args — `cli/index.ts` · `usage: 'rebuild',` · ~1174; classified write because it rewrites `.index.db` on disk, Design decision 12). <!-- historical-citation: quotes edit usage before `--continuity` joined it (plan:live seq:9) -->
