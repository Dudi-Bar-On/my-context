...
| Fact | Where verified |
|---|---|
| `promoteRevision` applies through `updateItem` with `origin: 'human'` | `core/revision.ts` · `export function promoteRevision(` · ~812 |
| `RevisionRecord { revisionId; itemId; changes; base; origin; stagedAt; state; settledAt; reason }` | `core/revision-log.ts` · `export interface RevisionRecord {` · ~69 |
| `PendingRevision extends RevisionRecord` adds `current`, `changedSince`, `stale`, `itemMissing` — **in `revision-log.ts` since Task 1 executed; `revision.ts` re-exports it** | `core/revision-log.ts` · `export interface PendingRevision extends RevisionRecord {` · ~313 |
| `REVISION_FIELDS = ['title','body','tags','extra']` — **in `revision-log.ts` since Task 1 executed; `revision.ts` re-exports it** | `core/revision-log.ts` · `export const REVISION_FIELDS = ['title', 'body', 'summary', 'tags', 'extra'] as const;` · ~304 |
| `canonicalValue` — was private to `revision.ts`; **Task 1 moved it and had to export it**, because `revisionIdFor` stayed behind and imports it back | `core/revision-log.ts` · `export function canonicalValue(value: RevisionValue): unknown {` · ~339 |
| `sameValue` — was private to `revision.ts`; **Task 1 moved it and had to export it**, because `normalizeChanges` stayed behind and imports it back | `core/revision-log.ts` · `export function sameValue(a: RevisionValue \| undefined, b: RevisionValue \| undefined): boolean {` · ~350 |
| `valuesOf` — was private to `revision.ts`; **Task 1 moved it and had to export it**, because `stageRevision` stayed behind and imports it back | `core/revision-log.ts` · `export function valuesOf(item: Item, changes: RevisionChanges): RevisionChanges {` · ~365 |
| **`changedFields` — `export`ed, formerly `fieldsOf`; two consumers**. `cli/commands/review.ts` imports it from `core/revision.ts`, which re-exports it since Task 1 moved it; `cli/commands/revision-view.ts` imports it from `core/revision-log.ts` directly | `core/revision-log.ts` · `export function changedFields(changes: RevisionChanges): RevisionField[] {` · ~394 |
| `decorate(ctx, record)` computes `current`/`changedSince`/`stale`/`itemMissing` | `core/revision.ts` · `function decorate(ctx: RevisionViewContext, record: RevisionRecord): PendingRevision {` · ~395 |
| `foldLog` is terminal-state folding; a settled revision never comes back pending | `core/revision-log.ts` · `export function foldLog(lines: LogLine[]): RevisionRecord[] {` · ~187 |
| `staleRefusal` names the moved fields and both values — **exported** | `core/revision.ts` · `export function staleRefusal(itemId: string, pending: PendingRevision): string {` · ~776 |
| `missingItemRefusal` names the gone item — **exported** | `core/revision.ts` · `export function missingItemRefusal(` · ~765 |
...
| `mycontext review` subcommands | `cli/commands/review.ts` · `'list', 'show', 'promote', 'discard', 'revisions', 'promote-revision', 'discard-revision',` · ~41 |
...
| `fieldDiff` | `cli/commands/revision-view.ts` · `export function fieldDiff(` · ~66 |
...
| `revision-view.ts` value-imported from `revision.ts` when this row was written, pulling `mutate.ts` into a runtime graph — **it no longer does: its value import is `changedFields` from `core/revision-log.ts`, and it imports nothing at all from `revision.ts`** | `cli/commands/revision-view.ts` · `  changedFields,` · ~3 |
| `resolveConfig` throws on unknown profile, unknown category key, invalid prefix, invalid enum, invalid shape | `core/config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1444 |
| `AGENT_EDITS = ['allow','review']` — declaration order is user-facing | `core/config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~154 |
| `SCOPE_POLICIES = ['global','required','inert']` | `core/config.ts` · `export const SCOPE_POLICIES: ScopePolicy[] = ['global', 'required', 'inert'];` · ~155 |
| `DEFAULT_BUDGETS = { pinned: 6000, jit: 6000, restored: 8000, index: 1200 }` | `core/config.ts` · `export const DEFAULT_BUDGETS: Budgets = { pinned: 6000, jit: 6000, restored: 8000, index: 1200 };` · ~67 <!-- historical-citation: quotes DEFAULT_BUDGETS as it was before the fifth budget, `continuity: 2000` (plan:live seq:9) --> |
| `agentEditsFor(config,type)` | `core/config.ts` · `export function agentEditsFor(config: Config, type: string): AgentEdits {` · ~229 |
| `scopePolicyFor(config,type)` | `core/config.ts` · `export function scopePolicyFor(config: Config, type: string): ScopePolicy {` · ~207 |
| `PROFILES` imported from `core/categories.ts` | `core/config.ts` · `  CATEGORIES, PROFILES,` · ~11 |
| Config file is `<projectRoot>/config.json` | `core/workspace.ts` · `const configPath = projectRoot ? path.join(projectRoot, 'config.json') : null;` · ~164 |
...
| `reviewQueue(items, type?)` — project-layer drafts | `core/select.ts` · `export function reviewQueue(items: Item[], type: string \| null = null): Item[] {` · ~912 |
...
1. **`decorate` has not been a store lookup since 2026-08-16** — `c4fbd51` gave it `itemNow`, which
   ...
   is the SessionStart injection path: `core/inject.ts` · `        pendingRevisions({ root: ws.projectRoot ?? stateRoot, store: null, items, config: ws.config }),` · ~620
...
3. **Step 1's `item()` factory omits `steps`,** which `Item` has required since procedures landed
   (`core/types.ts` · `  steps: Step[];` · ~261), so the test as printed does not compile. *Class: a
...
1. **Plan-2 server code lives in two modules, both inside the no-writes graph.** `src/ui/read-model-work.ts` and `src/ui/read-model-config.ts` are imported by `server.ts` (each exports a `register…Routes()` function), so `test/ui/no-writes.test.ts` covers them automatically. Neither may import `revision.ts`, `mutate.ts`, or `cli/commands/*` (the CLI command modules import `mutate.ts` and each other freely — e.g. `cli/commands/edit.ts` · `import { updateItem, type MutationContext, type UpdateInput } from '../../core/mutate.ts';` · ~6). `cli/commands/search.ts` was the example here until §0's `RELATION_TYPES` row: that import names `core/vocabulary.ts` now, and `search.ts` reaches a mutator through nothing but `./context.ts`'s type-only `MutationContext`.
2. **One staleness implementation.** The per-field staleness decoration is in `revision-log.ts` as `decoratePending(record, item)` (`core/revision-log.ts` · `export function decoratePending(record: RevisionRecord, item: Item | null): PendingRevision {` · ~419) — Task 1 executed 2026-08-21 and moved it, so this is a fact now rather than an intention. Its store dependency was never the single `ctx.store.get(id)` this line claimed: the lookup is `itemNow` (`core/revision.ts` · `function itemNow(ctx: RevisionViewContext, id: string): Item | null {` · ~379), which reads the store when there is one and the parsed corpus when there is not, so it stayed behind and the item — not the store — is the parameter. `revision.ts` calls the moved function (`core/revision.ts` · `function decorate(ctx: RevisionViewContext, record: RevisionRecord): PendingRevision {` · ~395); behaviour changes were none. See *Task 1 executed, 2026-08-21* above for the four things this plan said about that move that were wrong. Plan 1's Task 6 explicitly reserved this boundary for plan 2: "anything it needs beyond counts — decorated revisions, staleness — is plan 2's problem to solve on this same boundary."
3. **One diff implementation.** `lineDiff`, `DiffLine` and the private `linesOf` move from `cli/commands/revision-view.ts` to a new `src/core/revision-diff.ts` (with `linesOf` exported as `valueLines`); `revision-view.ts` imports them back. The server serves structured `DiffLine[]`s from the same LCS the terminal renders — not a browser re-implementation, which would be this project's most-repeated defect in a new medium. `revision-view.ts`'s one value import — `changedFields`, not `REVISION_FIELDS`, which it does not import — also retargets to `revision-log.ts`, so nothing the server imports reaches `revision.ts`.
...
5. **The composed settlement command always carries `--revision`.** Without it, `promote-revision`/`discard-revision` **refuse** as soon as the item has more than one pending revision (`pickPendingRevision`, `core/revision.ts` · `The default used to be "the oldest", and settlement is a write` · ~699). Settling the *oldest* was the default when this line was written; it was removed for this decision's own reason — the oldest may not be the one the human just read. The pasted line must settle exactly what was on screen, and a line without `--revision` now settles nothing rather than the wrong thing.
...
8. **`/api/config` reads `config.json` fresh from disk on every call.** The file is the user's to edit while the server runs (`pre-tool-use.ts` · ``changes to `.my_context/config.json` are the user`` · ~120 declares it so); an editor seeded from the server's boot-time `ws.config` would compose a diff against text no longer in the file. Unparseable JSON is reported as a field (`parseError`), not a 500 — `resolveWorkspace` throws on it (`workspace.ts` · `is not valid JSON: ` · ~154), so this endpoint reads the file itself.
9. **The editor's strictness beyond `resolveConfig` is labelled as the editor's — and `resolveConfig` has since closed all three silences this decision was built on.** It refuses enums, profiles, category keys and prefixes with exact wording, and the editor surfaces those refusals verbatim. It no longer *silently* ignores an invalid `budgets` value — that is refused by name (`core/config.ts` · `function requireBudgets(raw: unknown): Budgets {` · ~1377); no longer drops a non-string `watchedDocs` entry — that is refused rather than filtered (`core/config.ts` · `function requireWatchedDocs(raw: unknown): string[] {` · ~1423); and reads **five** top-level keys, not four, since `ui` joined them (`core/config.ts` · `export const TOP_LEVEL_KEYS = [` · ~1025). An unknown top-level key is not a silence either: R14.2 made it skipped **and disclosed**, carried on the resolved config as `skippedKeys` (`core/config.ts` · `const skippedKeys = Object.keys(input)` · ~1487) and worded once by `skippedKeyNotice` (`core/config.ts` · `export function skippedKeyNotice(config: Config): string {` · ~1752). So the `dropped` list stays in the response shape, still labelled the editor's own and never claimed to be a `resolveConfig` refusal — but what it carries is `skippedKeys`, surfaced in the UI's own words, and any silence a later loader change opens. It does not invent findings for the three cases above, which are now refusals.
...
11. **The glob tester composes `matchesAnyGlob` (`paths.ts` · `export function matchesAnyGlob(subject: string, patterns: string[]): boolean {` · ~70), deliberately.** Its question is "which files match this pattern" — a question about a *pattern*, which is exactly what `matchesAnyGlob` answers. The defect `matchesScope`'s comment documents is using it to answer "which items govern this file" (`select.ts` · `matchesAnyGlob(path, item.scope)` · ~554 — `query_items` re-derived it that way and kept hiding unscoped items from a path query); that question stays with `matchesScope`/`injection()` everywhere in this plan too.
...
**Why:** the review queue's whole justification is the diff (spec §2: "the diff is the capability; the approval is a paste"). The LCS diff exists (`lineDiff`, `revision-view.ts` · `export function lineDiff(from: string[], to: string[]): DiffLine[] {` · ~78) and must not be written a second time in the browser — but `revision-view.ts` still value-imported from `revision.ts` when this task was written, so the server could not import it. The value import is `changedFields`, not `REVISION_FIELDS` (`revision-view.ts` · `  changedFields,` · ~3): Task 1 moved both to `revision-log.ts` and `revision.ts` re-exports them. **This file has since been repointed at `core/revision-log.ts` directly, so the runtime edge to `mutate.ts` is gone rather than merely unchanged.** The diff and the value-to-lines rendering move to core; the CLI view imports them back. <!-- historical-citation: this paragraph quotes lineDiff where it stood BEFORE this task; Step 3 moved it to core/revision-diff.ts -->

**Files:**
- Create: `src/core/revision-diff.ts`
- Modify: `src/cli/commands/revision-view.ts` (delete the moved code; import it; retarget `changedFields` — the one value import, `REVISION_FIELDS` is not imported here — and the revision types to `revision-log.ts`)
...
**Interfaces:**
- Consumes: `pendingRevisionViews`, `pendingRevisionCounts`, `PendingRevision`, `RevisionField` (revision-log.ts, Task 1), `lineDiff`, `valueLines`, `DiffLine` (revision-diff.ts, Task 2), `reviewQueue` (`select.ts` · `export function reviewQueue(items: Item[], type: string | null = null): Item[] {` · ~912), `injection` (`cli/commands/injection.ts` · `export function injection(` · ~84), plan 1's `withStores`, `badRequest`, `unknownParams` (`read-model.ts`), `registerRoute` / `ApiContext` / `JsonResult` (`routes.ts`).
- Produces:
...
test('/api/revisions: a staged revision arrives as a per-field diff; a human edit marks it stale', () => {
  ...
  try {
    ...
    // real stageRevision (read its input shape at `core/revision.ts` · `export function stageRevision(` · ~606 when
...
**Interfaces:**
- Consumes: `filterItems` (`core/search.ts` · `export function filterItems(items: Item[], filters: ItemFilters, config: Config): Item[] {` · ~235), `anyFilterSet` (`core/search.ts` · `export function anyFilterSet(filters: ItemFilters): boolean {` · ~272), `matchesAnyGlob` (`core/paths.ts` · `export function matchesAnyGlob(subject: string, patterns: string[]): boolean {` · ~70), `listRepoFiles` (`doctor/repo-files.ts` · `export function listRepoFiles(repoRoot: string, limit: number = FILE_LIMIT): string[] {` · ~125), `injection`.
- Produces:
...
const SEARCH_DEFAULT_LIMIT = 50; // the CLI's own cap (`cli/commands/search.ts` · `const DEFAULT_LIMIT = 50;` · ~73)
...
export function apiGlob(ws: Workspace, url: URL): JsonResult {
  ...
  // with matchesScope/injection() (`core/select.ts` · `and consequently kept hiding unscoped` · ~554 names the difference).
  ...
  return {
    ...
    body: {
      ...
      fileWalkTruncated: files.length >= 20_000, // listRepoFiles' own bound (`doctor/repo-files.ts` · `export const FILE_LIMIT = 20_000;` · ~59)
    },
  };
}
```
...
**Interfaces:**
- Consumes: `resolveConfig` (`core/config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1444), `DEFAULT_BUDGETS` (`core/config.ts` · `export const DEFAULT_BUDGETS: Budgets = { pinned: 6000, jit: 6000, restored: 8000, index: 1200 };` · ~67), `AGENT_EDITS` (`core/config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~154), `SCOPE_POLICIES` (`core/config.ts` · `export const SCOPE_POLICIES: ScopePolicy[] = ['global', 'required', 'inert'];` · ~155), `PROFILES` (`core/categories.ts`), `node:fs` `readFileSync`/`existsSync`. <!-- historical-citation: quotes DEFAULT_BUDGETS as it was before the fifth budget, `continuity: 2000` (plan:live seq:9) -->
- Produces:
  ...
  - `apiConfigGet(ws, url): JsonResult` — `GET /api/config` →
    ...
    ```ts
    {
      ...
      meta: {                        // what the editor's controls are built from
        ...
        agentEdits: string[];        // AGENT_EDITS — declaration order is user-facing (`core/config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~154)
  ...
  - `apiConfigCheck(ws, url, body): JsonResult` — `POST /api/config/check` with body `{ candidate: unknown }` →
    ...
    - `dropped` is the editor's own strictness, labelled as such (Design decision 9) — **and the three cases this bullet was written around are refusals now, not silences.** An invalid `budgets` value is refused by name (`core/config.ts` · `function requireBudgets(raw: unknown): Budgets {` · ~1377) and a non-string `watchedDocs` entry is refused rather than filtered (`core/config.ts` · `function requireWatchedDocs(raw: unknown): string[] {` · ~1423), so both arrive as `ok: false` with the thrown message, not as a finding. The top level reads five keys, `ui` included (`core/config.ts` · `export const TOP_LEVEL_KEYS = [` · ~1025), and an unknown one is skipped **and disclosed** on the resolved config (`core/config.ts` · `  skippedKeys: string[];` · ~575). So `dropped` carries `skippedKeys` — one finding per skipped key, e.g. `{ where: 'budget', message: 'not a key this build reads — nothing under it was loaded' }` — and nothing else until a later loader change opens a new silence. A config editor that let the loader's silences through would be INV-nothing-is-dropped-silently violated in the one screen built to prevent it; the loader having closed them is why the list is now short, not why it is gone.
 ...
 * (`hooks/pre-tool-use.ts` · ``changes to `.my_context/config.json` are the user`` · ~120): configuration changes to .my_context/config.json
 ...
 * map (`core/config.ts` · `const categories: Record<string, ResolvedCategory> = Object.create(null);` · ~1513) becomes a sorted array. */
 ...
 * `core/config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1444 and the four keys it reads). These findings are the
...
export function apiConfigPreview(ws: Workspace, url: URL, body: unknown): JsonResult {
  ...
  return withStores(ws, (store) => {
    ...
    // 2 + 3. Policy diffs per category, through the ONE lookups (`core/config.ts` · `export function scopePolicyFor(config: Config, type: string): ScopePolicy {` · ~207, and `agentEditsFor` beside it).
...
- Produces (the palette screen renders this catalogue; nothing else defines a command):
  ...
  - The catalogue itself, one entry per verified command — the source citations are in this plan's Verified Facts table and each def carries the flag set exactly as the CLI declares it:
    - **write**: `add` (args category+title; flags body/file/note/**step**/scope/tags/severity/**extra**/yes — `cli/index.ts` · `const ADD_USAGE =` · ~497; **`--step` and `--extra` landed after this list was written** and belong in it, because the sentence above says a def carries the flag set exactly as the CLI declares it), `edit` (arg id; flags title/body/scope/tags/severity/always/status/extra/yes — `cli/commands/edit.ts` · `usage: 'edit <id> [--title|--body|--scope|--tags|--severity|--always|--status|--extra]',` · ~956), `pin`/`unpin`/`harden`/`soften` (arg id; flag yes — `cli/commands/edit.ts` · ``usage: `${entry.name} <id> [--yes]`,`` · ~1188), `supersede` (args id + `--by` id — `cli/commands/supersede.ts` · `usage: 'supersede <id> --by <id>',` · ~177), `refresh` (arg id — `cli/commands/refresh.ts` · `usage: 'refresh <id>',` · ~176), `repair` (flag yes — `cli/commands/repair.ts` · `usage: 'repair [--yes]',` · ~320), `lesson-accept` / `lesson-discard` (args id+key — `cli/commands/lesson.ts` · `usage: 'lesson-accept <id> <key>',` · ~487 and `cli/commands/lesson.ts` · `usage: 'lesson-discard <id> <key>',` · ~494), `review promote` (arg id; flags scope/always/severity/yes — `cli/commands/review.ts` · `mycontext review promote <id> [--scope "a/**,b/**"] [--always] [--severity hard|soft] [--yes]` · ~90), `review discard` (arg id; flag yes — `:85`), `review promote-revision` (arg id; flags revision/force/yes — `:87`), `review discard-revision` (arg id; flags revision/reason/yes — `:88`), and `rebuild` (no args — `cli/index.ts` · `usage: 'rebuild',` · ~1596; classified write because it rewrites `.index.db` on disk, Design decision 12). <!-- historical-citation: quotes edit usage before `--continuity` joined it (plan:live seq:9) -->
...
test('every composed write command is matched by one of the fourteen deny rules (or is rebuild)', async () => {
  ...
  // `README.md` · `| Command | What it does with no human in the loop |` · ~5947, verbatim — the protection the composed-not-executed
...
- The composed settlement always carries `--revision`, though the spec's canonical line (`mycontext review promote-revision <id> --yes`) omits it — because without it the CLI refuses as soon as the item has more than one pending revision (`pickPendingRevision`, `core/revision.ts` · `export function pickPendingRevision(` · ~710). Settling the *oldest* was the default when this line was written and was removed for the same reason it gives: the oldest may not be the one on screen. The longer line is the correct one; the deny rule `Bash(mycontext review promote-revision *)` matches both.
