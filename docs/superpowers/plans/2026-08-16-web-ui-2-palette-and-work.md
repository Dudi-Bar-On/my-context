...
**Interfaces:**
- Consumes: `resolveConfig` (`config.ts` · `export function resolveConfig(raw: unknown): Config {` · ~1071), `DEFAULT_BUDGETS` (`config.ts` · `export const DEFAULT_BUDGETS: Budgets = { pinned: 6000, jit: 6000, restored: 8000, index: 1200 };` · ~67), `AGENT_EDITS` (`config.ts` · `export const AGENT_EDITS: AgentEdits[] = ['allow', 'review'];` · ~126), `SCOPE_POLICIES` (`config.ts` · `export const SCOPE_POLICIES: ScopePolicy[] = ['global', 'required', 'inert'];` · ~127), `PROFILES` (`core/categories.ts`), `node:fs` `readFileSync`/`existsSync`. <!-- historical-citation: quotes DEFAULT_BUDGETS as it was before the fifth budget, `continuity: 2000` (plan:live seq:9) -->
- Produces:
  ...
  - `apiConfigCheck(ws, url, body): JsonResult` — `POST /api/config/check` with body `{ candidate: unknown }` →
    ...
    - `dropped` is the editor's own strictness, labelled as such (Design decision 9) — **and the three cases this bullet was written around are refusals now, not silences.** An invalid `budgets` value is refused by name (`config.ts` · `function requireBudgets(raw: unknown): Budgets {` · ~1004) and a non-string `watchedDocs` entry is refused rather than filtered (`config.ts` · `function requireWatchedDocs(raw: unknown): string[] {` · ~1050), so both arrive as `ok: false` with the thrown message, not as a finding. The top level reads five keys, `ui` included (`config.ts` · `export const TOP_LEVEL_KEYS = [` · ~799), and an unknown one is skipped **and disclosed** on the resolved config (`config.ts` · `  skippedKeys: string[];` · ~352). So `dropped` carries `skippedKeys` — one finding per skipped key, e.g. `{ where: 'budget', message: 'not a key this build reads — nothing under it was loaded' }` — and nothing else until a later loader change opens a new silence. A config editor that let the loader's silences through would be INV-nothing-is-dropped-silently violated in the one screen built to prevent it; the loader having closed them is why the list is now short, not why it is gone.
