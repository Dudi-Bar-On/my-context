...
export const cases = [
  ...
  { id: 'rebuild-with-args-dropped', kind: 'cli', argv: ['rebuild', '--nope'],
    note: 'args deliberately dropped — `src/cli/index.ts` · `run: (ws, _args, out) => cmdRebuild(ws, out),` · ~1598' },
...
export const cases = [
  ...
  { id: 'edit-status-superseded-refused', kind: 'cli', setup: seed,
    ...
    note: "explicitly refuses superseded — `src/cli/commands/edit.ts` · `if (status === 'superseded') {` · ~683" },
...
export const cases = [
  ...
  { id: 'query-detail-flag-rejected', kind: 'cli', setup: seed,
    ...
    note: 'rejects detail flags — `src/cli/commands/query.ts` · `if (!QUERY_FLAGS.includes(name)) {` · ~349' },
  ...
  { id: 'audit-role', kind: 'cli', setup: seed, argv: ['audit', '--role', 'human'],
    note: 'accepted; UNDOCUMENTED at v1.0.0, and since declared in USAGE — `src/cli/commands/audit.ts` · `[--role subject|injected|spilled]` · ~25' },
  ...
  { id: 'audit-replay-ledger', kind: 'cli', setup: seed, argv: ['audit', 'replay-ledger'],
    note: "hidden positional subcommand — `src/cli/commands/audit.ts` · `if (args[0] === 'replay-ledger') {` · ~309" },
  ...
  { id: 'focus-show', kind: 'cli', setup: seed, argv: ['focus', '--show'],
    note: 'real; under-documented at v1.0.0, and since declared in USAGE — `src/cli/commands/focus.ts` · `mycontext focus --show | --relations` · ~51' },
  ...
  { id: 'focus-clear-with-axis-refused', kind: 'cli', setup: seed,
    argv: ['focus', '--clear', '--tag', 'db'], note: 'refuses the combination — `src/cli/commands/focus.ts` · `my_context: --clear takes no axes, and you passed some.` · ~158' },
...
export const claims = [
  {
    ...
    where: '`README.md` · `has no commands line, and reports the commands and the skill together under one` · ~1988', // v1.0.0 said "the 38 commands"; the README now carries the correction
  ...
  },
  {
    ...
    where: '`README.md` · `91 slash commands` · ~1928', // 66 at v1.0.0
    evidence: 'slash/file-count',
    check: (r) => r.actual === 66,
  },
  {
    ...
    where: '`README.md` · `A single-valued flag given twice` · ~2246',
  ...
  },
  {
    ...
    where: '`README.md` · `so passing both is refused rather than resolved by precedence` · ~2248',
  ...
  },
  {
    ...
    where: '`README.md` · `and everything else — a positional,` · ~3865', // v2 refuses init's unknown flags by name; the v1.0.0 claim no longer holds
  ...
  },
  {
    ...
    where: '`README.md` · `No tool takes an` · ~5821',
  ...
  },
  {
    ...
    where: '`README.md` · `in particular refuses` · ~3685',
  ...
  },
  {
    ...
    where: '`README.md` · `the just-in-time tier, and the one refusal in the product` · ~1998',
  ...
  },
  {
    ...
    where: '`README.md` · `> **Focus never hides a` · ~3005',
  ...
  },
  {
    ...
    where: '`README.md` · `an unknown profile name is an error at load time, not a` · ~3937',
  ...
  },
  {
    ...
    where: '`README.md` · `6000 | the pinned tier at session start` · ~1837',
  ...
  },
  {
    ...
    where: '`test/cli/status.test.ts` · `the version status prints is package.json` · ~563', // the README no longer states it
