...
| `HELP_TOPICS = ['categories','scope','capture','workflow','cli','tools','slash']` — seven now. Three of them are generated: `cli` from `COMMANDS`, which needs the CLI loaded to render and refuses otherwise; `tools` from the MCP tool registry and `slash` from the committed `commands/` directory, neither of which has that precondition | `help/index.ts` · `export const HELP_TOPICS: HelpTopic[] = [` · ~16 |
