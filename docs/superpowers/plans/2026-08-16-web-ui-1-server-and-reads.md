...
| 10 | Activity pulse | `data-p="watch"` · `#pulse` · `watch.pulsen` | Plan 3 | ❌ **no** | Ten-second columns over `idx_audit_at` (`core/audit-db.ts` · `idx_audit_at` · ~84), coloured by record kind. Plan 3's stream. **Reported, not designed here.** |
