...
| Was | Is | Class | Where |
|---|---|---|---|
| The offset reader is `readFrom`, **private** at `audit-db.ts:178` | **It was already `export`ed — as `readSegmentFrom` when this row was written, and as `readCompleteLines` since Task 1 took the rename the next row predicted** (`core/audit-db.ts` · `export function readCompleteLines(file: string, offset: number): { text: string; consumed: number } {` · ~234). Half of Task 1's first seam was therefore already done | A task that exists to expose a symbol re-checks whether it is still hidden; work already landed is not work to plan | **Task 1** |
...
| Fact | Where verified |
|---|---|
| `AuditRecord { protocol; at; kind; op; origin?; itemId?; fields?; sessionId?; hook?; injected?; tokens?; spilled?; path?; note? }` | `core/audit.ts` · `export interface AuditRecord {` · ~859 |
| `tokens?: number` — absent means "not recorded", never zero | `core/audit.ts` · `tokens?: number;` · ~964 |
| `InjectedRef { id; tier; at? }` | `core/audit.ts` · `export interface InjectedRef {` · ~680 |
| `SpilledRef extends InjectedRef { reason: string }` | `core/audit.ts` · `export interface SpilledRef extends InjectedRef {` · ~698 |
| `AuditKind` — **four** members when this row was written, **six** since `access` (2026-08-20) and `progress` (2026-08-21) joined | `core/audit.ts` · `export const AUDIT_KINDS: AuditKind[] = [` · ~647 |
| `AUDIT_KINDS` exported | `core/audit.ts` · `export const AUDIT_KINDS: AuditKind[] = [` · ~647 |
| `FOCUS_OPS = ['focus-set', 'focus-clear']` | `core/audit.ts` · `export const FOCUS_OPS = ['focus-set', 'focus-clear'] as const;` · ~428 |
| `recordAudit(root, input)` — appends, never throws | `core/audit.ts` · `export function recordAudit(root: string, input: AuditInput): AuditWriteResult {` · ~1509 |
| `readAudit(root)` | `core/audit.ts` · `export function readAudit(root: string): AuditRecord[] {` · ~1551 |
| `filterAudit(records, filter)` | `core/audit.ts` · `export function filterAudit(records: AuditRecord[], filter: AuditFilter): AuditRecord[] {` · ~1639 |
| `AuditFilter { since?; until?; itemId?; sessionId?; kind?; op?; origin?; limit? }` | `core/audit.ts` · `export interface AuditFilter {` · ~1610 |
| `parseWhen(raw, flagName)` | `core/audit.ts` · `export function parseWhen(raw: string, flagName: string): string {` · ~1591 |
| `auditSegments(root)` — every segment oldest first, live `audit.jsonl` last | `core/audit.ts` · `export function auditSegments(root: string): string[] {` · ~1243 |
| Audit log lives under `<projectRoot>/.audit/` | `core/audit.ts` · `export function auditDir(root: string): string {` · ~1196 |
| `openProjection(root)` — discards and recreates on corruption or version mismatch | `core/audit-db.ts` · `export function openProjection(root: string): DatabaseSync {` · ~730 |
| `syncProjection(root, db)` — returns the state it FOUND; catches up incrementally; rebuilds only on divergence | `core/audit-db.ts` · `export function syncProjection(root: string, db: DatabaseSync): ProjectionState {` · ~282 |
| `projectionState(root, db)` — pure comparison; a shrunken or vanished segment is `diverged` | `core/audit-db.ts` · `export function projectionState(root: string, db: DatabaseSync): ProjectionState {` · ~195 |
| `queryProjection(db, filter)` — the WHERE builder was inline here when this row was written. **Task 1 has executed: it is the exported `filterSelect` beside it, which `queryProjection` and `ui/ask-model.ts` both call** | `core/audit-db.ts` · `export function queryProjection(db: DatabaseSync, filter: AuditFilter): AuditRecord[] {` · ~1225 |
| `summaryByOp(db, filter?)` | `core/audit-db.ts` · `export function summaryByOp(db: DatabaseSync, filter: AuditFilter = {}): SummaryRow[] {` · ~1284 |
| `topItems(db, role, limit)` | `core/audit-db.ts` · `export function topItems(db: DatabaseSync, role: string \| null, limit: number): SummaryRow[] {` · ~1337 |
| `sessions(db, limit)` | `core/audit-db.ts` · `export function sessions(db: DatabaseSync, limit: number): SummaryRow[] {` · ~1352 |
| `audit_item` side table — `role` is `TEXT NOT NULL`, **not** a CHECK-constrained enum | `core/audit-db.ts` · `  role    TEXT NOT NULL,` · ~123 |
| The three roles are documented in the schema comment | `core/audit-db.ts` · `and did not fit, and counting those by item is how a user finds a budget` · ~118 |
| **`readCompleteLines(file, offset)` — `export`ed**, carrying the name Task 1 gave it | `core/audit-db.ts` · `export function readCompleteLines(file: string, offset: number): { text: string; consumed: number } {` · ~234 |
...
| `readStdin()` — synchronous `readFileSync(0)`, `''` when no stdin | `hooks/io.ts` · `export function readStdin(): StdinRead {` · ~448 |
...
> **Re-extracted 2026-08-23 on build `2.1.239`** — `VERSION:"2.1.239"`, `GIT_SHA:"9bf8e9521fe06414183309865310e27c9b8db3dd"`, `BUILD_TIME:"2026-08-21T04:40:30Z"`, read from `C:/Users/UserC/.local/share/claude/versions/2.1.239` (`~/.local/bin/claude.exe` is a launcher byte-identical in size to it, and `claude --version` prints `2.1.239 (Claude Code)`). **No field this table depends on moved.** Two things did, and are corrected in the rows below:
>
> - **Minifier symbol names are not stable across builds.** The `context_window` builder is `TAw`/`wMo` on 2.1.233 and `pgA`/`HZo` on 2.1.239; its body is **byte-identical** once the two names are normalised, including the `e?…:0` branch. A row that quotes a symbol name is quoting something that will churn every build — the rows below now say which build each quote came from.
> - **The payload has grown.** It is stored WHOLE by `writeTee`, so growth drops nothing (INV-nothing-is-dropped-silently), but the "full payload also carries" row was an under-count. One of the new fields, **`session_name`, matters to a decision recorded elsewhere** — see the note after this table.
>
> The **2.1.233 binary is no longer on disk** (only `2.1.237`, `2.1.238`, `2.1.239` are), so the original reading cannot be re-run; the 2.1.233 quotes below are kept as the historical reading, not re-confirmed.
...
| `context_window` is built as `{total_input_tokens, total_output_tokens, context_window_size, current_usage, used_percentage, remaining_percentage}` | The construction function, verbatim from the binary. **2.1.233:** `function TAw(e,t){let r=wMo(e,t);return{total_input_tokens:e?e.input_tokens+e.cache_creation_input_tokens+e.cache_read_input_tokens:0,total_output_tokens:e?.output_tokens??0,context_window_size:t,current_usage:e,used_percentage:r.used,remaining_percentage:r.remaining}}` — **2.1.239** (re-extracted 2026-08-23), identical but for the two symbol names: `function pgA(e,t){let r=HZo(e,t);return{total_input_tokens:e?e.input_tokens+e.cache_creation_input_tokens+e.cache_read_input_tokens:0,total_output_tokens:e?.output_tokens??0,context_window_size:t,current_usage:e,used_percentage:r.used,remaining_percentage:r.remaining}}` |
...
| The full payload also carries `model: {id, display_name}`, `workspace: {current_dir, project_dir, …}`, `version` (the plain build string — `"2.1.233"` when this row was written, `"2.1.239"` on the installed build), `cost: {total_cost_usd, …}`, `exceeds_200k_tokens`, optional `rate_limits`. **Re-read 2026-08-23 on 2.1.239 and the list is longer than this row claimed:** also `session_name` (optional), `output_style: {name}`, `fast_mode`, `thinking: {enabled}`, optional `effort`, `vim`, `agent`, and `cost` carries `total_duration_ms`, `total_api_duration_ms`, `total_lines_added`, `total_lines_removed`; `workspace` carries `added_dirs` and optional `git_worktree`, `repo` | The payload-assembly function in the binary (`kAw` on 2.1.233), read in full. **2.1.239:** `return{...b_(e,f),...T&&{session_name:T},model:{id:_,display_name:Ih(_)},workspace:{current_dir:f,project_dir:e.project.originalCwd,added_dirs:s,...l&&{git_worktree:l},...c&&{repo:c}},version:{…}.VERSION,output_style:{name:b},cost:{…},context_window:pgA(S,w),exceeds_200k_tokens:r,fast_mode:n,…}` |
...
| `CLAUDE_CONFIG_DIR` overrides the `~/.claude` settings directory | 33 references in the 2.1.233 binary; **59 in 2.1.239** (re-counted 2026-08-23 — the count is evidence of the mechanism's presence, not itself a fact the design turns on) |
...
**A second finding from the 2026-08-23 re-extraction, flagged and NOT acted on here.** The status line payload on `2.1.239` carries **`session_name`** — the binary's own embedded status-line schema documents it as `"session_name": "string", // Optional: Human-readable session name set via /rename`, and the assembly emits it as `...T&&{session_name:T}` where `T=uwr(e.id)`. It is present identically in `2.1.237` and `2.1.238`, so it did not arrive in `2.1.239`; it arrived somewhere between `2.1.234` and `2.1.237`, and **`2.1.234` is not on disk, so the exact build cannot be pinned from here.** This bears directly on a decision recorded in another document: `2026-08-19-v2-scope-decisions.md` §6d ("Session names — mycontext owns them") rests on a **negative probe on 2.1.234** — *"no session name is visible anywhere a hook could reach"* — and explicitly says mycontext "reads Claude Code's if a later probe locates one"; `2026-08-20-v2-hooks-sessions-and-continuity.md` defers the same item, saying *"a later positive probe would remove the problem rather than change the design."* **This is that positive probe.** Two caveats keep it from being a contradiction: the §6d probe was scoped to *what a hook can reach*, and a status line command is **not** a hook; and the name is only populated when the user has set one via `/rename`. But the surface it appears on is **the one this plan tees WHOLE**, so the name is already reaching mycontext's disk. Amending §6d is that document's decision, not this plan's — recorded here so it is not lost.

**One spec drift found and flagged, not silently corrected:** §4b states the join is on `session_id` alone because "a finer join … would need a per-turn identifier that **neither side produces**." As of 2.1.233 Claude Code **does** produce one — `prompt_id`, optional, in both hook and status line payload bases. mycontext's side declared no such field when this was written (`hooks/io.ts` · `export interface HookInput {` · ~9); **as of 2026-08-21 it does — the v2 hooks plan's Task 5 declares it — and nothing reads it**, so the join in this plan stays on `session_id` exactly as specified either way. **The spec has since been amended** — `2026-08-16-web-ui-design.md` §4b now scopes the claim to what this repository can assert and writes down the condition under which a finer join becomes possible, citing this plan as where the drift was found. Nothing in this plan depends on either reading.
...
**Interfaces:**
- Consumes: `ensureLogDir` (`jsonl-log.ts` · `export function ensureLogDir(dir: string): string {` · ~93), `node:fs`, `node:path`.
...
> (`core/ledger.ts` · `from it undercounts by exactly the repeats the key swallowed. Which stamp` · ~477).
> The audit projection carries both facts on the SAME row, each indexed, so **no join is required**:
> `at` (`core/audit-db.ts` · `(rec ->> '$.at')` · ~75) under `idx_audit_at`, and `kind`
> (`core/audit-db.ts` · `(rec ->> '$.kind')` · ~76) under `idx_audit_kind`. Both are VIRTUAL
...
>    (`test/ui/no-writes.test.ts` · `+ 'module unreachable from the entry point and not named above is either dead code or a '` · ~1378),
...
>    (`test/ui/server-e2e.test.ts` · `'these routes are registered and never exercised by the no-write sweep, so nothing proves '` · ~804).
...
> (`cli/commands/query.ts` · `export function assertSelectOnly(sql: string): void {` · ~266). That
...
**Interfaces:**
- Consumes: `Store.openReadOnly` (`core/store.ts` · `static openReadOnly(dbPath: string): Store {` · ~382), `openProjection`/`syncProjection`/`queryProjection`/`filterSelect`/`summaryByOp`/`topItems`/`sessions` (`audit-db.ts` + Task 1), `parseWhen`/`AUDIT_KINDS`/`AUDIT_OPS` (`audit.ts`), `unknownParams`/`badRequest` (Task 6 Step 1), `registerRoute`.
- Produces:
  - `Store.raw(sql: string, params?: (string | number)[])` — the existing method with bind parameters; both existing callers are untouched by the default: `query.ts` · `const fetched = store.raw(withRowCap(sql, limit + 1));` · ~474 and `core/store.ts`'s own `openReadOnlyChecked` (`core/store.ts` · `const rows = store.raw('SELECT version FROM schema_version LIMIT 1') as` · ~405), which arrived with the read-only JIT open after this plan was written.
...
>    (`src/ui/server.ts` · `registerWatchRoutes();` · ~542). They could not wait for this task:
...
>    values (`src/ui/watch-model.ts` · `export type WatchProjectionState = 'fresh' | 'absent';` · ~160);
...
> `core/select.ts` · `function carriedDropReason(` · ~981 spells five of them and the caller spells
