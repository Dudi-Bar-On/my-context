# v2.0 Plan — the `todo` and `note` categories, and the one-shot `runbook` lifecycle
...
listing and promotion surfaces, and give the **already-shipped** `runbook` category a one-shot
lifecycle — a first-class `## Steps` field, an activation that is two deliberate human writes, a
completion that retires it, and step progress recorded outside `items/` so the corpus never moves
when somebody makes progress.

**Architecture:** Three separable pieces, in this order. (1) Two `def()` entries in
...
kind of audit record and nowhere else.

**Tech Stack:** Node ≥ 24 built-ins only. No framework, no build step, no runtime dependency.

**Spec:** `docs/superpowers/specs/2026-08-19-v2-scope-decisions.md` — the binding authority. **§6m
supersedes §2, §6f, §6g and §6h where they conflict**, and this plan is written against §6m first
and the earlier sections second. Executors read §1, §2 (with §2.1–§2.3), §6a, §6g, §6i and §6m.

**Survey:** `.superpowers/sdd/2026-08-19-v2-scope-decisions/survey-categories.md` — the file-level
map this plan is built on, and the source of most citations below. It was written **before** §6m
withdrew the third category, so it is correct about the code and stale about the vocabulary; §0
records every place the two part.

**Scope split (binding):** This plan covers §1 (both new categories, the promotion path, the `todo`
listing surface), §2 with §2.1–§2.3 (the `runbook` lifecycle), §6a's `## Steps` ruling, §6g's step
progress, §6i.1/§6i.2/§6i.4/§6i.5, and §6m.1/§6m.2/§6m.3/§6m.9/§6m.12. Everything else in that
spec — export, packs, imports, sessions, hooks, `watchedDocs`, the rule-file exporter, cross-session
continuity — is out of scope and belongs to sibling plans. **"What this plan is not doing" below
names the parts of even the in-scope sections that are deliberately left unbuilt.**
...
- **`INV-nothing-is-dropped-silently`.** A field accepted and ignored, a step line parsed and then
  destroyed on the next `persist()`, a `ready` runbook that reaches no index line and no count — all
...
- **The trust boundary is the category's tier, and nothing else.** `trustedStatus` demotes every
  ...
  agent writes them directly; `runbook` is normative, so an agent-authored one lands as a draft.
...
a third new category
-->

**These corrections are enforced, not merely recorded.** The block above lists the phrases this plan
retired; `npm run check:retired` fails if any of them reappears anywhere below §0. The survey this
plan is built on names a category §6m.1 withdrew, and the cheapest way for that name to leak back
into a task body is for an implementer to copy a row out of the survey verbatim.

Every row names the **class** of error, not only the instance.

| Was | Is | Class | Where it lands |
|---|---|---|---|
| The survey is titled *"what it takes to add `todo`, `note` and …"* a further category, and sizes everything for that count | **Two categories.** §6m.1 withdrew the third: `runbook` already ships (`categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~40) and takes the one-shot lifecycle instead | A survey written against a draft spec is stale about the *decision* while still correct about the *code*. Reuse its file-level findings; re-derive its counts | Tasks 1, 2, 10 |
| Survey §7.3 row 12: add `24: { en: 'twenty-four', … }` to `CATEGORY_WORDS` | **Nothing to add.** The catalogue goes 21 → **23**, and `counts.test.ts` · `  23: { en: 'twenty-three', he: 'עשרים ושלושה' },` · ~287 already spells it | A count derived from a withdrawn scope is wrong in both directions; re-derive counts from the decision, never from the survey | Task 2 |
| Survey §7.3 rows 9/11: `21` → `24` in the catalogue tests | `21` → **`23`**, in both places (`core/categories.test.ts` · `test('there are 21 categories', () => {` · ~5 and `core/categories.test.ts` · `  assert.equal(PROFILES.standard.length, 21);` · ~70) | as above | Task 2 |
| Survey §7.2: the new categories add **6** generated command files | **4** — `add-todo.md`, `list-todo.md`, `add-note.md`, `list-note.md` | as above | Task 2 |
| Survey §6.2's parenthetical treats the shipped `runbook` seed's `1. `/`2. `/`3. ` body as settled, because the new shape was going to live elsewhere | **The shipped `runbook` seed is now the thing that must change shape**, and one committed assertion is directly in the way: `help.test.ts` · ``  for (const step of ['1. ', '2. ', '3. ']) assert.ok(runbook.includes(step), runbook);`` · ~268 | When a decision moves a behaviour onto an *existing* symbol, every test pinning that symbol's old behaviour becomes work. Survey rows about "the new thing" become rows about the old thing | Task 10 |
| §6m's controller ruling on F6 lists *"a `validateBody` carve-out"* among the work | **No behavioural carve-out is needed.** Steps never enter `body`, so `validateBody` (`validate.ts` · `export function validateBody(body: string): void {` · ~234) is correct exactly as written. What *is* needed is that its message names the new route, because a user pasting a whole runbook is refused with a message that today offers only observations | A cost estimate written before the shape was fixed can over-state as well as under-state. Verify each named site against the code before scheduling it | Task 5 |
...
`skills/mycontext/SKILL.md` needs after two names and their reflow (Task 2 establishes it by
...
| `runbook` already ships: normative, prefix `RUN`, enabled | `categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~40 |
...
| `deprecated` is counted in `retired`, so a finished runbook stays in a session-visible number | `select.ts` · `const RETIRED_STATUSES = new Set(['superseded', 'deprecated', 'validated']);` · ~308 |
...
| `edit --status superseded` is refused on purpose; `supersede --by` is the route | `cli/commands/edit.ts` · `      if (status === 'superseded') {` · ~519 |
...
| Content identity, which must learn `steps` or two runbooks differing only in steps dedupe onto each other | `content-hash.ts` · `interface ContentShape {` · ~13 |
| …and its item-side entry point | `content-hash.ts` · `export function itemContentHash(item: Item): string {` · ~104 |
| The injected block, which must emit steps or an `active` runbook arrives without the content it exists to deliver | `render-item.ts` · `export function renderItemBlock(item: Item): string {` · ~172 |
...
| Revisions carry four fields, and `steps` must be explicitly none of them | `revision.ts` · `export const REVISION_FIELDS = ['title', 'body', 'tags', 'extra'] as const;` · ~129 |
...
| Both READMEs' hand-written counts | `README.md` · `The catalogue holds **21** categories` · ~3774 |
| …and | `README.md` · ``Two profiles: `minimal` (8 categories) and `standard` (all 21, the default)`` · ~2972 |
| The Hebrew mirror | `docs/README.he.md` · `הקטלוג מחזיק **21** קטגוריות` · ~4072 |
...
| `todo`, `note` and `runbook` contain no `_`, so the slug generator produces no collision | read the slug-clash throw in `plugin/commands.ts`; `commandSlug` only rewrites `_` |
| `CATEGORY_WORDS` already spells 22 and 23 | read `test/docs/counts.test.ts` ~282-288 |
...
| `npm run verify:citations` and `npm run check:retired` both exit 0 on the tree this plan was written against (190 citations, 23 documents, 0 broken) | executed, 2026-08-20 |
...
7. **`Step` is `{ text: string; checked: boolean }`, and the parser accepts exactly `- [ ] ` and
   ...
   `checked: true`.**
8. **`## Steps` renders before `## Observations`.** Steps are what a runbook *is*; observations are
...
10. **`ready` is a tag, not an `extra` field.** §6m.2 permits either. A tag is free, searchable today
    ...
    have to join `runbook.extraFields`, which flows into the MCP `create_item` schema built from the
...
11. **Activation is one command performing two writes, human-only by construction.**
    `mycontext runbook activate <id>` issues a single `updateItem` carrying both `status: 'active'`
    ...
    ships a runbook that is merely eligible — indexed, not delivered.
12. **Progress is workspace-scoped and lives in the audit log.** Not session state: no CLI surface
    ...
    recent `step-reset`, which `runbook activate` writes. **The cost is stated in the command's own
    output:** two terminals working one runbook in one workspace share one record set.
13. **A new `AuditKind`, not a new `MUTATION_OPS` member.** A step tick changes no item, so filing it
    under `mutation` would make `mycontext audit --kind mutation --item RUN-x` a question with a
    wrong answer — the identical argument `core/audit.ts` makes for `focus` being a fourth kind.
    This is the fifth.
14. **Injection never carries progress.** `renderItemBlock` emits the stored steps and nothing else.
    An injected runbook is the knowledge; progress is a display concern of `mycontext runbook show`.
    Anything else would make two sessions receive different text for the same item.
15. **`mycontext todo` lists; there is no `mycontext note` command.** §6m.9 names one surface and this
...
- **No `ready` injection, and no per-item injection mode in `select()`.** §2.1 is explicit:
  *"Nothing may be built on 'index line only' until that is decided."* `isEligible` admits `active`
  only, so a `ready` runbook — a draft carrying a tag — reaches no index line today. This plan does
  not add a per-item mode and does not add a `Status` member. What it does instead is make the
  consequence visible: `mycontext runbook list` prints, for every `ready` runbook, that the model
...
- **No further new category, and no repeatable-sequence category.** §6m.1. A repeatable sequence is a
  `standard` or a `rule` with ordered steps.
...
- **No execution of runbook steps.** A runbook is knowledge delivered to a model; a command that ran
...
- **No `mycontext note` command and no `mycontext inbox` listing.** Design decision 15.
...
| # | Site | What must change for `todo` + `note` | Pinned by |
|---|---|---|---|
| 1 | `src/core/categories.ts` (`CATEGORIES`) | two `def()` entries | — (it is the source) |
| 2 | `src/core/categories.ts` (`PROFILES.minimal`) | **no change** — decision 2, recorded as a comment | the size assertion in `categories.test.ts` |
| 3 | `src/help/he.ts` | two Hebrew descriptions | set equality against `CATEGORIES` |
| 4 | `src/help/index.ts` (`SEEDS`) | two worked specimens | the no-placeholder assertion |
| 5 | `src/help/topics/categories.md` | two sections, ≥150 chars, each naming a nearest neighbour | two assertions |
...
| 7 | `skills/mycontext/SKILL.md` | two names in the **Rationale** bullet | set equality, both directions |
| 8 | `test/plugin-assets.test.ts` | raise the character ceiling, with a recorded reason | the ceiling itself |
| 9 | `test/core/categories.test.ts` | `21` → `23` | itself |
| 10 | `test/core/categories.test.ts` | two rows in the pinned `(name, prefix, tier, defaultEnabled)` table | itself |
| 11 | `test/core/categories.test.ts` | `PROFILES.standard.length` `21` → `23` | itself |
| 12 | `test/docs/counts.test.ts` (`CATEGORY_WORDS`) | **nothing** — `23` is already spelled (§0) | itself |
...
| 15 | `README.md` / `docs/README.he.md` | two `<!-- example: examples <name> --short -->` marker blocks each, plus the prose around them | the markers are hand-typed — **Task 1's test** |
| 16 | `test/docs/counts.test.ts` slash-command breakdown | per-category 42 → 46, and the totals in both READMEs' §5 sentences | itself, against `commands/` |
| 17 | `test/plugin/commands.test.ts` (`GENERIC`) | grows once per new generic command (Tasks 3, 4, 9, 11) | itself |
| 18 | `docs/TUTORIAL.md` | the parenthesised rationale list, and its count `8` | **nothing today — Task 1** |
| 19 | `docs/TUTORIAL-ADVANCED.md` | the appendix rationale list, and its count `8` | **nothing today — Task 1** |
| 20 | `CHANGELOG.md` | a v2.0 entry | `scripts/changelog-section.ts` in CI |
| 21 | `docs/ROADMAP.md` | tracking rows | explicitly **not** tested |
| 22 | `commands/*.md` | four generated files | byte-identity against the generator |
...
src/cli/commands/runbook.ts       # `mycontext runbook list|show|activate|done|step` (Task 9)
...
commands/runbook.md               # generated (Task 9)
...
test/cli/runbook.test.ts          # (Task 9)
test/fixtures/runbook-with-steps.md       # RAW fixture for byte-identity (Task 5)
```

Modified files:

```
src/core/categories.ts        # + todo, note; runbook's description rewritten (Tasks 2, 10)
...
src/help/he.ts                # + two Hebrew descriptions; runbook's rewritten (Tasks 2, 10)
src/help/index.ts             # + two SEEDS; runbook's seed rewritten; exampleItemShort (Tasks 2, 5, 10)
src/help/topics/categories.md, .he.md  # + two sections; runbook's rewritten (Tasks 2, 10)
skills/mycontext/SKILL.md     # + two names in the Rationale bullet (Task 2)
README.md, docs/README.he.md  # counts, specimens, new commands, the lifecycle (Tasks 2, 3, 4, 9, 12)
docs/TUTORIAL.md, docs/TUTORIAL-ADVANCED.md  # category lists (Task 2)
...
## Task 2: Add `todo` and `note`, and close all 22 enumeration sites in one commit
...
- Create (generated): `commands/add-todo.md`, `commands/list-todo.md`, `commands/add-note.md`,
  `commands/list-note.md`

**Interfaces:**
- Consumes: Task 1's guard.
- Produces: two `ResolvedCategory` entries reachable by every surface that resolves through
  `resolveConfig` — `mycontext add todo …`, `create_item{type:"todo"}`, `search --type note`,
  `/mycontext:add-todo`. **Nothing else in this plan may assume they exist before this task lands.**
...
| `description` | `Something to build or fix later, captured the moment it occurs to you` | `Anything that arose during development and must not be lost` |
| `extraFields` | none | none |
| joins `PROFILES.minimal` | **no** | **no** |

- [ ] **Step 1: Make the counting tests fail first**

Edit `test/core/categories.test.ts` only, and run it:

- `test('there are 21 categories', …)` → rename to `23` and assert `23`
  (`core/categories.test.ts` · `test('there are 21 categories', () => {` · ~5).
- `assert.equal(PROFILES.standard.length, 21);` → `23`
  (`core/categories.test.ts` · `  assert.equal(PROFILES.standard.length, 21);` · ~70).
- Add two rows to the pinned table, at the END of the rationale block, in catalogue order
...
Expected: FAIL — 21 ≠ 23, and the pinned table has two rows the catalogue does not.

- [ ] **Step 2: Add the two entries**

In `src/core/categories.ts`, after the `reference:` entry
(`categories.ts` · `export const CATEGORIES: Record<string, CategoryDef> = {` · ~19):
...
decision rather than leaving it to be re-derived:

```ts
  // `todo` and `note` are deliberately absent. `minimal` is the smallest useful
  // NORMATIVE vocabulary for a project that wants one; an inbox is orthogonal
  // to that, and the per-category `"enabled": true` in config already switches
  // one on and says which.
```
...
which also asserts each value actually contains Hebrew.

- [ ] **Step 4: Two worked seeds**
...
`src/help/topics/categories.md` and `src/help/topics/categories.he.md`. Two `### ` sections in each,
**in the same position in both files** — the Hebrew source is checked heading-for-heading and
...
add `` `todo` `` and `` `note` `` to the **Rationale** bullet. They are not optional text — the
bullets are asserted to be exactly the enabled set in both directions, so omitting them fails and
adding them to the wrong bullet fails differently.
...
comment block above it in the register the six previous raises use: which two names were added, why
they are not optional (the bullets are the tier table), and the measured cost.

- [ ] **Step 7: Regenerate**

```bash
npm run gen:commands   # writes commands/add-todo.md, list-todo.md, add-note.md, list-note.md
...
| Where | 21 → 23 |
|---|---|
| `README.md` · ``Two profiles: `minimal` (8 categories) and `standard` (all 21, the default)`` · ~2972 | yes |
| `README.md`, the generated category table's row count sentence | yes |
| `README.md` · `The catalogue holds **21** categories` · ~3774, and the "enables all **21**" beside it | yes |
| `README.md`, "twenty-one specimens, twenty-one types" and "not a fixed list of twenty-one nouns" | → **twenty-three** |
| `docs/README.he.md` · `הקטלוג מחזיק **21** קטגוריות` · ~4072 and its three siblings | digits **and** the number-word `עשרים ושלושה` |
| Both, §5's slash-command breakdown | per-category `42` → `46`; the "All N carry `disable-model-invocation: true`" total `65` → `69` |
| Both, §8's per-category command count | `21` → `23` |

Then, in **both** documents, add two `<!-- example: examples todo --short -->` and
`<!-- example: examples note --short -->` marker blocks with their surrounding prose, in catalogue
order beside the other rationale specimens, and re-run `npm run gen:docs` to fill them.

Then the two tutorials, which Task 1 now pins:

- `docs/TUTORIAL.md` · `- **Normative** categories (13 of them:` · ~252 — the **Rationale** bullet
  below it goes from `(8: …)` to `(10: …)` and gains both names.
- `docs/TUTORIAL-ADVANCED.md` · `**The 13 normative categories:**` · ~461 — the rationale list below
  it goes from `**The 8 rationale categories:**` to `**The 10 rationale categories:**` and gains both.
...
```bash
git add -A
git commit -m "feat(categories): add todo and note on the rationale tier, and close every enumeration site"
```
...
- Create: `test/core/steps.test.ts`, `test/fixtures/runbook-with-steps.md`
...
`body` and `observations`.** `computeItemChecksum` hashes `JSON.stringify` of a fixed-order object,
...
there is nothing to go stale. Omitting it would make two runbooks differing only in their steps
...
const FIXTURE = path.join(import.meta.dirname, '..', 'fixtures', 'runbook-with-steps.md');
const REL = 'items/runbook/RUN-rotate-the-stripe-webhook-secret.md';
...
test('two runbooks differing only in their steps do not dedupe onto each other', () => {
...
Write `test/fixtures/runbook-with-steps.md` by hand — **not** by rendering it — with frontmatter, a
...
```ts
/**
 * One line of a runbook's `## Steps` section.
 *
 ...
 * Progress made through `mycontext runbook step` is recorded in the audit
 * log and never in the item — spec §6m.3 — so the file on disk does not
 * move when somebody makes progress, `checksum` stays stable, and
 * `UPDATE_FIELD_POLICY` (trust.ts) is never asked to classify a third kind
 * of field.
 */
...
exactly as written (§0). Its **message** gains one clause: a user pasting a whole runbook is refused
today with a message that offers only observations, and it must now also name
`mycontext add runbook --step "<text>"`.

- [ ] **Step 7: Record that `steps` is not stageable**

`src/core/revision.ts` · `export const REVISION_FIELDS = ['title', 'body', 'tags', 'extra'] as const;` · ~129
does not change; add to the comment above it that `steps` is deliberately absent for the same reason
...
test('steps is create-only, so the field-policy table is never asked about it', () => {
  ...
  const update: UpdateInput = { id: 'RUN-x' };
...
`active` runbook inject without the content it exists to deliver **and** under-count its budget, so
...
display concern of `mycontext runbook show` (Task 9). Anything else would make two sessions receive
...
const RUNBOOK = { /* a full Item literal with three steps, one observation and a scope */ };

test('the injected block carries the steps, in order, as checkbox lines', () => {
  const text = renderItemBlock(RUNBOOK);
...
test('the block never shows progress — an injected runbook is the same text in every session', () => {
  assert.doesNotMatch(renderItemBlock(RUNBOOK), /\d+ of \d+/);
});

test('the budget charges for the steps, because cost is derived from this exact text', () => {
  const withSteps = estimateTokens(renderItemBlock(RUNBOOK));
  const without = estimateTokens(renderItemBlock({ ...RUNBOOK, steps: [] }));
  assert.ok(withSteps > without,
    'itemCost reads renderItemBlock; if the block omits steps the selector under-counts a ' +
    'runbook and admits it to a tier it does not fit');
});
```
...
```bash
git add -A
git commit -m "feat(injection): an injected runbook carries its steps, and the budget charges for them"
```
...
**Interfaces:**
- Consumes: Task 5.
- Produces: `steps?: string[]` on `CreateInput`; `mycontext add runbook "<title>" --step "<text>"`
  (repeatable); `create_item{steps: string[]}`. Task 10's seed and Task 9's `runbook show` consume
  the result.
...
test('a step is not comma-split — a step is a sentence and sentences contain commas', () => {
  withWorkspace((dir) => {
    runCli(['add', 'runbook', 'X', '--step', 'Stop the worker, then drain the queue', '--yes'],
      dir, () => {});
    assert.equal(readItem(dir, 'RUN-x').steps.length, 1);
  });
});
...
test('a step containing a line break is refused, and nothing is written', () => {
  withWorkspace((dir) => {
    const out: string[] = [];
    assert.equal(runCli(['add', 'runbook', 'X', '--step', 'a\nb', '--yes'], dir,
      (s) => out.push(s)), 1);
    assert.match(out.join('\n'), /line break/);
    assert.equal(runCli(['show', 'RUN-x'], dir, () => {}), 1);
  });
});
...
(`cli/index.ts` · `const ADD_VALUE_FLAGS = ['body', 'file', 'note', 'scope', 'tags', 'severity'];` · ~184),
...
longer one of them. And add, to `--step`'s own help text, that **steps cannot be edited afterwards
through any command** — correcting one means editing the Markdown and running `mycontext repair`.
Stating a limitation is cheaper than a user discovering it.

**Also update the `NOTE_CATEGORY` doc comment** (`cli/index.ts` · `const NOTE_CATEGORY = 'note';` · ~206):
...
```ts
      steps: {
        ...
        items: { ...S_STRING },
        description:
          'Ordered steps for a runbook. Stored as "- [ ] text" lines; progress is never ' +
          'stored in the item.',
      },
```
...
- if the entry validator already refuses unknown keys, **improve the message** to name
  `mycontext add runbook --step` and stop;
...
- Test: `test/core/progress.test.ts`; extend `test/core/audit.test.ts`
...
- Produces:
  ...
  - `export function runbookProgress(records: AuditRecord[], itemId: string): Set<number>` in
...
`mutation` would make `mycontext audit --kind mutation --item RUN-x` a question with a wrong answer.
...
`mycontext runbook step` is a CLI command, and `core/focus.ts` ·
...
command discloses rather than hides: progress is per workspace, so two terminals working one runbook
...
**Why three ops and not one.** `step-done` alone has no reset boundary, so a runbook activated a
second time would inherit the first run's ticks — and a runbook is one-shot precisely so that "is
this finished" has an answer. `step-reset` is written by `runbook activate` (Task 9) and is the
...
arriving for real; this plan does not solve it (quarantine-on-import belongs to the export/import
plan, §6m.10) and must therefore name it in `CHANGELOG.md` (Task 12) as a one-way step.
...
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { runbookProgress, progressLine } from '../../src/core/progress.ts';
import type { AuditRecord } from '../../src/core/audit.ts';

const rec = (at: string, op: string, note: string): AuditRecord =>
  ({ protocol: '1', at, kind: 'progress', op, itemId: 'RUN-x', origin: 'human', note }) as AuditRecord;

test('done ticks accumulate, in log order', () => {
  const done = runbookProgress([
...
test('records for another runbook are not counted', () => {
  const other = { ...rec('2026-08-20T10:01:00Z', 'step-done', 'step 1'), itemId: 'RUN-y' };
  assert.deepEqual([...runbookProgress([other], 'RUN-x')], []);
});

test('no records at all is zero of N — never "unknown" and never a crash', () => {
  assert.equal(progressLine(runbookProgress([], 'RUN-x'), 5), '0 of 5');
});
```
...
`AuditRecord` does **not** change: a progress record uses `itemId` for the runbook, `origin` for who
...
```ts
export function runbookProgress(records: AuditRecord[], itemId: string): Set<number> {
...
cannot parse is **skipped in the count and reported by the caller** — Task 9's `runbook show` prints
...
git add -A
git commit -m "feat(audit): a progress kind — step ticks recorded outside items/"
```

---

## Task 9: `mycontext runbook` — list, show, activate, done, step

**Files:**
- Create: `src/cli/commands/runbook.ts`
- Modify: `src/cli/index.ts`, `src/plugin/commands.ts`, `README.md`, `docs/README.he.md`
- Modify (tests): `test/plugin/commands.test.ts` (`GENERIC`)
- Create (generated): `commands/runbook.md`
- Test: `test/cli/runbook.test.ts`

**Interfaces:**
- Consumes: `Item.steps` (Task 5), `runbookProgress`/`progressLine` (Task 8), `updateItem`,
  `readAudit`/`recordAudit`.
- Produces: `mycontext runbook list|show <id>|activate <id>|done <id>|step <id> <n> [--undo]` and
  `/mycontext:runbook`.

**The lifecycle this command implements, mapped onto what ships (§6m.2 — nothing is added to
`Status`):**

| Stage | Representation | What injects | Command |
|---|---|---|---|
| proposed | `status: draft` | nothing | `mycontext add runbook …` (an agent's lands here by `trustedStatus`) |
| ready | `status: draft` **+ tag `ready`** | **nothing, and not even an index line** | `mycontext edit <id> --tags …` |
| active | `status: active` **+ `always: true`** | the full block, every session | `mycontext runbook activate <id>` |
| done | `status: deprecated` | nothing; counted in `retired` | `mycontext runbook done <id>` |
...
status. A command that set only the status would ship a runbook that is merely eligible: indexed, not
...
`validated` as still governing, so a completed runbook filed there would keep its guarded-field
...
so a finished runbook still appears in a session-visible number instead of vanishing from every tally.
...
`buildIndex` enumerates only eligible items — so a `ready` runbook reaches no index line. §2.1 forbids
...
test('activate sets BOTH status and always — a runbook that is only eligible is a bug', () => {
  withWorkspace((dir) => {
    seed(dir);
    assert.equal(runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'],
      dir, () => {}), 0);
    const shown: string[] = [];
    runCli(['show', 'RUN-rotate-the-webhook-secret'], dir, (s) => shown.push(s));
    assert.match(shown.join('\n'), /status:\s*active/);
    assert.match(shown.join('\n'), /always:\s*true/);
  });
});

test('an activated runbook is injected in full at session start', () => {
  withWorkspace((dir) => {
    seed(dir);
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    const injected: string[] = [];
    runCli(['inject', '--event', 'session-start'], dir, (s) => injected.push(s));
    // The steps are the point: an injected runbook without them is the
...
test('done retires it to deprecated, and it stays counted as retired', () => {
  withWorkspace((dir) => {
    seed(dir);
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    runCli(['runbook', 'done', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    const shown: string[] = [];
    runCli(['show', 'RUN-rotate-the-webhook-secret'], dir, (s) => shown.push(s));
    assert.match(shown.join('\n'), /deprecated/);
    const injected: string[] = [];
...
test('step records progress WITHOUT touching the item — checksum and file do not move', () => {
  withWorkspace((dir) => {
    seed(dir);
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    const before = readFileBytes(dir, 'items/runbook/RUN-rotate-the-webhook-secret.md');
    runCli(['runbook', 'step', 'RUN-rotate-the-webhook-secret', '2'], dir, () => {});
    const after = readFileBytes(dir, 'items/runbook/RUN-rotate-the-webhook-secret.md');
    assert.equal(after, before, 'progress must never enter items/ — spec §6m.3');
    const shown: string[] = [];
    runCli(['runbook', 'show', 'RUN-rotate-the-webhook-secret'], dir, (s) => shown.push(s));
    assert.match(shown.join('\n'), /1 of 3/);
...
test('re-activating clears the previous run', () => {
  withWorkspace((dir) => {
    seed(dir);
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    runCli(['runbook', 'step', 'RUN-rotate-the-webhook-secret', '1'], dir, () => {});
    runCli(['runbook', 'done', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    const shown: string[] = [];
    runCli(['runbook', 'show', 'RUN-rotate-the-webhook-secret'], dir, (s) => shown.push(s));
...
test('step on a runbook that is not active is refused, and names activate', () => {
  withWorkspace((dir) => {
    seed(dir);
    const out: string[] = [];
    assert.equal(runCli(['runbook', 'step', 'RUN-rotate-the-webhook-secret', '1'],
      dir, (s) => out.push(s)), 1);
    assert.match(out.join('\n'), /runbook activate/);
  });
});
...
test('a step number outside the list is refused rather than recorded', () => {
  withWorkspace((dir) => {
    ...
    runCli(['runbook', 'activate', 'RUN-rotate-the-webhook-secret', '--yes'], dir, () => {});
    const out: string[] = [];
    assert.equal(runCli(['runbook', 'step', 'RUN-rotate-the-webhook-secret', '9'],
      dir, (s) => out.push(s)), 1);
    assert.match(out.join('\n'), /3 step/);
  });
});

test('`runbook list` discloses that a ready runbook reaches no index line', () => {
  withWorkspace((dir) => {
    seed(dir);
    runCli(['edit', 'RUN-rotate-the-webhook-secret', '--tags', 'ready', '--yes'], dir, () => {});
    const out: string[] = [];
    runCli(['runbook', 'list'], dir, (s) => out.push(s));
    assert.match(out.join('\n'), /ready/);
    assert.match(out.join('\n'), /not injected and not named in the index/);
  });
});

test('an item that is not a runbook is refused by name', () => {
  withWorkspace((dir) => {
    runCli(['add', 'rule', 'Never log secrets', '--yes'], dir, () => {});
    const out: string[] = [];
    assert.equal(runCli(['runbook', 'show', 'RULE-never-log-secrets'], dir, (s) => out.push(s)), 1);
    assert.match(out.join('\n'), /is a rule, not a runbook/);
  });
});
```
...
Run: `node --test test/cli/runbook.test.ts` → FAIL, no such command.

- [ ] **Step 3: Write the command**

`src/cli/commands/runbook.ts`. Subcommand dispatch modelled on `src/cli/commands/review.ts`;
confirmation/`--yes` on `activate` and `done` modelled on `src/cli/commands/supersede.ts`.

- **`list`** — every `runbook` in the corpus, grouped by lifecycle stage as the table above names
  them, each row `id · stage · N of M · title`. Two disclosure lines, both carrying their condition:
  - for `ready` rows: `"a ready runbook is not injected and not named in the index — the model does
    not learn it exists until \`mycontext runbook activate\` runs. Nothing is lost: it is a draft,
    and \`mycontext runbook list\` is where it is visible."`
...
- **`show <id>`** — the rendered item, with the stored `- [ ]` lines **overlaid** by the session's
  progress: a step in `runbookProgress` prints `- [x]`. Say in the same output that the tick is
  *rendered from the audit log* and the file on disk is unchanged, or a reader will believe the file
  ...
  the count of those.
- **`activate <id>`** — one `updateItem({ id, status: 'active', always: true, origin: 'human' })`,
  then `recordAudit(root, { kind: 'progress', op: 'step-reset', itemId: id, origin: 'human', note: 'activated' })`.
  ...
  Refuse a runbook with no steps? **No** — allow it, and say in the output that it has none, so a
  half-written runbook is visible rather than blocked.
- **`done <id>`** — `updateItem({ id, status: 'deprecated', origin: 'human' })`, behind a preview
  ...
  mode is a runbook left `active` forever, and its guard is that an agent may *ask* and a human
  decides.
- **`step <id> <n> [--undo]`** — refuse unless the item is a `runbook` **and** `status === 'active'`
  (a runbook not yet initiated has no run to record against; the message names `runbook activate`);
...
Register the command and add `import './runbook.ts';` to `src/cli/index.ts`'s command-module import
block.

- [ ] **Step 4: The slash command, and what an agent may and may not run**

`runbook.md` in `genericCommands()`, following `commands/promote.md`'s shape, and stating the split
explicitly:

- `runbook list`, `runbook show` and `runbook step` — **the agent may run these.** `step` changes no
  item, crosses no trust boundary and takes no lock.
- `runbook activate` and `runbook done` — **the agent must not run these.** Use `commands/promote.md`'s
...
Then `'runbook.md'` into `test/plugin/commands.test.ts` · `const GENERIC = [` · ~129, and
`npm run gen:commands`.
...
`mycontext runbook` named in `README.md` (inventory test), added to the non-per-category
**enumeration set** in both documents, counts moved `25` → `26` and `71` → `72`.
...
git add -A
git commit -m "feat(cli): mycontext runbook — the one-shot lifecycle, with progress outside items/"
```

---

## Task 10: `runbook` becomes one-shot in every place it is described

**Files:**
- Modify: `src/core/categories.ts` (description), `src/help/he.ts` (Hebrew description),
  `src/help/index.ts` (`SEEDS.runbook` and `exampleItemShort`),
  `src/help/topics/categories.md`, `src/help/topics/categories.he.md`,
  `README.md`, `docs/README.he.md`
- Modify (tests): `test/help/help.test.ts`
- Regenerate: `commands/add-runbook.md`, `commands/list-runbook.md`, both READMEs' generated blocks

**Interfaces:**
- Consumes: Tasks 5-7 (steps must exist before the seed can carry them) and Task 9 (the commands the
  text names must exist before the text names them).
- Produces: documentation only. Nothing imports it.

**§6m.1 in one sentence:** `runbook` gains the lifecycle and **its description is rewritten to say it
is performed once**; a repeatable sequence stays a `standard` or a `rule` with ordered steps.

- [ ] **Step 1: Make the pinned assertion fail first**

`test/help/help.test.ts` · ``  for (const step of ['1. ', '2. ', '3. ']) assert.ok(runbook.includes(step), runbook);`` · ~268
pins the shipped seed's steps as **body prose**. Rewrite it to pin the new shape, and keep the
comment's argument — the specimen's value is the ordering — while changing what carries it:

```ts
  // `runbook`'s value is the ORDERING, and after v2.0 the ordering lives in
  // `## Steps` rather than in numbered body prose: a runbook seed that reads
  // like an `instruction` would teach the wrong distinction in both READMEs,
  // which print it.
  const runbook = exampleItemShort('runbook', CONFIG);
  for (const step of ['- [ ] ', '- [ ] ', '- [ ] ']) assert.ok(runbook.includes(step), runbook);
  assert.ok(runbook.indexOf('Deploy') < runbook.indexOf('Promote'), runbook);
  // And the box is EMPTY: the shipped specimen must not teach that progress
  // is stored in the file.
  assert.doesNotMatch(runbook, /- \[x\]/);
```

Run: `node --test test/help/help.test.ts` → FAIL.

- [ ] **Step 2: The description, in both languages**

`src/core/categories.ts` · `runbook:       def('runbook', 'RUN', 'normative', true,` · ~40:

```ts
  runbook:       def('runbook', 'RUN', 'normative', true,
    'The steps for one named operation, performed once when a human initiates it'),
```

`src/help/he.ts` · `export const HE_CATEGORY_DESCRIPTIONS: Record<string, string> = {` · ~24 — the
`runbook` value becomes the Hebrew of exactly that sentence. The two tables are compared row for row
by machine facts and by set membership, so both must move in the same commit.

- [ ] **Step 3: The seed**

`src/help/index.ts` · `  runbook: {` · ~270 — the numbered prose becomes a body that says **when to
run it** plus a `steps:` array:

```ts
  runbook: {
    title: 'Rotating the Stripe webhook secret',
    body:
      'Run this when the shared secret leaks or is due for rotation. It is performed once, '
      + 'in this order; rolling the endpoint secret before the next secret is deployed loses '
      + 'every event that arrives in between.',
    steps: [
...
`Seed` gains `steps?: string[]`, and `exampleItemOf` passes it through (the `Item` literal at
`help/index.ts` · `  const item: Item = {` · ~463 already gained `steps: []` in Task 5 — change it to
`seed.steps?.map((text) => ({ text, checked: false })) ?? []`).

**`exampleItemShort` must render steps** (`help/index.ts` · `export function exampleItemShort(type: string, config: Config): string {` · ~414),
or the README's `runbook` specimen shows a runbook with no steps — a specimen that teaches the
opposite of the category. Emit the `- [ ] ` lines after the body, and add to the function's doc
comment why steps earn their place on the same terms `source_file` and the `extra` fields do: they
are what makes this category different from `instruction`.

- [ ] **Step 4: Both topic sources**

Rewrite the `runbook` sections in `src/help/topics/categories.md` and
`src/help/topics/categories.he.md` — same position, same order, both languages. They must now say:

- a runbook is **performed once**, when a human initiates it; a thing you do every time is a
  `standard` or a `rule` with ordered steps;
- the lifecycle in the five rows Task 9's table gives, in prose;
- that activation is `mycontext runbook activate`, and that it is two writes for one reason: `active`
  makes it eligible, `always: true` is what delivers it in full;
- that a `ready` runbook (a draft tagged `ready`) is **not injected and not named in the index** —
  the model does not learn it exists until it is activated;
- that progress is never stored in the item;
- and keep a `**Nearest neighbour: \`x\`.**` line — `instruction` is the right one, and the
  distinction to write is that an `instruction` governs the agent's process continuously while a
  runbook is a single operation performed on a human's initiative.

- [ ] **Step 5: Regenerate and re-check**

```bash
npm run gen:commands   # add-runbook.md / list-runbook.md carry the new description
npm run gen:docs       # the category table and the runbook specimen in both READMEs
```
...
npm test && npx tsc --noEmit && git status --porcelain
git add -A
git commit -m "docs(runbook): one-shot in the catalogue, the seed, both topic sources and both READMEs"
```
...
`72` → `73`, and one sentence in §5 saying what it is for: a category you defined yourself, or one a
...
Tasks 2, 3, 4, 9 and 11 already made the **enforced** edits — counts, enumerations, command names.
This task writes the prose those numbers sit in, which no test can check and which is therefore the
...
- [ ] **Step 3: The runbook lifecycle (English, then Hebrew)**
...
- `done` is `deprecated`, not `validated`, and why: a `validated` runbook would keep governing.
- **What a `ready` runbook does today: nothing.** It is a draft with a tag; it is not injected and
  not named in the index, so the model does not learn it exists until it is activated. Say that this
  is a deliberate open question (spec §2.1) rather than an oversight, and that `mycontext runbook
  list` is where a `ready` runbook is visible.
...
- **Progress never enters the corpus**: it is audit records, counted at display time; the file on
  disk and its checksum do not move when you tick a step; a tick shown in `runbook show` is rendered
  over the stored list. And the limit in the same breath: progress is per **workspace**, not per
  session.
- A runbook is performed **once**. A thing you do every time is a `standard` or a `rule`.

- [ ] **Step 4: The audit log gains a fifth kind**

One paragraph in both documents, beside the existing description of `--kind`: `progress` records step
ticks against a runbook, changes no item, and is separate from `mutation` for the reason `focus` is.

- [ ] **Step 5: `CHANGELOG.md`**

A v2.0 entry naming, at minimum:

- two categories added, catalogue 21 → 23;
- `## Steps` as a **file-format addition** — a stepless item's bytes and checksum are unchanged, and
  ...
  (they do not);
- the `runbook` lifecycle and the three new commands;
- **and the one-way step:** an audit log containing `progress` records cannot be read by v1.0.2,
  ...
  an unknown one. Downgrading is not supported. This is the kind of consequence that must be in the
...
git commit -m "docs: the inbox categories and the one-shot runbook lifecycle, in both documents"
```
...
| §2 / §6m.1 `runbook` becomes the one-shot procedure; no new category | 10, and stated in Global Constraints |
...
§6h, §6j, §6k, §6m.4-§6m.8, §6m.10, §6m.11.

**2. Placeholder scan.** This plan contains **five explicit establish-by-executing points**, each
...
`steps` key and branch on what actually happens); Task 12 step 1 (find the insertion points by
grepping the headings). Three test files name a helper by shape rather than by import path
(`withWorkspace`, `readItem`, `readFileBytes`, `createAgentNote`) with an instruction to establish
the real name by reading the neighbouring suite — that is a lookup, not a decision, and the
assertion beside it is fully specified. No "add error handling", no "similar to Task N", no test
named without its assertions.
...
`runbookProgress(records, itemId): Set<number>` and `progressLine(done, total): string` have one
signature each, given in Task 8 and called in Task 9. The three new CLI command names —
`todo`, `inbox-promote`, `runbook` — are spelled identically in their own task, in
...
- **`computeItemChecksum` includes `steps` conditionally.** No spec section addresses it; the reason
  is `test/core/corpus-checksums.test.ts` and every existing corpus.
...
//   a progress record: { kind: 'progress', op, itemId: '<RUN-id>', origin, note: 'step <n>' }
...
//   runbook · RUN · normative · enabled  — description now says "performed once"

// CLI
mycontext todo [--tag <t>] [--all] [--limit <n>]
mycontext inbox-promote <id> --to <category> [--title <text>] [--yes]
mycontext runbook list
mycontext runbook show <id>
mycontext runbook activate <id> [--yes]     # status=active AND always=true, then step-reset
mycontext runbook done <id> [--yes]         # status=deprecated
mycontext runbook step <id> <n> [--undo]    # writes an audit record; touches no item
mycontext add <category> "<title>" [--step <text>]…   # --step repeatable, order preserved

// slash commands (all disable-model-invocation: true)
/mycontext:todo  /mycontext:inbox-promote  /mycontext:runbook  /mycontext:add
...
needs both 7 and 9. Every task ends with `npm test` green.
