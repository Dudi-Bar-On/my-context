...
took that number from the report and attached it to the wrong noun. For scale: all of `src/`
(code, not the archive) holds 748 ellipses; every tracked file in the whole repository holds
3,452. `fold.js` itself was **102 lines when the matcher's own header first estimated its size**;
...
## 15.2 The floating find panel — regex, whole word, case, opened from the document

**This section is accurate against `HEAD` at the time it was written (2026-09-16) and describes
three tickable options.** It was already one day behind by the time this chapter was
re-verified: an in-flight, untracked lane (`TASK-the-find-panel-offers-regular-expressions-and-no-help-and`,
`semantic/11`) has since exported `MODES = ['normal', 'wildcard', 'logical', 'regex']` from
`fold.js` — four **exclusive** read modes, with `caseSensitive` and `wholeWord` as two independent
booleans beside them (`FindOptions.mode`, `conversation-search.ts`), rather than the three
independent checkboxes described below. That work was not in this reference's scope and had not
landed at `HEAD` when this section was written, so the description below is not a documentation
defect — it is a chapter one refactor away from being current, and is flagged here rather than
silently left to look wrong. Everything below describes the shipped, three-option surface as it
stands at the commit this reference was verified against.

Right-click a turn → *"Search in this conversation /"* opens a floating **Search** panel: the same
find box, plus three options a reader can tick, plus a match stepper and count line, all *moved
into* the panel (not duplicated — see §15.3) while it is open.
...
### The three options, and the measurement behind each refusal or acceptance
...
virtualised-DOM defect this project exists to refuse. All three live in `fold.js`, called
identically by the server's count and the browser's paint, so a checkbox can never draw highlights
that disagree with the number above them (proved directly: with **Match case** ticked, the count
was 8 while fewer than 8 rows were drawn — the page cannot see what it is counting, only the server
can).
...
`byte offset` as a literal costs **48 ms**; the identical words as a regex pattern cost **3 ms** —
**16x faster** — because V8's regex engine prefilters literals natively while the hand-written
folded matcher walks every code point in JavaScript, carrying two offsets at once. So the regex
toggle is not a "power user" mode to be steered around; for a plain phrase it is measurably the
cheaper path.
...
|---|---|
| Extended escapes (`\n`, `\t`, …) | the regex toggle already buys this, in a real spec, at no extra control; both are reachable today by ticking Regular expression |
...
| Prefix `*` | a trigram index already matches substrings — `"byte"*` and `"byte"` return identical rows, so the control would teach a false model of the index |
...
- **This whole surface has no CLI or MCP equivalent.** Regex/whole-word/case-sensitive search over
  one open transcript exists only inside a browser tab with that document already open. Chapter 14
