<!--
  ...
  SCREENSHOTS: the five placeholder blocks are gone and six images stand in
  their place, at the same five points (§15.3, §15.4 ×2, §15.5, §15.6 ×2) —
  `rulings/101`, 2026-09-17, in `15-document-and-lane-viewer.he/`. Every one
  was shot in HEBREW against this repository's own session; NOT ONE is the
  English image flipped, and none may ever be replaced by one. What each
  Hebrew frame carries that its English twin cannot is measured in its own
  caption: the cascade at 24/56/88 from the RIGHT edge, the `⤢` at `left:16`
  against the English `left:1235`, the menu unfurling leftward from the click
  point, and the reference table's left column still reading LTR inside an RTL
  table.

  The English §15.8 sentence that said no screenshot existed is replaced on
  both sides in the same pass, so neither edition now claims the other's
  state.
...
[![חלונית החיפוש במצב ביטוי רגולרי, טבלת הסימוכין פתוחה עד הסעיף האחרון ושורת (X+)+](15-document-and-lane-viewer.he/01-search-panel-regex-and-reference.png)](15-document-and-lane-viewer.he/01-search-panel-regex-and-reference.png)

**החלונית, "ביטוי רגולרי" נבחר, הסימוכין פתוח — פריים אחד, והוא הפריים שנושא את §15.2 ואת §15.3
יחד.** צולם מול הסשן של המאגר הזה עצמו, <span dir="ltr">146.6 MB</span>, עם
<span dir="ltr">`\b\d{3}\.\d px\b`</span> בתיבת החיפוש: שורת הספירה אומרת **13 תורות כאן מכילים
את מה שהקלדתם** ושורת המיקום מתחתיה **אתם על 1 מתוך 13 התורות שמכילים את זה**.

**מה שרק פריים עברי יכול להראות, ונמדד בו:** החלונית יושבת בקצה ה**ימני** — <span dir="ltr">24
px</span> מקצה החלון הימני, <span dir="ltr">`left`</span> שלה הוא <span dir="ltr">808</span> —
וכפתור הסגירה עבר לקצה ה**שמאלי** של הראש, <span dir="ltr">10 px</span> מהשפה השמאלית של הדיאלוג.
<span dir="ltr">`direction`</span> של השורש הוא <span dir="ltr">`rtl`</span>, והעמודה השמאלית של
טבלת הסימוכין — <span dir="ltr">`abc`</span>, <span dir="ltr">`\d \D`</span>,
<span dir="ltr">`(X+)+`</span> — עדיין קוראת משמאל לימין בתוך טבלה ימין-לשמאל.

**החלון היה צריך להיות בגובה <span dir="ltr">3,200 px</span> כדי לקלוט את זה, וזו עובדה על הטבלה
ולא על הצילום.** גובה החלונית חסום ב-<span dir="ltr">`max-block-size:calc(100vh - 4rem)`</span>,
והתוכן שלה עם הסימוכין פתוח הוא <span dir="ltr">2,590 px</span> בעברית. בחלון של
<span dir="ltr">1,000 px</span>, שבו צולם כל פריים אחר בפרק הזה, החלונית היא
<span dir="ltr">780 px</span> ושורת <span dir="ltr">`(X+)+`</span> נמצאת בערך
<span dir="ltr">1,800 px</span> מתחת לתיבת החיפוש, בתוך הגולל של החלונית עצמה. או שהחלון גדל או
שהתמונה היא שתי תמונות. הוא גדל.
...
[![שלוש החלוניות פתוחות בבת אחת מעל מסמך חי, כשהמפל יורד מהקצה הימני](15-document-and-lane-viewer.he/02-three-panels-open-at-once.png)](15-document-and-lane-viewer.he/02-three-panels-open-at-once.png)

**כל השלוש פתוחות בבת אחת, וזו התמונה היחידה שיכולה להוכיח את קביעת האי-מודאליות.** שלוש כותרות
עבריות נבדלות, שלושה כפתורי סגירה, והתמליל עדיין מצויר מאחוריהן וביניהן. נמדד באותו פריים:
<span dir="ltr">`:modal`</span> אינו מתאים לאף אחת מהשלוש — זו התשובה של המנוע עצמו לשאלה אם
<span dir="ltr">`showModal()`</span> נקרא אי־פעם.

**והמפל יורד מהקצה הימני, וזה נמדד ולא הונח.** שלוש החלוניות עומדות על
<span dir="ltr">`left`</span> <span dir="ltr">808 / 776 / 744</span>, כלומר
<span dir="ltr">24 / 56 / 88</span> **מהקצה הימני** של החלון — אותם שלושה מספרים שהפריים האנגלי
מודד מהקצה השמאלי, באותו סדר. זו ההבחנה שצילום אנגלי אינו יכול לשאת.

[![חלונית הצעידה באמצע הליכה, ושורת המיקום שלה קוראת "אתם על 3 מתוך 661 הנקודות המסומנות כאן."](15-document-and-lane-viewer.he/03-step-panel-mid-walk.png)](15-document-and-lane-viewer.he/03-step-panel-mid-walk.png)

**חלונית הצעידה באמצע הליכה**, אחרי "להתחלה" ואז שלוש לחיצות על "הסימון הבא", כך שהמונה מדגים
שאינו על 1: **אתם על 3 מתוך 661 הנקודות המסומנות כאן.** השורה שמעל הבאר אומרת את אותו דבר במשפט
האחר — *נקודה מסומנת 3 מתוך 661* — ונוקבת בסימון שעליו נחתה.

**והפריים האנגלי אומר <span dir="ltr">660</span>, לא <span dir="ltr">661</span>, וזה לא פגם.**
שני הצילומים נלקחו בדקות עוקבות מול סשן שנכתב בזמן הצילום, והמאגר קיבל סימון אחד ביניהם. המספר
בכל כיתוב הוא מה שהתמונה שמעליו באמת מראה.

**בדיקת המספר-בתוך-רצף-עברי עוברת:** שני המספרים קוראים <span dir="ltr">3</span> ואז
<span dir="ltr">661</span> בסדר הזה על המסך, ולא מתחלפים.

**ההליכה שצולמה היא הליכת הסימונים ולא הליכת ההתאמות, וכאן רשימת הצילומים והמסך אינם מסכימים.**
המשפט שהתמונה הזאת נתבקשה עליו, *"אתם על 3 מתוך 40 התורות שמכילים את זה"*, הוא
<span dir="ltr">`conv.nav.place`</span>, והוא מצויר בחלונית ה**חיפוש**, ליד צועד ההתאמות שמחזיק
אותו — <span dir="ltr">`.tvnavfoundplace`</span>, חלונית אחת משם. שתי שורות המיקום של חלונית
הצעידה עצמה הן <span dir="ltr">`conv.nav.placeMarks`</span> ו-<span dir="ltr">`conv.nav.placeYous`</span>,
והראשונה מביניהן היא מה שלמעלה. אותו <span dir="ltr">`drawPlace`</span>, אותה צורה, הליכה אחרת;
המשפט שנתבקש אינו יכול להופיע בחלונית הזאת כלל.
...
[![תפריט הלחיצה הימנית מעל תור מסומן: שלוש הפתיחות, מפריד, ואז שינוי שם וביטול הסימון](15-document-and-lane-viewer.he/04-right-click-menu-over-a-marked-turn.png)](15-document-and-lane-viewer.he/04-right-click-menu-over-a-marked-turn.png)

**התפריט מעל תור מסומן** — שלוש שורות הפתיחה כשתג ה-<span dir="ltr">`/`</span> על הראשונה ואין
מקש על השתיים האחרות, המפריד, ואז שני המעשים שהשורה הזאת יכולה לעשות: *שינוי שם* ו*ביטול הסימון*.
תג ה-<span dir="ltr">`⚑ מסומן`</span> של התור עצמו נמצא בפריים, ולכן הפריים אומר בעצמו מעל איזה
סוג שורה הוא נפתח.

**והתפריט נפרש שמאלה מנקודת הלחיצה, וזה נמדד.** הלחיצה הייתה ב-<span dir="ltr">x = 842</span>
והתפריט תופס <span dir="ltr">602–842</span>: הקצה ה**ימני** שלו הוא נקודת הלחיצה והוא מתפשט
שמאלה. באותו צילום באנגלית הלחיצה הייתה ב-<span dir="ltr">x = 398</span> והתפריט תופס
<span dir="ltr">398–646</span> — הקצה השמאלי הוא נקודת הלחיצה. זה ההבדל שצילום אנגלי אינו יכול
לשאת.

**פריים שני מעל תור לא מסומן הוצע ונדחה.** מעל תור לא מסומן שתי השורות האחרונות הן שורה אחת,
*סמן את הנקודה הזו*, ושום דבר אחר בתפריט אינו משתנה — הרשימה שבראש הסעיף כבר אומרת אילו שורות כל
סוג תור מציע, ואומרת זאת ככלל ולא כדוגמה אחת.
...
[![הכרטיס בלי שום דבר מוקלד: הראש, ואז הבאר, ובלי שום שורת ספירה ביניהם](15-document-and-lane-viewer.he/05-the-card-with-nothing-typed.png)](15-document-and-lane-viewer.he/05-the-card-with-nothing-typed.png)

**הכרטיס בלי שום דבר מוקלד, כשכל החלוניות סגורות — ואין שום שורת ספירה כלל.** זה הפריים היחיד
שהכלל הזה ניתן להראות בו: <span dir="ltr">`p.tvcount`</span> נמצא ב-DOM ומסומן
<span dir="ltr">`hidden`</span>, ולכן אחרי הראש באה הבאר ושום דבר אחר. כל פריים שנלקח בזמן שחיפוש
פעיל היה מצייר את השורה ומוכיח את ההפך.

**ה-<span dir="ltr">`⤢`</span> יושב בקצה השמאלי, וזה נמדד:** <span dir="ltr">`left = 16`</span>
בחלון של <span dir="ltr">1280</span> — באנגלית אותו כפתור עומד על
<span dir="ltr">`left = 1235`</span>. הרצועה שנעלמת בפריים הבא היא המסילה ה**ימנית**, כי בעברית
שם היא עומדת.

**גובה הכרום מעל המציג בפריים הזה הוא <span dir="ltr">67.5 px</span>, שהוא המספר ש-§15.6 נושא;
הבאר היא <span dir="ltr">662.9 × 1034</span>, שאינו.** הסעיף אומר
<span dir="ltr">650.9 × 1034</span> מהדוח המוקדם, באותו חלון <span dir="ltr">1280 × 1000</span>,
והקריאה המורחבת שלמטה תואמת לאותו דוח עד הספרה האחרונה. כלומר <span dir="ltr">12 px</span> ההפרש
הם מתחת לבאר ולא מעליה, והוא אינו מוסבר כאן — שתי הקריאות אמיתיות, של אותו מסך בימים שונים, וזו
שנלקחה יחד עם התמונה הזאת היא זו שהכיתוב הזה מדווח.

[![אותו כרטיס עם פקד ההרחבה לחוץ: המסילה הימנית נעלמה והמציג לוקח את המסך](15-document-and-lane-viewer.he/06-the-card-expanded.png)](15-document-and-lane-viewer.he/06-the-card-expanded.png)

**אותו כרטיס עם <span dir="ltr">`⤢`</span> לחוץ**, באותו חלון: המסילה הימנית הלכה, הפקד דולק,
והבאר היא <span dir="ltr">833.9 × 1248</span>. שני הפריימים הם זוג והשני אינו אומר דבר בלי
הראשון — המספרים בסעיף הזה הם ההפרש ביניהם.
...
- **כל נתון פיקסלים בפרוזה של §15.6 נישא, לא נגזר מחדש**, מ-
  <span dir="ltr">`reports/2026-09-17-the-card-gives-up-its-controls.md`</span>, שלקח אותם. אותו
  דבר נכון לספירות הפגיעות לכל דוגמה ב-§15.3. **שני הנתונים בכיתובי התמונות של §15.6 אינם
  נישאים** — <span dir="ltr">`rulings/101`</span> מדד אותם בפריימים שצילם, על שרת משלו ולא על זה
  של הבעלים, ואחד מהשניים חולק על הפרוזה ב-<span dir="ltr">12 px</span>. הכיתוב אומר זאת במקום שבו
  הוא נוקב במספר; אף אחת משתי הקריאות לא נרדפה עד לסיבתה.
- **ששת צילומי המסך ב-§15.3–§15.6 צולמו על ידי <span dir="ltr">`rulings/101`</span> מול הסשן של
  המאגר הזה עצמו**, דרך שרת שהופעל לצורך כך על פורט משלו, וכל אחד מהם צולם **בעברית** ואף אחד אינו
  התמונה האנגלית מהופכת. כולם הכלי האמיתי; אף אחד אינו ציור. שניים מחמשת מצייני המקום לא סופקו
  כפי שנכתבו ואומרים זאת בכיתובים שלהם: פריים חלונית הצעידה ב-§15.4 מצלם את הליכת הסימונים כי
  המשפט שרשימת הצילומים נקבה בו הוא של חלונית החיפוש, ופריים §15.3 נזקק לחלון בגובה
  <span dir="ltr">3,200 px</span> כי טבלת הסימוכין גבוהה מכך שתיכנס לחלון רגיל. הפריים השני של
  §15.5 הוצע ונדחה.
