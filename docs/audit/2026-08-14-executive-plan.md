...
| ~~lesson-accept preview: `scope: (none — never auto-injected; add one during review if it should activate on files)`; update the stale comments quoting the old wording (`src/lesson/derive.ts:281`, `test/cli/lesson.test.ts:144-147`)~~ **RETRACTED 2026-08-15 — DO NOT EXECUTE.** Spec §3.2 was amended: scope restricts rather than enables, so an unscoped item now applies to every file and the preview's original wording became true. Executing this row would introduce the false claim it was written to remove. The preview instead reads `(none — applies to every file)`, matching `review promote`. | §3.5-F2, R107/R14 | XS | None — verifier confirmed no test asserts the string. |
