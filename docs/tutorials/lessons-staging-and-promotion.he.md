...
**למועמד כלל יש בדיוק חמישה שדות** — ‏`title`, ‏`directive`, ‏`body`, ‏`scope`,
‏`severity` — וכל דבר אחר נדחה בנקיבת שם ולא נזרק.

**קבלה של אחד יוצרת כלל אמיתי, מקושר בחזרה.** הפריט החדש נושא `derived_from` אל
השיעור, כך שהמקור שורד.

**פקודת ה‑slash עוצרת בכוונה לפני הסוף.** ‏`/mycontext:lesson-stage` מדפיסה את
...
‏`--title`, ‏`--scope`, ‏`--severity` ו‑`--directive` על `lesson-accept` מאפשרים
לך לצמצם מועמד בזמן שאתה מקבל אותו. ‏`mycontext lesson-discard <lesson> <key>`
דוחה אחד לצמיתות.

**פקודות ה‑slash.** ‏`/mycontext:lesson` רושמת אחד;
‏`/mycontext:lesson-stage` מעמידה מועמדים ועוצרת.
