...
הבקשה ממשיכה עם הכללים ל-`title`, <span dir="ltr">`summary`, `body`, `scope`, `tags`,
`severity`, `observations`</span> ו-`extra`, אחר כך הנתח עצמו וסכמת ה-JSON, ומסתיימת
בנקיבת שם החזרה:
...
<span dir="ltr">`/mycontext:ingest`</span> מנהלת את שלושתן מתוך סשן, וזו הדרך להשתמש בזה —
...
**פקודות ה-slash.** <span dir="ltr">`/mycontext:ingest`</span> ו-`/mycontext:refresh`.
...
**מה שהשורה יודעת לעשות כאן והממשק לא.** את כל ה-ingest. <span dir="ltr">`ingest`,
`ingest-apply`</span> ו-`ingest-status` אינם בקטלוג הפקודות של הדפדפן, ואף מסך אינו
מצייר סשן ingest.
