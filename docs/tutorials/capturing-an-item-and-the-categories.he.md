...
- קטגוריות **נורמטיביות** (16: ‏`constraint`, `invariant`, `rule`, `requirement`,
...
- קטגוריות **רציונל** (13: ‏`adr`, `decision`, `lesson`, `tradeoff`, `assumption`,
  `edge_case`, `risk`, `measurement`, `reference`, `plan`, `task`, `todo`,
  `note`) — אלה מתעדות למה. הן לעולם אינן מוזרקות; הן היסטוריה שניתן לחפש בה,
...
מיד. *Claude* כותב אחד כזה דרך כלי ה‑MCP‏ `create_item` — והוא נוחת כ**טיוטה**
...
- **`origin`** — ‏`human`, `agent` או `ingest`. שום כלי אינו מאפשר לקורא לקבוע את
...
**פקודות ה‑slash.** לכל קטגוריה יש פקודה משלה, ‏`/mycontext:add-<category>` —
...
**מתוך סוכן**, אותה יכולת היא כלי ה‑MCP‏ `create_item`, וזה מה שגורם לתיעוד
נורמטיבי לנחות כטיוטה.

**מה שהשורה יודעת לעשות כאן והממשק לא.** ‏`--always` (בקשה לשכבת ה‑pinned כבר
בזמן התיעוד), ‏`--original-id` ו‑`--summary-omitted` קיימים רק בשורה. ‏`--note`,
‏`--observation`, ‏`--step` ו‑`--extra` ניתנים לחזרה בשורה; הממשק מרכיב ערך אחד
לכל דגל.
