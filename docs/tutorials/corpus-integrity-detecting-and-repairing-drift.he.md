...
### The other integrity check — בדיקת השלמות האחרת, ולא על הקורפוס שלכם

<span dir="ltr">`mycontext rules verify`</span> נראית אותו דבר ועונה על משהו אחר, ולכן כדאי
להפריד בין השתיים כבר מהפגישה הראשונה איתן.

כל מה שלמעלה עוסק ב**פריטים שלכם**: קבצים ב-<span dir="ltr">`.my_context/`</span> שאתם ו-Claude
כתבתם, שבהם סטייה פירושה שמישהו שינה אחד מהם מחוץ לכלי והשאלה היא אם התכוונתם.

<span dir="ltr">`mycontext rules verify`</span> עוסקת באוסף הקטן של הקבועים שנשלחים **בתוך**
החבילה — עובדות ואיסורים ותקנים על my_context עצמה. לא אתם כתבתם אותם ואי אפשר לערוך
אותם מכאן; הם אינם בקורפוס שלכם, הם לעולם אינם מופיעים ב-`list`, ב-`ready` או
ב-`doctor`, והם אינם מוציאים דבר מהתקציב שלכם. <span dir="ltr">`verify`</span> בודקת כל אחד
מהם מול הסיכום שנשלח איתו ומנקבת בשם כל אחד שחסר, שונה, או לא נשלח מעולם, ו-<span dir="ltr">`--restore`</span>
מחזירה את מה שנשלח, מהחבילה המותקנת ולעולם לא מהרשת.

ההתנהגות האחת שכדאי להכיר: מאגר שאינו מאומת מסרב **כתיבות** ועדיין מתיר **קריאות**.
חסימת קריאות הייתה מענישה אתכם על התקנה שעדיין אפשר להתאושש ממנה — וזה במכוון אינו
אותו דבר כמו סירוב מטעמי תקציב, שאינו קורה בהתקנה שלכם כלל.
