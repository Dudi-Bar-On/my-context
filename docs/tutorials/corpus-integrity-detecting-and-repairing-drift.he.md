...
**פקודות ה‑slash.** ‏`/mycontext:doctor` ו‑`/mycontext:refresh`.
...
**מה שהשורה יודעת לעשות כאן והממשק לא.** ‏`ack`. ‏`repair`, ‏`rebuild`
...
לכל רמה — ‏`error`, ‏`warning`, ‏`notice` — כל שורה נושאת את קוד הממצא שלה,
...
‏`repair`, ‏`rebuild` ו‑`refresh` יושבים בקטלוג של **מרכיב הפקודות**. ‏`rebuild`
