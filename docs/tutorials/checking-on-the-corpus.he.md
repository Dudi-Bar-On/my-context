...
בשקט. ממצאים מגיעים בשלוש רמות — ‏`error`, ‏`warning`, ‏`notice` — וקוד היציאה הוא
0 אלא אם יש שגיאות.
...
**פקודות ה‑slash.** ‏`/mycontext:status`, ‏`/mycontext:doctor`
ו‑`/mycontext:ready`.

**מתוך סוכן**, ‏`status_report`, ‏`doctor` ו‑`ready` הם כלי MCP עם אותן שלוש
תשובות.
